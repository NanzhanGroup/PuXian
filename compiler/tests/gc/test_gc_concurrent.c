...
static LXValue worker_garbage(LXValue* args, int nargs, void* ctx) {
    ...
    int64_t sum = 0;
    for (int i = 0; i < n; i++) {
        LXValue s = lx_str("concurrent-garbage");
        LXValue l = lx_list(4);
        lx_list_push(l, lx_int(i));
        lx_list_push(l, s);   // l 引用 s；函数返回后 l/s 均无外部引用 → 垃圾
        (void)l;
    ...
    lx_chan_send(g_ch, lx_int(sum));   // 阻塞发送：同时验证 chan 在并发 GC 下完好
    return lx_null();
}
...
static LXValue worker_share(LXValue* args, int nargs, void* ctx) {
    (void)ctx; (void)nargs;
    int n = (int)args[0].as.i;
    LXValue lst = lx_list(n);
    for (int i = 0; i < n; i++) lx_list_push(lst, lx_str("shared"));
    lx_chan_send(g_ch, lst);   // 把大列表传给主线程（跨线程可达 → 必须存活）
    return lx_null();
}
...
int main(void) {
    ...
    int live = 0, total = 0;
    int runs0 = lx_gc_stats(&live, &total);

    // --- 场景 A：4 worker 并发分配垃圾 + 主线程同时分配，验证并发 GC 真正执行 ---
    g_ch = lx_chan_create(8);
    lx_set_global("__ch", g_ch);
    LXValue arg = lx_int(20000);
    lx_spawn(worker_garbage, &arg, 1);
    lx_spawn(worker_garbage, &arg, 1);
    lx_spawn(worker_garbage, &arg, 1);
    lx_spawn(worker_garbage, &arg, 1);
    // 主线程同时大量分配（触发 GC → 必须暂停 4 个 worker 后回收）
    for (int i = 0; i < 30000; i++) {
        LXValue s = lx_str("main-garbage");
        LXValue l = lx_list(4);
        lx_list_push(l, lx_int(i));
        (void)s; (void)l;
    }
    ...
    for (int i = 0; i < 4; i++) {
        LXValue v = lx_chan_recv(g_ch);
        assert(v.type == LX_INT);
        worker_sum += v.as.i;
    }
    int runs1 = lx_gc_stats(&live, &total);
    ...
    int runs2 = lx_gc_stats(&live, &total);
    int nshare = 8;
    for (int i = 0; i < nshare; i++) lx_spawn(worker_share, &arg, 1);
    // 主线程继续分配垃圾（触发并发 GC，压力跨线程引用保护）
    for (int i = 0; i < 20000; i++) { LXValue s = lx_str("junk"); (void)s; }
    for (int i = 0; i < nshare; i++) {
        LXValue v = lx_chan_recv(g_ch);
        assert(v.type == LX_LIST);
        assert(lx_len(v) == 20000);                    // 列表完整存活
        LXValue e = lx_index(v, lx_int(0));
        assert(e.type == LX_STR && strcmp(e.as.obj->as.str.data, "shared") == 0);
    }
    int runs3 = lx_gc_stats(&live, &total);
