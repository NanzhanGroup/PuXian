...
// M181（第 59 轮 · 缺陷 193）：**帧内绑定「未初始化即读」** —— 哨兵两指令。
//   背景：帧局部槽的初值原是 calloc 的 PX_NULL，与「已声明且值为 null」**不可区分**
//   ⇒ `def f(): print(x); let x = 42` 在 VM/C 轨**静默给 null**，而解释轨（唯一参考）
//   报 `R1001 未定义变量: 'x'`。同族还有：条件内声明（分支未走）、空循环的循环变量、
//   `let x = x + 1`（修前退化成 R1002「无法相加: null + int」——与根因无关的一条错）。
//   纪律（对齐 C 轨 M181 的 `px_uninit()`）：① 帧入口对**每个 hoist 局部槽**发 UNINIT，
//   参数槽**不发**（参数永远已初始化）；② **只有**「编译器无法证明该声明必已执行」的读点
//   才发 CHKINIT —— 其余发射文本一字不变（常规代码零代价；保守多判只是多一条指令，
//   语义无害：真值已初始化则检查必过）。
//   为什么不做静态拒绝：本仓是 **Python 式函数级作用域**（M62-L5/M169），
//   `let g = fn (): x` 之后才 `let x = 1`、调用发生在声明**之后**是**合法**的
