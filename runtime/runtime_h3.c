...
static bool h3_send_fields(int64_t conn, int64_t sid, LXValue fields, LXValue body_val, int fin) {
    ...
    // M170（缺陷 187 同族）：fields 只活在**C 局部**（创建者 h3_make_request_fields /
    //   h3_make_response_fields 返回时已归还自己的登记）⇒ 下面 px_qd_enc / px_h3_qenc
    //   会分配（⇒ 可能触发 GC）：此时 fields 必须被登记住，否则编码读的是**已释放对象**
    //   （实测：PX_GC_STRESS=1 下客户端发出的字段值错乱 —— :authority 的值变成名字等）。
    //   出口多（含两处 free 后 return false）⇒ 用深度式登记 + 各出口 restore。
    int rd_marks = 0;
    ...
    if (st && st->used && st->qd > 0 && st->enc_sid > 0) {
        ...
        if (nf < 0) { free(q); free(f); px_root_restore(rd_roots, rd_marks); return false; }
    ...
    free(q); free(f);
    px_root_restore(rd_roots, rd_marks);   // M170：本帧登记到此为止
    return sent >= 0;
}
...
static int h3_read_section(int64_t conn, int64_t sid, int64_t timeout_ms,
    ...
    LXValue fields;
    // M170（缺陷 187 同族）：fields 只活在**本帧 C 局部**，而下面取 body 的 h3_take_frame
    //   会走 I/O 与分配（⇒ 可能触发 GC）⇒ 必须在两次 take 之间把它登记住，否则返回的
    //   字段列表是**已释放对象**（实测：PX_GC_STRESS=1 下服务端解出空 method/path/headers）。
    //   用 depth/restore 而非 push/pop：本函数有多个出口（含 free 后 return -1），
    //   逐个配 pop 容易漏；restore 在**每个**出口都把本帧的登记一次清零。
    int rd_marks = 0;
    int rd_roots = px_root_depth(&rd_marks);
    h3conn_state* st = h3_st(conn);
    ...
    PX_KEEP(fields);   // M170：登记点必须在**下一次分配之前**（free 不分配，但下面的 take 会）
    free(hd);
    if (fields.type != PX_LIST) { px_root_restore(rd_roots, rd_marks); return -2; }
    ...
    if (t != H3_FRAME_DATA) {
        ...
        if (t == -1 && px_quic_raw_stream_fin(conn, sid) == 1) {
            ...
            free(bd); px_root_restore(rd_roots, rd_marks); return -1;
        }
    }
    *pfields = fields;
    *pbody = bd;
    *pblen = blen;
    px_root_restore(rd_roots, rd_marks);   // M170：本帧登记到此为止（调用者自会登记它自己的）
    return 0;
}
...
static LXValue h3_fields_to_request(LXValue fields, int64_t sid,
    ...
    LXValue hdr = px_dict(); //   `d=px_dict(); hdr=px_dict(); PX_KEEP(d);` 时 hdr 那次
    PX_KEEP(hdr);            //   分配就可能回收 d（实测即此形态崩溃）
    LXObject* fo = fields.as.obj;
...
static LXValue h3_make_request_fields(const char* method, const char* scheme,
    ...
    LXValue p3 = px_list(2); PX_KEEP(p3); px_list_push(p3, px_str(H3_PATH)); px_list_push(p3, px_str(path)); px_list_push(fields, p3);
    h3_append_headers(fields, headers_val);
    px_root_pop();
    return fields;
}
...
static LXValue bi_h3_serve_read_request(LXValue* args, int nargs, void* ctx) {
    ...
    int64_t conn = args[0].as.i;
    int64_t timeout = px_arg_dur_ms(args[1], "h3_serve_read_request", "timeout_ms");
...
static LXValue bi_h3_client_read_response(LXValue* args, int nargs, void* ctx) {
    ...
    int64_t conn = args[0].as.i;
    int64_t timeout = px_arg_dur_ms(args[1], "h3_client_read_response", "timeout_ms");
    int64_t sid = (conn > 0 && conn <= H3_MAX_CONN) ? g_last_sid[conn - 1] : -1;
    if (sid < 0) return px_null();
    LXValue fields; uint8_t* bd = NULL; int blen = 0;
    int st = h3_read_section(conn, sid, timeout, &fields, &bd, &blen);
...
static LXValue h3_out_make_fields(int status, const char* ct, long long body_len,
    ...
    // HEAD 也带 Content-Length（同 HTTP/1.1 begin：头总是有 Content-Length）
    {
        ...
        LXValue p = px_list(2); PX_KEEP(p);
        px_list_push(p, px_str("content-length")); px_list_push(p, px_str(lb)); px_list_push(hdr, p);
    }
    h3_extra_to_headers(hdr, extra);
    (void)head_only;
    LXValue rf = h3_make_response_fields(status, hdr);
    px_root_pop();
    return rf;
}
...
static LXValue bi_h3_server_listen_stateless(LXValue* args, int nargs, void* ctx) {
    ...
    //   `h3_server_listen_stateless(9000, "c", "k", 多余的)` 被静默接受；`(9000, "c")`（缺 key）也会静默
    //   把 cert 丢掉。M190 的普查只覆盖「三轨 rc **分叉**」，这一类是三轨一致的宽容。
    if (nargs != 1 && nargs != 3)
        px_error("R1002: h3_server_listen_stateless 需要 (port) 或 (port, cert, key) 参数");
    if (args[0].type != PX_INT)
        px_error("R1002: h3_server_listen_stateless 的 port 需要整数，实际是 %s", px_type_name(args[0]));
...
static LXValue bi_h3_server_listen(LXValue* args, int nargs, void* ctx) {
    (void)ctx;
    // M195（缺陷 223）：修前是 `nargs < 1`（**无上界**）——
    //   `h3_server_listen(9000, "c", "k", 多余的)` 被静默接受；`(9000, "c")`（缺 key）也会静默
    //   把 cert 丢掉。M190 的普查只覆盖「三轨 rc **分叉**」，这一类是三轨一致的宽容。
    if (nargs != 1 && nargs != 3)
        px_error("R1002: h3_server_listen 需要 (port) 或 (port, cert, key) 参数");
    if (args[0].type != PX_INT)
        px_error("R1002: h3_server_listen 的 port 需要整数，实际是 %s", px_type_name(args[0]));
    int port = (int)args[0].as.i;
    const char* cert = "";
    const char* key = "";
    // M194：可选的 cert/key **存在即校验**（`null` = 未提供）
