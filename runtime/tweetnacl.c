...
/* ---- M83-S3 (PuXian runtime/runtime_ed25519.c) 扩展：RFC8032 由 32B seed 派生 sk64 ----
 * 官方 tweetnacl-20140427 无 crypto_sign_seed_keypair（crypto_sign_keypair 用 randombytes
 * 自造 seed）。Go x509 PKCS8 导出的是 32B seed（RFC8410），为支持 Go 侧 seed 直签而新增
 * 同名官方 API：入参 sk[0..31]=seed，出参 pk + sk[0..31] 保持 seed、sk[32..63]=pk
 * （即 64B sk = seed||pub，RFC8032 / Go ed25519.PrivateKey 字节布局）。其余逻辑与
 * crypto_sign_keypair 逐行一致（仅 seed 来源不同）。本文件除本函数外与上游逐字节相同。 */
int crypto_sign_seed_keypair(u8 *pk, u8 *sk)
{
  u8 d[64];
  gf p[4];
  int i;
