## M208s1 · **CI 四红收口**（缺陷 272 / 273 —— 「门外的编译档」与「门自己的锚点」）（第 87 轮补）

> M208 推上去后 CI **红在 4 个 job**（`m67-aarch64` / `m67-armv7` / `m67-riscv64` + 质量门的 `m191`），
> 而**本机全量门当时没拦住** —— 两个真因**都不在语言语义里**，而在**判据的环境面**：
> ① 交叉档的编译器比宿主严；② 门自己的锚点按**行号**写死。
> 处置原则：**能本机复现的，就不留给 17 分钟的 CI 往返**（本机本来就有三档交叉工具链）。

### 一 两个编号

| 编号 | 形状 | 修法 |
|---|---|---|
| **273** | `runtime/vm.c` 的 M208 诊断用 `write(2,…)` 却**没 include `<unistd.h>`** —— 宿主 gcc 隐式声明侥幸通过，**三档交叉 cc（clang/musl）一律 `error: call to undeclared function 'write'`** ⇒ CI 的 m67 三个 job 全红 | `#include <unistd.h>`（与 `coro.c` / `runtime.c` 同款） |
| **272** | m191 门的 `scan_errcodes.py` 把「转发豁免」**按行号**锚在 `runtime/coro.c:804` —— M208 给该文件 +32 行 ⇒ 锚点失效；**而报错那一行自己还有格式化 bug**（3 个占位符 2 个实参）⇒ 扫描器 `TypeError` 崩掉，门只报「静态判据全绿 FAIL」，**指不到真因** | 锚点改**内容正则 + 唯一性断言**（找不到 / 不唯一都**响亮指名**）；修掉格式化 bug；`coro.c` 纳入门的 snapshot/restore |

### 二 新增门：`selfhost/check_cross_ports.sh`（交叉档可编译性 · 本机门）

- 三档（aarch64 / armv7 / riscv64）**各自独立**：有工具链 + 库 ⇒ 必须编译成功**且产物架构正确**；
  缺 ⇒ 记 **SKIP 并打印原因**（不冒充通过）；**全档 SKIP** ⇒ 门整体 SKIP 并**指明权威覆盖在 ci.yml 的 m67**。
- `--self-test`：假 cc 记 SKIP（不冒充通过）· 坏源码必判红。已挂进 `selfhost/m116_gates.sh`。
- ⚠️ **建门时自己踩的坑（已修）**：首版把 `/opt/aarch64-linux-musl-cross/bin` 也塞进 PATH ⇒ 它把
  `aarch64-linux-musl-ld` 顶到前面、抢走 clang 包装器该用的链接器 ⇒ 链接期
  `read-only segment has dynamic relocations` **假红**（同一个 cc 只加 `/opt/muslcc-bin` 时是绿的）。
  ⇒ **纪律：门只做「找得到就不动环境」的最小干预；门自己造出的环境差异 = 假红来源。**

### 三 m191 门新增两道负控（新判据必须有独立的牙）

- **F 锚点不唯一**（复制一行）⇒ 必须判红 ✅ · **G 锚点找不到**（改形态）⇒ 必须判红 ✅
- 门结果 **20 通过 / 0 失败**（原 18 + 2）。

### 四 验收

| 项 | 结果 |
|---|---|
| m191 门 | **20/0**（原 16/2 ⇒ 修前本机可复现，与 CI 同签名） |
| 交叉三档（本机复现 CI 失败点） | **3/3 绿**（修前 aarch64/armv7/riscv64 全红） |
| 入库件重烘 | **12/12**（烘期间源码未变 · 62 文件快照一致）|
| `--check-all` / `--check-vm` | **14/14** · 字节码镜像 **40412 行**逐字节一致 |
| 全量门 `m116_gates.sh` | 见提交信息（本轮新增 `cross_ports` 一档）|
| CI / Tag Guard / Release | 见发布（tag `v0.2.0-m208s1`）|
...
> 使用的对象」**全部**是「被某一次**人工**压力筛筛出来的」——「还剩多少」此前**无法回答**。
> 本轮把**跑法本身**做成常设工具 `selfhost/gcstress_sweep.sh`（可重复 · 可分批 · 可复现 ·
> 可判据化），并配六层门 + 三条负控。
> 结果：**新抓到 7 个编号 / 11 处真站点**（缺陷 259–264 · 266），其中 **1 处是静态审计器抓的、
> 6 处是压力筛抓的**；**判据自身也补了五个缺口**（全由「实跑」照出来）。

### 一 这一轮量的是「**谁**在负责发现」

| 已量过的 | 它的盲区 |
|---|---|
| M190「三轨 rc 分叉」 | 三轨**一致**的静默 |
| M194/195/197/198「逐参类型 / arity / 上界」 | **native 桥内部**的 C 局部生命周期 |
| M206「静态形状扫描 + 压力取证」 | 扫描器**自己看不见的形状**（跨行签名 / 消费语义 / 构造函数） |

⇒ 本轮的产出不只是「又修了几处」，而是**把发现动作本身变成可重复的判据**。

### 二 判据（写进 `docs/GC_ROOTS.md` §7）

- **正常档跑两遍** ⇒ 两遍 stdout 逐字节一致（否则 `NONDET`：程序自身不确定，**如实登记**）；
- **压力档**（`PX_GC_STRESS=1 PX_GC_INLINE=1 [+ PX_GC_LIVECHK=1]`）与正常档比：
  stdout 一致 · rc 一致 · 无检测器标记 · 未被信号杀死；
- **确认步（本轮新增）**：`FAIL_OUT` 时压力档**再跑一遍**，两遍自身不一致 ⇒ 降级 `NONDET`
  —— 挡掉「正常档两遍侥幸一致、压力档把计时放大」的假红（实测 `examples/m117_realworld_defects`
  打印「用时 701ms（阈值 700ms）」）；剩余**确定性计时类**进 `KNOWN.tsv`（**人工实跑定性**后登记，
  规模上限 8、每条必须带理由、`SKIP_KNOWN` 随行）。

分类：`PASS` / `FAIL_OUT·FAIL_RC·FAIL_SIG·FAIL_DIAG`（**真信号**）/ `STIMEOUT`（O(n²)，人工定性）/
`NONDET` / `SKIP_NORM`（**正常档就不通** —— 常驻服务、需要参数/fixture、负样例；原因随行）/
`SKIP_KNOWN` / `BUILDFAIL`。

**工程护栏（都是「全量筛跑起来才暴露的」）**：
1. **`timeout` 必须带 `-k <宽限>`** —— 目标忽略 SIGTERM（如 `px_serve` 的优雅关闭）时裸 `timeout`
   **会一直等** ⇒ 整轮**永久卡住**（实测 `s2c_pxserve` 卡住 3 分钟以上、进程都杀不掉）；
   宽限后 SIGKILL ⇒ 每候选**有界结束**。
2. **运行副作用护栏** —— 语料在**自己的目录**里跑（相对路径 fixture 才命中），程序写出的文件会落在
   仓库里（实测 `m155` 写 `.bin`、`m170` 写 SQLite 库 `__DB__`，**都不在 .gitignore 里**）
   ⇒ 每候选跑前记顶层清单、跑完**清掉新增项**（`build` 除外）；`--keep-side` 关闭。

### 三 判据自身的五个缺口（**都由「实跑」照出来** · 纪律：判据的「找不到」先怀疑判据）

| # | 缺口 | 后果 | 修法 |
|---|---|---|---|
| ① | `split_functions` 用**单行**正则匹配 `name(...) {` | **跨行签名**的函数**整体不在审计面上** —— 实测漏掉 `h_exchange`（缺陷 260 所在） | 累积签名行直到「括号闭合后的 `{`」；遇顶层 `;` 判为原型丢弃（函数数 **1456 → 1587**）|
| ② | `CONSUME_RX` 把 `px_dict_set`/`px_list_push` 当「消费」 | 「**先建容器、再往里放东西**」这一族**整族漏报**（漏掉 `bi_os_capture`） | 从「消费」摘出 —— 它们持有**值**，但**不登记接收者** |
| ③ | `px_func_env` 被当作**安全持有者**（担豁免） | MKCLO 的 `env` 漏报（缺陷 262） | 从 `HOLD_RX` 摘出 —— 它**先 `gc_register` 再填 env 字段** |
| ④ | `_ALLOC_NAMES` 缺 `px_iter_at` | **字符串取值会新建串对象**（一条分配路径，缺陷 263） | 补进分配名单 |
| ⑤ | `px_list_n` 被当作**安全持有者**（担豁免） | `spawn_capture` 的输出串被回收（缺陷 266） | 从 `HOLD_RX` 摘出 —— 它内部先 `px_list(n)`（**注册 + 可能触发 GC**）再逐项入列 |

⇒ **判据定稿（两条可复用）**：
**（a）「构造器 vs 持有者」**：`HOLD_RX` 只保留**核对过源码顺序**为「先填字段、后 `gc_register`」的
构造器（`px_ok`/`px_err`/`px_some`/`px_tuple`/`px_struct`/`px_cell`）与「写进**已注册**容器/入根」的调用；
**「看起来会持有」不算数**。
**（b）「未注册的 GC 对象」**：不是「根没指到」，而是「**对象不在 `g_objs` 表里**」——
标记阶段工作表 = 「`g_objs` 里 `gc_mark` 为 1 的项」，未注册的对象**永远不被处理** ⇒ 子对象全不标记。

**锚点自证**：`--self-test` 4 → **8 锚点**（新增「跨行签名」「`px_func_env` 不担豁免」
「`px_list_n` 不担豁免」「跨行签名 + 已登记」）—— 前三条是缺口 ①③⑤ 的**独立**自证。

### 四 收口（7 编号 / 11 处）

| 编号 | 站点 | 修前症状（默认档多为**静默错值**）|
|---|---|---|
| **259** | `bi_os_capture` 的容器在两次 `px_str_len` 之间未登记 | `r["stdout"]` 读到**键名串** ⇒ `int("stdout")` 报 `R1002`（实测 m116 / m130 两个既有门当场红）|
| **260** | `h_exchange`「同名头**首次升级为 list**」时 `nv` 未登记 | 服务端原始响应（curl 视角）是 `X-Multi: r1` + `X-Multi: r2`，px 客户端聚合出 **`[r1, rr]`**（rr = body）|
| **261** | `px_http_dispatch` 的 `.px` 脚本分派：`get/post/server/env/jv/resp` 五个裸 C 局部跨十余次分配 | 脚本拿到的 `GET`/`POST` 变空、`resp.body` 悬垂后响应体错乱（**Mahesvara 静态站 `.px` 通路**）|
| **262** | `vm_run_loop` MKCLO 的 `env` 未登记（循环里 `px_dict_set` 会分配键副本）| 闭包丢捕获 ⇒ `R9001: VM f1:31 CALL 闭包对象缺少所属函数` |
| **263** | `px_as_list` 的 tuple/str 支：先建 list、再在循环里 `px_iter_at` 分配 | `join("-", "abc")` 起即 `[PX_GC_LIVECHK] … px_list_push(list)` + SIGABRT |
| **264** | `px_gen_lazy` / `px_gen_from_list` **不 `gc_register`** | 生成器不在 `g_objs` ⇒ 标记永不处理它 ⇒ 物化 list / seq / transform / filter **全不被标记** ⇒ 被回收。⚠️ **列表源看不出来**（不分配 ⇒ GC 不跑），**字符串源**每取一项都分配 ⇒ 必现 |
| **266** | `bi_os_spawn_capture` 的 `res[1]`（`px_list_n` 之前未登记）| `spawn_capture` 的输出串丢 ⇒ `echo 输出含 hello world` / `sh stdout+stderr 合并` 两项红（实测 m65 自测）|

### 五 判据化（门）

`examples/m207_gcstress/` —— **14 通过 / 0 失败**（`M207-VERIFY-OK`）：

- **① 工具自证**：`--help`（stdout · rc=0）· 未知选项（stderr · rc=2，§7.1 通道口径）·
  候选规模下限 · 候选形状 · **分批完备性**（3 批并集 == 全量，逐字节）· `--only` 生效 ·
  **`--known` 生效**（被豁免的记 `SKIP_KNOWN` + 理由、未列出的照常判定）·
  **`KNOWN.tsv` 形态**（条数 ≤8 · 每条带理由）；
- **② 精选语料**：`probe_gen.px`（生成器 / 可迭代实参 / 闭包环境）· `probe_env.px`（`os_capture`）
  两档必须全绿；
- **③ 正判据**：`--env PX_M207_PERTURB=1` **故意造出 stdout 差异** ⇒ 必须判 `FAIL_OUT` 且 rc=1
  （证明「逐字节比对」这一层真的有牙）；
- **④⑤ 负控 A/B**（真缺陷复现）：撤 `px_gen_lazy` 的 `gc_register`（缺陷 264）·
  撤 `px_as_list` 的 `PX_KEEP`（缺陷 263）⇒ 各自独立判红，源**逐字节还原**；
- **⑥ 负控 C（判据自伤）**：把本工具的逐字节比对改成恒真 ⇒ ③ 必须**不再红**；
- **⑦ 覆盖边界**（如实登记）。

`examples/m206_gcroot/`（M206 门）随判据升级同步：自证 **4 → 8 锚点** · 规模下限
**函数 ≥1500 · 登记站点 ≥200**（扫描面 1456→1587 / 171→208）；基线仍**只登记假阳**，
且**不含任何「真」** —— 本轮新增的候选**全部**是真缺陷 ⇒ 无需扩充基线。

### 六 验收 + 全量筛结果

| 项 | 结果 |
|---|---|
| **M207 门** | **14 通过 / 0 失败**（`M207-VERIFY-OK`）—— 含 3 条负控 + 1 条正判据 |
| **M206 门**（判据升级后） | 自证 **8/8** · 候选 ⇄ 基线 8 条全对齐 · 规模下限 函数 1587 / 登记站点 208 |
| 静态审计 | 25 文件 · 函数 **1587** · 分配站点 **1178** · 登记站点 **208** · 候选 **8**（全为已判定假阳）|
| 入库件重烘 | `--rebake-all` **14/14** · 源码链 `PXSRC-93a1b557af514cd5` · runtime 链 `PXRT-61979043358eaccf` |
| `--check` / `--check-vm` | C 轨 **55 例**逐字节一致 · VM 轨字节码镜像 **40412 行**逐字节一致 |
| 发射冻结门 | **399 → 401 件**（新增 = 本门 2 个探针）· **类别 B 为空** ⇒ 已 `--freeze` 并复核 |
| **全量压力筛（381 候选）** | PASS **184** · SKIP_NORM 162 · STIMEOUT 12 · NONDET 10 · BUILDFAIL 7 · SKIP_KNOWN 1 · **FAIL 5** |

**5 条 FAIL 逐条定性（本轮最值钱的一张表）**：

| 候选 | tag | 定性 |
|---|---|---|
| `m117_realworld_defects/m117_defects.px` | FAIL_OUT → **SKIP_KNOWN** | **假阳 · 计时**（正常 701ms / 压力 702ms，阈值 700ms ⇒ 语义正确）|
| `m89_s3d/vm_spawn_smoke.px` | FAIL_OUT → **SKIP_KNOWN** | **假阳 · 并发顺序**（仅开 STRESS 连跑两次即得两种顺序，`total` 恒 999003）|
| `m173_http_proxy/vhost_gzip.px` | **FAIL_RC** | **缺陷 265（pre-existing · 未修）** |
| `m93_s3/coro_gc_block.px` | **FAIL_SIG** | **缺陷 267（未修 · 家族）**（3 连跑 1/3 通过；LIVECHK `px_list_push(list)`；另一轮 `R1003 25 → 26`）|
| `m88_s3/s1b_gc_stress.px` | **FAIL_SIG** | **缺陷 267 同族**（200 线程 × 300 分配；LIVECHK `px_dict_set(dict)`）|
| `m96_s3/m96_gc.px` | **FAIL_SIG** | **缺陷 267 同族**（150 挂起协程 offload + 1850 分配协程）|
...
服务端不响应（无 `[px-access]` 日志）；同进程内**外部 `curl` 正常 404**（服务端健康）·
`http_request`（连接池）正常。**A/B**：`git worktree` 取 **HEAD（M206）运行时**编同一探针 ⇒ **同样失败**。
影响面：`examples/m173_http_proxy/vhost_gzip.px` 压力档 rc=1。

**缺陷 267（家族 · 未修 · 高优先级）**：**并发 / 挂起执行流在 `PX_GC_INLINE=1`（每次分配即 STW）下
容器被误回收** —— 三个独立语料同一签名（LIVECHK 报「读到已回收对象」，对象类型 = 容器）：
`m88_s3`（200 线程）· `m93_s3`（协程 chan 乒乓 + 让出）· `m96_s3`（offload 挂起协程）。
⇒ 方向：**帧协程 / 挂起协程 / 多线程三者的帧槽根面**在「每次分配即 STW」档下是否**都是精确根**。
⚠️ 本族**只在 `PX_GC_INLINE=1` 下必现**（默认档与仅 STRESS 档多能通过）。

### 八 纪律（本轮新增 / 重申）

1. **判据的「找不到」先怀疑判据**：五个缺口全是**漏报**，且每一处都由「把静态结论拿去**实跑**」照出来。
2. **静态候选不得凭直觉判「假阳」**：`vm.c` 的 MKCLO 候选我一度判成假阳（触发行归因不准），
   **实跑一次就翻案**（缺陷 262）。⇒ 假阳只能由**实跑证伪**后登记。
3. **「测试通过」≠「语料跑过压力档」**：m109 / m116 / m130 / m158 / m160 / m161 / m164 / m165 /
   m169 / m171 / m173 / m177 / m178 这些**既有门**在压力档下红了一片，而它们的默认档一直全绿。
4. **工具也要有自证**：规模下限 · 分批完备性 · 正判据（造差异必须判红）· 负控自伤（撤比对必须不红）。
5. **常设工具必须「有界 + 无副作用」**：`timeout -k`（目标忽略 SIGTERM 时会永久卡住）·
   运行副作用清理（否则把工作树搞脏）。
6. **改静态判据要同步锚点与门的判据**：`--self-test` 锚点数（4 → 8）是 m206 门的判据之一。
7. **「假阳」也要有出口**：`KNOWN.tsv`（人工实跑定性后登记 · 规模 ≤8 · 每条带理由 · `SKIP_KNOWN` 随行）——
   让「已解释的噪声」不再年年重审，同时**不许**把真缺陷塞进去（表头写明这条）。
...
> 副项 = 同轮由门的探针照出来的 **246**（缺失成员上的复合赋值，解释轨拿 `null` 硬算 ⇒ 与编译两轨
> 不同码不同文）。两条都是 **M203 的门顺手把 `m[0] += 10` 印出来** 才暴露的。

### 一 缺陷 245（**高严重度 · 静默错值**）

`selfhost/cg_stmt.px` 在 `Index` / `Field` 目标上**无条件**发射
`px_index_set(o, i, rhs)` / `px_field_set(o, "f", rhs)` —— `op`（`+=` / `-=` / …）**被静默丢弃**。
`Var` 目标那一支走 `cg_assign_op_local`，**一直是**对的。

| 写法 | 解释轨 / VM 轨 | **C 轨（修前）** |
|---|---|---|
| `let m=[1,2,3]; m[0] += 10` | `[11, 2, 3]` | **`[10, 2, 3]`** |
| `let q=[5,6]; q[1] -= 2` | `[5, 4]` | **`[5, 2]`** |
| `let d={"k":5}; d["k"] += 1` | `{k: 6}` | **`{k: 1}`** |
| `struct P: x,y` ; `p.x += 10` | `11` | **`10`** |
| `l[0] = l[0] + 100`（显式展开） | `[101, 2]` | `[101, 2]` ✅ |

⇒ **只坏在"复合"这一条路**；计数器 / 累加器 / 词频统计这类代码在 C 轨**全错且难察**。

**修法**：`op != "Assign"` 时把目标对象/键**先落 `_sN` 临时**（**各求值恰一次**），
旧值 = `px_index(sN, sM)` / `px_field(sN, "f")`，新值 = `<op>(旧值, rhs)`，再 set。
`_sN` 走**专用计数器** `cg_seq_new()`（**不占** `cg_tmp()` 的 `_tN` 序号 ⇒ 其余发射文本一字不变）。

**求值次数与顺序**（守 M165 的**词法左→右**：目标对象 → 键 → 右值）：
`getL()[idx()] += rhs()` 的副作用日志实测 `[o, k, v]`；显式展开对照 `getL3()[0] = getL3()[0] + 1`
⇒ `[o, o]`（两次）—— 门把这个差异**写成判据**（负控 C/D 分别考「顺序」与「恰一次」）。

### 二 缺陷 246（同轮照出）

| 写法 | 解释轨（修前） | VM / C 轨 |
|---|---|---|
| `let d={}; d["k"] += 1` | `R1002 无法相加: null + int` | `R1008 字典没有键 'k'` |
| `let p=P(1); p.z += 1` | `R1002 无法相加: null + int` | `R1008 结构体没有字段 'z'` |

统一为 **`R1008`** —— **与成员的「读」一致**（`d["k"]` / `p.z` 本来就报 R1008；Python 亦为 KeyError）。

### 三 验收（硬数据）

```
门 examples/m204_assign_op/   M204-ASSIGN-OP-VERIFY-OK · **通过 22 · 失败 0**
  [1] 静态 6 项 · [2] 正例 2 组 × 三轨逐字节一致（12 种复合运算符 × list/dict/struct ·
      嵌套 · 显式展开对照 · Var 零回归 · **求值次数与顺序**）
  [3] 拒绝侧 3 例 × 三轨（缺键 / 缺字段 / 越界：rc≠0 + 同码 + **整条消息体逐字相同**）
  [4] 负控 5 道各自独立判红 —— A Index 退回旧形态 · B Field 退回旧形态 · C **顺序违规** ·
      D **重复求值** · E 解释轨 246 退回
入库件      重烘 12/12 · `--check-all` 14/14 · `--check`（55 例）· `--check-vm`（**40412 行**镜像）
自举双轨    `bootstrap_prove`（C 轨）· `bootstrap_prove_bc`（BC 轨 · 40412 行 dump）
对拍        diffcheck **六路全绿** · 发射冻结门 **397 件逐字节一致**（本改动只在 C 轨发射器）
上游回归    100 用例 × 双轨：185 通过 · 0 失败 · 15 跳过 · 缺失 0
引擎一致性  engine_parity ✅ · interp_builtin_parity ✅ · zombie_reap 5/5 ✅
```
...
| 产物 | 归族判据 | 实测 |
|---|---|---|
| `golden/compiler.c` | 编号归一化后**纯加法** | 新增 **37** · 修改 **0** · 删除 **0** ✓ |
| `golden/compiler.bc.dump` | **直接法**（退回 HEAD 的三个源 → 造「旧 C 轨编译器」→ `bc compiler.px` 对拍） | **40282/40282 行逐字符一致** ✓ |
| `golden/s0*.c / v0*.c` | —— | **未变**（无语料用复合索引/字段赋值） |
| `emitc_freeze.sha` | —— | **未变**（397 件一致：本改动只在 **C 轨**发射器） |

⚠️ **`compiler.c` 归族必须「先编号归一化」**：本轮**新增了函数** `cg_seq_new()` 与两个分支 ⇒
编译器自身产物里的 `_vN` / `_tN` / `_ilN` / `px_err_N_*` **整体位移**，逐行 diff 会被编号噪声淹没
（首版实测「不可归类 3651 行」，归一化后 **0**）。M165/M167 的 `norm_num.py` 同款做法。

### 五 门自己的两处判据错（实测抓回）

1. **消息串里的反引号**（`` `Var` ``）被 shell 当命令替换 ⇒ `Var: command not found`（判据本体没错，
   但证据链上多一行假错误）。**这是 M203 同款的第二次** ⇒ 立规矩：**门的 chk 消息串里不写反引号**。
2. **`judge_rej` 的实参用单引号包** ⇒ 消息本身含单引号时（`结构体没有字段 'z'`）引用被打破，
   门报「同文案」失败而三轨其实完全一致。⇒ 改双引号。

### 六 工具坑（第三次重申同一纪律）

`devbuild.sh --vm` 依赖 `/tmp/pxcdev` 已存在；我 `rm -f /tmp/pxcdev*` 后直接跑 `--vm`
⇒ `--emit-c 失败 / failed to run command '/tmp/pxcdev'`。**先跑无参 `devbuild.sh` 再跑 `--vm`**；
且**拿到 dev 二进制先自证含本次改动**（`strings /tmp/pxcdev | grep …`）。

## M203 · **「此类型不支持X / 不可迭代」族的三轨同码同文**（缺陷 243-a/b/c）+ 顺带照出缺陷 245（第 82 轮）

> 主题 = 把一条**四站点只收口了一个**的族补齐（M191 修的是「切片」）：
> 修前同一份源码在三条轨上给出**两/三个答案**，而「迭代」那条更糟 —— 编译两轨借了
> `px_len`，于是 `for x in 1` 报 **`len 不支持类型 int`**：消息里出现的 `len`
> **用户根本没写过**（诊断纪律：错误消息里的名字必须是用户写过的）。

### 一 三个站点（缺陷 243-a/b/c）

| 站点 | 解释轨（修前） | 编译两轨（修前） | 修后（唯一） |
|---|---|---|---|
| 切片 | `此类型不支持切片: <t>` ✅ M191 | 同 | `R1002 此类型不支持切片: <t>` |
...
**为什么「迭代」自成一码**：迭代与索引在 M164 已明确分开（`px_iter_at` ⇄ `px_index`）——
拿索引的词条去描述迭代，正是 M164 要根治的那类混淆。

### 二 实现：**一份实现，三轨共用**

runtime 新增 `int px_iter_prepare(LXValue v)` —— 可迭代（`str`/`bytes`/`list`/`dict`/`tuple`/生成器）
⇒ 返回长度；否则响亮报 `R1002 此类型不可迭代: <t>`。

- **C 轨**：直接发射 `px_iter_prepare(...)`（**两条**发射点：for 循环 + 推导式）；
- **VM 轨**：经**内部全局** `__iter_len` 调**同一个**函数（`__` 前缀 = 内部名 ⇒
  `tools/gen_native_table.sh` 增加过滤、不进公开 native 索引；公开名册仍 387）。

### 三 顺带发现：**缺陷 245（高严重度 · C 轨静默错值）**

```
let m = [1,2,3]; m[0] += 10    解释轨 [11,2,3]   VM [11,2,3]   **C 轨 [10,2,3]**
let d = {"k":5}; d["k"] += 1   解释轨 {k:6}      VM {k:6}      **C 轨 {k:1}**
struct P: x,y ;  p.x += 10     解释轨 11         VM 11         **C 轨 10**
l[0] = l[0] + 100              三轨都对          ← 只坏在「复合」这一条路
```

根因：`selfhost/cg_stmt.px` 在 `Index` / `Field` 目标上**无条件**发射
`px_index_set(o, i, rhs)` / `px_field_set(o,"f",rhs)` —— `op` 被**静默丢弃**（等价于直接赋值）。
`Var` 目标那一支是对的。⇒ 计数器/累加器/词频统计这类代码在 C 轨**全错且难察**。
**归 M204 专项**（要动发射路径 + 重定基 + 守 M165 的求值顺序）。

### 四 验收（硬数据）

```
门 examples/m203_iter_index_msg/   M203-ITER-MSG-VERIFY-OK · **通过 28 · 失败 0**
  [1] 静态（四站点 + 两条发射路径都接了新入口 + `__` 内部名不进公开索引）
  [2] 正例 2 组 × 三轨逐字节一致（7 种可迭代类型 · 两条发射点 · 函数内/闭包内/嵌套/空迭代 ·
      索引 5 型 · 索引赋值 3 型 = **零回归面**）
  [3] 拒绝侧 8 例 × 三轨（rc≠0 + 同码 + **整条消息体逐字相同**）
  [4] 负控 5 道：A/B/C 三个解释轨站点各自退回旧文案（**重编 dev 解释器**后跑）·
      D 迭代入口文案退回「借 len」⇒ **VM 与 C 两轨都必须读到**（= 两条发射路径真接了）·
      E 对 int 放行 ⇒ 响亮退回静默
入库件      重烘 12/12 · `--check-all` 14/14 · `--check`（55 例）· `--check-vm`（**40281 行**镜像）
自举双轨    `bootstrap_prove.sh`（C 轨 · 18452 行，与基准逐字节一致）·
            `bootstrap_prove_bc.sh`（BC 轨 · **40281 行** dump，与基准逐字节一致）
对拍        diffcheck **六路全绿**（lexer/parser/errors/codegen/value/interp）
冻结门      397 件 `--emit-c` 重定基（**类别 A 0 / 类别 B 113 已逐件归族**，见下）
上游回归    100 用例 × 双轨：185 通过 · 0 失败 · 15 跳过 · 期望值缺失 0
引擎一致性  engine_parity ✅ · interp_builtin_parity ✅ · zombie_reap 5/5 ✅
```

### 五 **重定基的「先归族、后覆盖」**（本轮最值钱的方法）

改发射路径必然让**所有含该结构的语料**整体漂移（本轮 397 件里 113 件）。盲目 `--freeze`
会把真回归一起「重定基」掉，所以本轮对**每一类**产物都做了**归族证明**：

| 产物 | 归族判据 | 实测 |
|---|---|---|
| `selfhost/golden/compiler.c`（C 轨自举基准） | 逐行 diff，差异必须**全部**属本族 | 32 行：2 处发射格式串 + 14 处 `_ilN` 发射 ✓ |
| `selfhost/golden/compiler.bc.dump`（BC 轨基准） | 同上（用 difflib 对齐后分类） | 76 行：2 处字符串常量 + `# G 计数 +1` + 11 条全局项（1 新 + 10 位移）+ 62 条 `GETG` 操作数 ✓ |
| `selfhost/golden/s0*.c / v0*.c`（codegen golden） | **逆变换**（`px_iter_prepare(` → `(int)px_len(`）后与旧 golden **逐字符相同** | 7 件全中 ✓ |
| `emitc_freeze.sha`（发射冻结门 · 397 件） | 同上 **+ 直接法**：把三个发射源**临时退回 HEAD** 造一台「旧发射器」，对全部 397 件复现基准 | **397 件逐字符一致** ✓ |

**直接法（值得复用）**：`git checkout HEAD -- <发射源>` → `devbuild` 造旧编译器 → 对全量语料
复现基准哈希 → 还原。比「逆变换」更强（不依赖我对改动族的假设），代价约 4 分钟。

### 六 门自己的四处判据错（本轮实测抓回，记下来防复发）

1. **消息串里的反引号被 shell 当命令替换**（`过滤 \`__\`` ⇒ `__: command not found`）——
   判据本体没错，但**证据链上多一行假错误** ⇒ 去掉反引号。
2. **`judge_rej` 的「三轨同文」等式**：解释轨把 `行:列` 放在 **R 码之后**（`[R1002] 3:1: …`），
   编译两轨用 `R1002: …` ⇒ 一个 sed 同时切两种形态**必有一边为空** ⇒ 8 条假红。
   改用 python 归一化助手（切码后剥 `行:列`）。
3. **`[4E]` 的判据过宽**：`px_iter_prepare` 只被**编译两轨**用，解释轨有自己的 `i_iter`
   ⇒ 放行 int 后只有 VM/C 会静默 ⇒ `judge3`（要求三轨全绿）必然失败。改成逐轨断言。
4. **重定基脚本的 glob 写窄了**：`startswith("s0")/("v0")` 漏掉 `s12`/`s15` ⇒ 那两件没重定基、
   diffcheck 报「19 例中 2 例不一致」。改成 `[sv]\d+_.*\.c`。

### 七 工具坑（记一条纪律）

`selfhost/devbuild.sh --vm pxi` **没有重建 `/tmp/pxcdev`**（沿用旧件）⇒ 我用它跑 C 轨，
看到的是**旧发射器**的行为（`len 不支持类型 int` 依旧），排查了一阵。
⇒ **纪律：拿到 dev 二进制先自证它含本次改动**（`strings /tmp/pxcdev | grep px_iter_prepare`），
同「调试前先 `--version` 确认版本」。

## M202 · **加密/排序扩展面**（base32 · hmac_sha1 · `sorted` key）+ 缺陷 244：**非字符串实参的渲染别名**（第 81 轮）

> 主项 = 第三方 `registry-px` 登记的最后三条**能力缺口**（PX-DEF-032/033/034）**一并收口**；
> 副项 = 由本轮探针**现场照出来**的缺陷 **244** —— `bdata`/`val_cstr` 对非字符串实参
> 共用一处静态缓冲 ⇒ **安全原语静默算错**（`hmac_sha256(123, 456)` 实测 == `hmac(456, 456)`）。

### 一 三条登记（都是「三轨**一致地缺**」——不是分叉，是**能力缺口**）

| 编号 | 形状 | 影响 | 处置 |
|---|---|---|---|
| **PX-DEF-032** | 无 base32 native（RFC 4648 §6） | OTP 秘钥的**标准文本格式**就是 base32（Google Authenticator / RFC 4226 §3.1 的 `secret`）⇒ 官方 `registry/totp` 只能**纯 .px 自实现**解码（`o_b32_decode`：`to_upper` + 5bit 累积），既慢又无法与 C 轨共享 | ✅ `base32_encode` / `base32_decode` / `bytes_base32` / `base32_to_bytes` |
| **PX-DEF-033** | 无 `hmac_sha1`（HMAC 族只有 sha256） | RFC 4226（HOTP）/ RFC 6238（TOTP）的**默认算法**就是 HMAC-SHA1 ⇒ totp 只能用 `sha1_bytes` **手工拼** 64 字节块 + ipad/opad | ✅ `hmac_sha1`（40 hex）/ `hmac_sha1_bytes`（20 字节），底层 mbedtls `mbedtls_md_hmac(SHA1)` |
| **PX-DEF-034** | `sorted` 只收 1 个参数（无 key） | 自然排序等只能靠 `[键, 下标, 原值]` 包装绕行（官方 `registry/natsort` 即此法） | ✅ `sorted(xs[, key_fn])` |

**口径（写进速查表事实 229/230/231）**：

- base32 与 base64 族**逐项对齐**，**同一个陷阱照旧**：`base32_decode` 回的是 **str**，
  而载荷可能是任意字节（含 NUL / 非法 UTF-8）⇒ **要二进制一律用 `base32_to_bytes`**。
...
- `sorted(xs, key_fn)`：**取键恰一次/元素、按原始顺序**（副作用可观测）；比较仍走 `compare_values`
  ⇒ M162 的「值比较 + 稳定排序」**不变**；键相等保持原序；**未提供 key 时逐字节等价于旧行为**（零回归）。
- **明确不做 comparator（自定义比较器）**：语言的全序由 `compare_values` 唯一确定，引入任意 int
  比较器会让「稳定排序」失去可判定性（同一输入可有不同输出），与 M162 立的「同一比较器下输出唯一」
  直接冲突。要逆序写 `fn(x): 0 - x`（数值）或把键包成 `[-k, x]`（通用）。

### 二 缺陷 244（高严重度 · 静默错值 · 安全原语）

**形状**：`bdata()` / `val_cstr()` 对**非 str/bytes** 实参的渲染走**同一处** `static char tmp[64]`
⇒ 同一函数里对**两个不同实参**各取一次指针时，前一个被后一次调用**就地覆盖**：

```
hmac_sha256(123, 456)          ==  hmac(b"456", b"456")          ← key 被 msg 顶掉
regex_match(123, 456)          ==  regex_match("456", "456")     ← 模式被文本顶掉
pbkdf2_sha256(123, 456, 1, 8)  ==  pbkdf2(b"456", b"456", …)     ← 密钥派生的 salt 被顶掉
```

**扫描面**：`runtime/*.c` 静态器命中「多实参指针绑定」**10 个函数** —— `bi_hmac_sha256` ·
`bi_hmac_sha1` · `bi_hmac_sha1_bytes` · `bi_pbkdf2_sha256` · `bi_regex_find` · `bi_regex_match` ·
`bi_regex_search` · `bi_regex_find_all` · `bi_regex_replace` · `bi_regex_split`。
...
**为什么现在才发现**：这一族只在**两个实参都不是字符串**时才发作 —— 现实里
`hmac_sha256(key, body)` 几乎总是字符串；本轮为 `sorted` 写的探针里带了「非字符串实参必须与
字符串形态同值」这一条，把 `hmac_sha1(123, 456)` 印出来才暴露。

### 三 验收（硬数据）

```
门 examples/m202_crypto_ext/   M202-CRYPTO-EXT-VERIFY-OK · **通过 31 · 失败 0**
  [1] 静态 9 项（含「缺陷 244 修法在位 / 旧形态已消失」与名册已重跑）
  [2] 正例 5 组 × 三轨逐字节一致 —— RFC 4648 §10 全 7 向量 · RFC 2202 §3 全 7 向量 ·
      0..12 字节全长度往返 · 取键次数/原序 · 稳定性与**无 key 零回归** · 非串实参同值
  [3] 拒绝侧 6 例 × 三轨（rc≠0 + 同码 + 同文 + 前置标记）
  [4] 负控 5 道各自独立判红 —— A 原生忽略 key · **B 解释轨忽略 key（重编 dev 解释器后跑）** ·
      C 去掉解码合法性校验 · D 稳定性 `>= 0` · E `bdata` 退回共享 static（244 复现）
...
上游回归    100 用例 × 双轨：**185 通过 · 0 失败 · 15 跳过 · 期望值缺失 0**（与 M201 持平）
```

### 四 门自己的两个判据错（本轮实测抓回，记下来防复发）

1. **负控「去掉有效位掩码」没有牙** —— `acc &= (1u << nbits) - 1;` 是**防御性**的：读取窗口恒为
   `[nbits, nbits+8)` 且 `nbits ≤ 7` ⇒ 低位始终由移位累积正确维护，去掉后输出**逐字节不变**
   （门当场判「未判红」）。换成「去掉长度/填充合法性校验」才有牙（d12/d13/d14 从 `null` 变错值）。
2. **独立性判据选错了「无关面」** —— 用 `sorted_key` 当 [4D] 的无关面是错的：它的 `k06`
   本身就**依赖稳定性**（等键保持原序）⇒ 假红。改用 `b32_rt` 才是无关面。

### 五 上游库的「语言面规避」可以退休了

`registry/totp`（上游包，**逐字节照搬、不改**）顶部写着「无 base32 native → 自实现」/
「无 hmac_sha1 → 用 sha1_bytes 手拼」。本轮之后这两条规避**都不再必要**（原生已有一等公民）。
该文件属上游包（`MANIFEST.sha256` 逐字节对拍）⇒ 本仓**不改**，等上游自行改用原生；
本仓只在 `docs/PX_DEF_TRIAGE.md` §10.4 登记「已具备 + 可用」。
...
> arity 文案」。**没有任何一门量过「同一操作 × 每一个位置 × 错类型」**。补上这一面后，实测到两种
> 形状 —— 而且**都不是**"功能不好用"，是"**同一操作三条轨给不同答案**"：
> · **三元兜底**：`(nargs >= 2 && args[1].type == PX_STR) ? args[1]… : " "` 把「**不是**字符串」
>   与「**缺省**」混为一谈 ⇒ `split("s", {})` 解释轨 **R1002**、编译轨 **rc=0 输出 `[s]`**（静默错值）。
> · **入口不同**：解释轨 `ibuiltin.px` 的 `replace` 分支经**方法面**执行（`args[0].replace(a,b)`）⇒
>   位置错时报 `R1007 类型 dict 没有方法 'replace'`，而编译轨是**函数面** `R1002: replace 需要 3 个字符串参数`。
>
> **判据（本轮建立 · `docs/ERROR_CODES.md` §6.10）**：
> **[S10-a] 静态** —— 全部**已注册** native 的逐参守卫声明（**256 函数 / 457 位置**）
>   + 「三元兜底」站点清单 ⇄ **登记表双向对拍**（未登记 0 · 表漂移 0）。
> **[S10-b] 动态** —— 从声明**自动生成**探针（全参类型正确、只第 P 位为毒值 `{}`），合成
>   **一个**驱动器 ⇒ **两次编译**覆盖全量；三轨（解释/VM/C）对拍 `rc + R 码 + 消息体 + stdout`，
>   **分叉必须为 0**（**176** 条探针 / **116** 函数 + 合法侧 10 条）。
>
> ⚠️ **本轮最值钱的工程决定**：一例一次 `build` 是 **136 例 × ~200s ≈ 7.5 小时**（不可行 ——
> M198 的 14 例已接近上限）。把全部用例塞进**一个**程序的 `args()[1]` 分派里之后，两次编译即可，
> 每次执行 ~10ms ⇒ 「逐参类型面」从**抽样**变**全量**。这条工具（`gen_probes.py` + `three_tracks.py`）
> 是后续所有"逐参 × 类型"工作的基础。

### 一 缺陷 237 · **三元兜底族**（5 处静默，同族第 6 处见 238 行）

| # | 站点 | 修前（编译轨实测） | 修后（三轨同码同文） |
|---|---|---|---|
| a | `split(s, sep)` 第 2 参 | `split("s", {})` → **`[s]`**（静默按单空格切） | `R1002: split 参数需要 string` |
| b | `exit(code)` | `exit({})` → **静默退出 0** | `R1002: exit 的 code 需要整数，实际是 %s` |
| c | `udp_open(port)` | `udp_open("x")` → **静默记 0**（= 系统随机分配） | `R1002: udp_open 的 port 需要整数，实际是 %s` |
| d | `ws_heartbeat(conn, interval_ms, timeout_ms)` | 非 int → **静默取默认值** 10000 / 60000 | `R1002: ws_heartbeat 的 interval_ms/timeout_ms 需要整数，实际是 %s` |
| e | `tls_server(cert, key, hostname)` | 非 str → **静默当 NULL**（= 静默不启用 SNI） | `R1002: tls_server 的 hostname 需要字符串，实际是 %s` |
| f | `quic_connect(ip, port, alpn, session)` | 非 str → **静默当 NULL**（= 静默放弃会话恢复） | `R1002: quic_connect 的 session 需要字符串，实际是 %s` |

**边界（有意保留、写进 §6.10）**：可选位置的 `null` **仍表示「未提供」**（§6.3）；
时长族的 `0` / 负数**仍走文档化的默认值分支**（那不是类型错）。

### 二 缺陷 238 · 解释轨 `replace` 经**方法面**路由

`replace({}, "a", "b")`：解释轨 `R1007 类型 dict 没有方法 'replace'`（p1/p2 则给
`方法 replace 参数 N 需要 string`）⇄ 编译轨 `R1002: replace 需要 3 个字符串参数`。
⇒ `ibuiltin.px` 该分支**先**按**函数面**口径守卫（与 C 层 `bi_replace` 同码同文），再执行。
（探针 `replace__p0/p1/p2` 现在三轨完全一致。）

### 三 缺陷 239 · **发布包架构卫生**（aarch64 包内夹带 x86-64 / Windows 构建输入）

aarch64 引导包里带着 `runtime/third_party/sqlite3/sqlite3.o`（**x86-64**）、`sqlite3-windows.o`
（**COFF**）、`mbedtls/lib/*.a`、`zlib/lib/*.a`、`openssl|ngtcp2/lib/*.a`（均 x86-64）——
M168 的可移植性门只判**可执行件**，包内**构建输入**无人看。两重后果：① 白送体积；
② **错架构陷阱** —— aarch64 用户按 README 的路径家族显式 `--sqlite-obj …/sqlite3.o` 会拿到
`file in wrong format`（错误信息**指不到根因**）。

- 新增 `selfhost/check_pkg_arch.sh`：逐文件 + **逐归档成员**判架构（PE/COFF **一律判红**；
  非 ELF 记 INFO 并计数，不静默）；`--self-test` 自带 **5 条自证（3 负控）**，fixture **取自仓库**
  （x86-64 对象 / aarch64 对象 / COFF 对象 / 两架构静态库）⇒ **判据与环境无关**（M168 教训：负控写死
  「本机架构」会在对应 runner 上恒绿）。
- `release.yml` aarch64 打包步：**裁剪**异架构/跨平台输入 → 跑架构卫生门 → **从裁剪后的包内**
  再跑一次零参数 `build` + 运行（证明没裁掉 aarch64 必需项）。
- 实测（模拟包，本机）：裁剪前 **判红 1266** → 按口径裁剪后 **判红 0**（129 件全 AArch64）；
  runtime 子树 **44M → 20M**。
- 诚实边界：本机是 x86_64，**包内自证**必须在 aarch64 runner 上跑（release.yml 正是如此）——
  故本地不跑该自证，避免产出依赖宿主架构的假结论（已写进证据脚本注释）。
- `ci.yml` 质量门新增 `check_pkg_arch.sh --self-test`（零成本防漂移）。

### 四 门与验收

```
门 examples/m199_argtype/   **M199-VERIFY-OK · 通过 19 · 失败 0**
  [1] [S10-a] 静态：未登记 0 · 表漂移 0 · 结论一致 + 覆盖下限（≥240 函数 / ≥430 位置）
  [2] [S10-b] 动态：探针 176 条 · 三轨**分叉 0**（rc + R 码 + 消息体 + stdout 全等）
  [3] 显式词条 6 条（split/replace×2/exit/udp_open/tls_server 三轨同码同文）
  [4] 合法侧 LEGAL.tsv（10 例）三轨 rc=0 且 stdout 逐字节一致
  [5] 负控 4 道**各自独立判红**：A 编译轨退回三元兜底 · B 解释轨删守卫后**重烘 pxi** ·
      C 登记表删一行（未登记判红）· D 登记表加幽灵站点（表漂移判红）
      + **B 附带自证**：还原后重烘 pxi 与入库件**逐字节一致**（证明重烘确定性）
入库件      --rebake-all 12/12 + pxc/pxc_vm · --check-all **14/14**（PXSRT 见下发物）
```

### 五 三条教训（写进纪律）

1. **扫描器只收 `static LXValue bi_*` 会静默漏函数** —— `runtime_ws.c` 等 **12 个文件**用非 static
   定义（`LXValue bi_ws_heartbeat(...)`，共 9+ 处），首版漏掉 `ws_heartbeat` 整函数 = **假绿**。
   ⇒ 声明扫描器必须收 `static`/非 `static` 两种定义（解析数 315 → **370**）。
2. **绝不并发跑两次重烘** —— 我在 `rebake_bin.sh --rebake-all` 跑到「步骤 3/4」时又启动了
   `--entries=pxi`，结果 `pxc_vm` 的内嵌指纹读回成**随机值**（`3cebc2c082e2c8d3` ≠ `PXSRC-f87f5d14…`），
   整轮判红；无争用时重跑**一次通过**。⇒ 与 M191 的 `m116_gates.sh` PID 锁同族：
   **共享产物的重活必须串行**。
3. **聚合驱动器 > 逐例 build** —— 见上文「最值钱的工程决定」；本轮把 136 例从"不可行"变成"两次编译"。
...
> **收尾补记（同轮第二次提交）**：
> · **上游 `registry-px` 同步到 `7da3397e`**：引入器新增 **`--update`** 通道（上游"**同一版本目录就地改**"
>   的唯一正规写法；默认仍是冲突即拒）。实测 **4 件就地更新**（cli / pg / mysql / passhash）·
>   mysql **删 `sha1.px`**（改用 M188 加的原生 `sha1`）· **新增 `stmt.px`**（参数化查询）·
>   **撤销 passhash 本地补丁**（上游自修 `is_int_str` 守卫 —— 与 M189 的补丁同法）·
>   上游 `cli.px` 也改用原生 `is_int_str`（消费 M185）⇒ **补丁列现已全空：53 包逐字节照搬**。
>   `upstream-tests/` 增 2 例（`mysql_params_test` / `pg_params_test`，需真实服务端 ⇒ SKIP 且登记理由）。
>   回归：**通过 99 · 失败 0 · 跳过 11 · 期望值缺失 0**。
> · **三处"门自身"随改动更新**（都是本轮改动**打到旧门锚点**上）：m194 的前置锚点
>   （`px_arg_int` → `px_arg_dur_ms`）；m194 / m195 的 [S5] 计数由**等式 137** 改为**下限 ≥137**
>   （M197 纪律 —— 本轮新增 2 处守卫 ⇒ 实际 139）；m187 的 passhash 判据
>   （补丁撤销 + **补丁通道保留** + **全部**包补丁列为空）。⇒ **第六次**撞"改旧行前先 grep 旧门锚点"。
> · **缺陷 229 的一处方向修正**：`tcp_opt` 的 `read_timeout_ms` / `write_timeout_ms` **包含 `0`**
>   （`0` = **清零超时**，M149 门 D9 实测）—— 第一版写成 `> 0` 是**回归**，被门当场抓回。
> · 重烘：**12/12 + pxc/pxc_vm** ⇒ `--check-all` **14/14**（新指纹 `PXRT-dcd11607fa2d6aba`）。
> · 收尾门（逐门独立跑）：m149 ✅ · m194 ✅ · m195 ✅ · m187 ✅ · m198 ✅ · 上游回归 ✅。
...
> **发布物独立复核（M198 · `v0.2.0-m198`）**：把 aarch64 引导包下载解包复核 ——
> sha256 与发布方 `.sha256` **逐字符一致**（`8646ed60…2f69`）· 包内 **12 件可执行全部 `ARM aarch64`
> + `NEEDED=0`（全静态）+ GLIBC 引用 0** ⇒ M168「动态件装不上」的病未复发。
> ⚠️ **登记一处打包卫生问题（→ M199）**：包内 `runtime/third_party/sqlite3/sqlite3.o` 是 **x86-64**
> 对象文件（另有正确的 `sqlite3-aarch64.o`）；它是**对象文件**、不会被当程序执行，
> 且 `tools/px` 的 rtcache 选料**按 `file` 判架构**（M169 起）⇒ 影响有限，但**不该出现在 aarch64 包里**
> （既是 ~1.5MB 冗余，也是"拿到错架构对象"的陷阱）。修法方向：打包前按目标架构过滤 `runtime/third_party/**`。
> 复核脚本的自身缺陷（第四次"工具骗我"）：v1 用 `-name 'px*'` 把 **`.px` 源文件**也当二进制判架构；
> v2 的 `case` 模式写成 `*AArch64*`（`file` 输出是小写 `aarch64`）⇒ 12 件**假报**架构不符。
