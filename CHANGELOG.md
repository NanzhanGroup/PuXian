## M217 · **移位计数的「未定义行为」收口 —— 同一份 C 在 `-O0`/`-O2` 下给两个答案**（缺陷 309）（第 96 轮）

> 主题：M215（算术溢出 UB）→ M216（浮点→int 转换 UB）→ 本轮的**第三弹**：
> `a << n` 在 `n < 0`、`n >= 64`、或**左移结果不可表示**时，是 **C11 6.5.7p3/p4 的未定义行为**。
> M215 收尾时曾把它登记为「未文档化」候选 —— 本轮把它钉死：**不是"未文档化"，是没有定义**。

### 一 缺陷 309（三个面）

| 面 | 形状 | 实测 |
|---|---|---|
| **309-a · UB / 优化档相关** | `a << n` 的裸移位（`px_shl`/`px_shr` 与 VM 轨 `PXOP_SHL/SHR` 快路径） | 同一份 C：`f(1,64)` **-O0 ⇒ 1**（`shl` 指令掩码）· **-O2 ⇒ 0**；`f(1,65)` **2 ⇒ 0**；常量 `1LL << 64` ⇒ 0（gcc 直接给 `-Wshift-count-overflow` 告警） |
| **309-b · 又一处「只收口一个」** | 同族 4 个移位里，`px_ushr` / `PXOP_SHRU` **早已**显式 `& 63u`（注释明写「与解释器 `wrapping_shr` 一致」），而 `px_shl`/`px_shr`/`PXOP_SHL`/`PXOP_SHR` **一直是裸移位** | 「同一语义面只收口一处」—— M191 / M203 / M206 / M215 之后的**第 N 次复发** |
| **309-c · 修前静默错值** | `n < 0` 走掩码路径（`-1 & 63 == 63`） | 干净树 HEAD 实测：`8 << -1` **三轨一致地**给 `8`（运行期形态）或 `0`（常量形态）⇒ 与 M216 同款「静默错值」 |

**为什么旧门按定义看不见**：三轨跑的是**同一份运行时代码**（`px_shl`/`px_shr` 或 `shl` 指令），
输出**逐字节一致** ⇒ 「三轨对拍」这条判据恒真；而「常量化简」在 PuXian 侧**不存在**
（编译器不做算术常量折叠）⇒ 常量与运行期也一致。**它靠的是「恰好都用 `shl`」**。

### 二 修法（4 处收敛 + 1 条显式语义）

`runtime.h` / `runtime.c` 新增 **唯一实现**：

| 函数 | 语义 |
|---|---|
| `px_shift_count(n, what)` | `n < 0` ⇒ **`R1003` 响亮**；否则返回 `n & 63` |
| `px_shl64(a, n)` | **经 `uint64_t`** 左移（无符号回绕**良定义** ⇒ 连「结果不可表示」那半 UB 也消除） |
| `px_shr64(a, n)` | 算术右移（计数已钳在 `[0,63]`） |
| `px_shru64(a, n)` | 逻辑右移（按 `uint64_t` 解释） |

**四处收敛**：`px_shl`/`px_shr`/`px_ushr`（C 表）· VM 轨 `PXOP_SHL/SHR/SHRU` 三个快路径 ·
解释轨 `i_bin_numeric` 的三个分支（**守卫写在三个分支之前** ⇒ 一处覆盖三个，不会再「只收口一个」）。

**语义决策（两条，各有依据）**：
- **`n >= 64` 保持掩码**（`1 << 64 == 1`）—— 与本仓既有 `>>>`/`px_ushr` 口径、
  以及 Rust `wrapping_shl/shr` **一致** ⇒ **零行为变更**；改的只是「靠 UB 碰巧」→「显式良定义」。
- **`n < 0` 改为响亮 `R1003`** —— **这是行为收紧**（修前静默给 `INT64_MIN`）。
  依据：Python `ValueError: negative shift count` · Go run-time panic · 且负计数**没有"合理值"**
  ⇒ 与 M216（`int(1e30)` 越界响亮）**同向**。生态扫描（本仓 212 处移位用法人工抽看）
  **未发现负计数**；常见形态是 `x >> 6` / `(0x10000 >> m)`（m 为月份 1..12）等**小常量**。

### 三 门 `examples/m217_shift_count/`（**M217-VERIFY-OK**）7 层

- **语料生成器是单一事实源**（`gen_cases.py` → `cases.px` + `CASES.tsv`，第 ① 层做**漂移检测**）
- **19 例**（合法 10 / 拒绝 9）× **三轨对拍**，且**与 Python 独立真值**一致
  （`px_shl/px_shr/px_shru` 用 bignum 按**定义式**算，与被测 C 实现**不同构**）：
  · 计数 **0..63 全量扫描** × 5 种操作数（1 / −1 / −8 / `INT64_MIN` / `INT64_MAX`）
  · 越界计数 64/65/66/127/128/129/255/256/1000/4095/`INT64_MAX`
  · **常量形态 vs 运行期形态** —— 逐例断言**两者同答案**
  · 位运算族回归 + 链式组合
- **第 ③ 层是「UB 举证」**（静态 · 可复现）：同一份 C 的**三种 UB 症状**逐项测量 ——
  `UB_SYMPTOMS=3`（告警 + 优化档不同 + 常量/运行期不同）⇄ 新形态 **`NEW_SYMPTOMS=0`**；
  反汇编 `OLD_HAS_AND=0` ⇄ `NEW_HAS_AND=1`（`-O0 -fno-inline` 下看得见 `and $0x3f`）。
- **负控 3 道各自独立判红**：**A** 撤回 C/VM 轨守卫 ⇒ 拒绝侧 **9 例判红** ·
  **B** 撤回解释轨守卫 ⇒ **9 例判红**（三轨分叉）· **C** 判据自伤 ⇒ A 的红**消失**。
- ⚠️ **门自己的两个坑（如实登记）**：① 首版把 `old.c`/`new.c` 放**同一 TU** 测告警 ⇒
  新形态被旧函数的告警牵连 ⇒ 假红；② 首版用 `-O1` 编 objdump 目标 ⇒ 调用点常量被传播、
  `n & 63` 被折叠掉 ⇒ `NEW_HAS_AND=0` 假红 ⇒ 改 `-O0 -fno-inline`。

### 四 验收

- 入库件**重烘 12/12** · `--check-all` **14/14** · `--check`（55 例）· `--check-vm`（字节码镜像）
- 发射冻结门：**类别 A/B 判定**见 `emitc_freeze`（本轮**不改发射层** ⇒ 只新增本门语料）
- **全量门 `m116_gates.sh`**：见下
- 上游 `registry-px` 回归：随全量门

## M216 · **浮点→int 的「未定义行为」收口 —— 三轨一致地错，且两个架构各错一种**（缺陷 308）（第 95 轮）

> 主题：M215 收尾时留了一句「**浮点→int 转换族疑似平台相关**」。
> 本轮把它钉死：`(int64_t)f` 在 `f` 越界/非有限时是 **C11 6.3.1.4p1 的未定义行为**，
> 两个架构各自「选了一种」—— x86_64 `cvttsd2si` 给 **`INT64_MIN`**（不定值哨兵）、
> aarch64 `fcvtzs` **饱和**给 `INT64_MAX` ⇒ **同一份源码打印不同的数**，且都是**静默**的。
> 一句话：**这不是"某个平台的行为"，这是"没有定义的行为"。**

### 一 缺陷 308（两个面）

| 面 | 形状 | 修前实测 |
|---|---|---|
| **308-a · UB / 平台相关** | 全仓**各自**写 `(int64_t)f`（`int()` 内建 · `//` 的浮点支 · 内部的 `int_val`） | `int(1e30)` · `int(-1e30)` · `int(1e19)` · `int(float(9223372036854775807))` · `1e30 // 1.0` ⇒ **三轨一律** `-9223372036854775808`（x86 的 `cvttsd2si` 不定值）；同源码在 aarch64 上会得到 `INT64_MAX` |
| **308-b · 三轨分叉** | **切片界**：解释轨让 float **直接参与整数比较**，编译两轨走 `int_val` 的 `(int64_t)` | `l[0:1e30]` **解释 `[1,2,3]` vs VM/C `[]`** · `l[1e30:2]` **解释 `[]` vs VM/C `[1,2]`** · `bytes_slice(b,1e30,2)` 三轨都**静默给错值** |

### 二 修法（4 处 + 1 处安全网）

| 位置 | 修法 |
|---|---|
| `runtime.h` / `runtime.c` 新增 **`px_f2i(f, what)`** | 浮点→int 的**唯一**入口：非有限（NaN/±Inf）或落在 `[INT64_MIN, 2^63)` 之外 ⇒ **`R1003` 响亮**；区间内 ⇒ 向零截断（此时 `(int64_t)` **良定义**） |
| `bi_int`（`int()` 内建） | `(int64_t)a.as.f` ⇒ `px_f2i(a.as.f, "int()")` |
| `px_idiv` 浮点支 | `(int64_t)floor(…)` ⇒ `px_f2i(floor(…), "整除")` |
| `px_slice` / `bi_bytes_slice` | 新增 **`px_req_slice_idx`**：切片界与**索引位**同口径（M179）⇒ 非 int ⇒ `R1002: 切片索引必须是整数，实际是 <t>` |
| `int_val`（内部入口） | **不再静默截断 float** —— M179/M194 收紧的只是 native **参数位**，内部位一直留着同一条口子 |
| `selfhost/ival.px` 的 `i_slice` | 补同口径守卫（**解释轨**此前让 float 界直接比较 ⇒ 这才是分叉的另一半） |

**语义决策的依据**：与同族的 `px_str_to_i64`（「str→int 溢出即非法 ⇒ 响亮」）**同向** ——
「转换」的越界不走「回绕」（那是 M215 对**算术**的选择），而是**响亮**。
⚠️ **上界是 `2^63` 而不是 `2^63-1`**：`9223372036854775807.0` 在 double 里**本身就是** `2^63`
⇒ `int(float(INT64_MAX))` 报错**才是诚实的**（那个 double 不等于该整数）。
**没有照抄 Go**：Go 的 `int64(f)` 同样是 implementation-dependent。

### 三 门 `examples/m216_float_to_int/`（**M216-VERIFY-OK**）7 层

- **语料生成器是单一事实源**（`gen_cases.py` → `cases.px` + `CASES.tsv`）· 第 ① 层做**漂移检测** + 规模下限（22 例 / 合法 9 · 拒绝 13）
- **驱动器一次编译**（M199 教训）：按 `args()[1]` 分派 ⇒ 每例只跑不编
- **合法侧真值由 Python 独立算出**（`int(f)` 向零截断 / `//` floor —— 与被测实现**不同构**）：**22 例三轨逐字节一致且与真值相符**
- **第 ③ 层是「指令级平台举证」**：同一段旧形态 `(int64_t)f` 用宿主 gcc 与 `aarch64-linux-musl-gcc` 各编一份 ⇒ `objdump` 得 **x86 `cvttsd2si` ⇄ arm `fcvtzs`**（两架构指令不同 + 无守卫）；新形态两侧**都有守卫**（`ucomisd` / `fcmpe`+分支）⇒ 把「平台相关」从断言变成**可复现的静态证据**
- **负控 3 道各自独立判红**：**A** 撤回编译轨 4 处守卫（8 个补丁点）⇒ **13 例判红** · **B** 撤回解释轨 `i_slice` 守卫 ⇒ **3 例判红**（分叉复现）· **C** **判据自伤**（把所有 `bad.append` 换成 sink）⇒ ④ 的红**消失**（证明红来自比对逻辑）
- ⚠️ **门自己的两个坑（本轮如实登记）**：① `devbuild.sh` 的约定是「**给了名字就只编那些**」（`[ -n "$NAMES" ] || NAMES="pxc"`）⇒ 首版写成 `devbuild.sh pxi --vm` 会**不重建 `/tmp/pxcdev`**，于是「C 轨编译器还是上一版」而门**看不出原因** ⇒ 修法 = 显式 `pxc pxi --vm` + **新增「溯源行」**（把 `rtcache` 键与三个件的 sha256 打进日志）；② 负控 C 首版只把「三轨一致性」判据改恒假、**漏了拒绝侧的两条比较** ⇒ 仍报 10 例、NC-C **假红**（会误判"红不来自比对"）⇒ 改为**全量 sink**。

### 四 ⚠️ 全量门抓回的一条**连带回归**（本轮最值钱的一段 · 已固化为语料）

`m148_ieee_div` 在第一次全量门里判红：**解释轨 rc=1**
```
运行时错误 [i_fmt_float 行120]: R1003: int() 超出 int64 范围: 1.7976931348623157e+308
```
- **真因**：`selfhost/ival.px` 的 `i_fmt_float` 用 `int(f)` 做**整值判断**，而它的守卫是
  按 **double 范围**（`f > DBL_MAX`）短路 ±Inf/NaN —— **`DBL_MAX` 本身不算越界** ⇒
  落到 `int(DBL_MAX)`。修前它静默给 `INT64_MIN`「碰巧」不出错
  ⇒ **那段代码是拿缺陷本身当护栏**；M216 把越界改成响亮 ⇒ 护栏没了，回归立刻显形。
- **修法**：前置条件由 **double 范围** 改为 **int64 范围**，且只在真正需要时求值：
  `let is_int_val = absf < 1e15 and f == int(f)`（`and` 短路 ⇒ `int(f)` 必在范围内，语义不变）。
- **固化**：该条回归已作为**合法侧 4 例**（`ok_fmt_dbl_max` / `ok_fmt_dbl_neg` /
  `ok_fmt_int_like` / `ok_fmt_sci`）进 M216 语料（22 → **26 例**）。
  ⚠️ 这 4 条的期望值是**语言约定**（最短 roundtrip · spec §十三.7/13.8），
  **不是** Python 的 `str(float)`（`str(1e15)` Python 给 `1000000000000000.0`，PuXian 给 `1e+15`）。
- ⚠️ **方法论教训（规矩升级）**：M194 立的「收紧类型口径前先看生态用法」，
  本轮只做了**一半** —— 我扫了**语法形态**（全仓 1236 文件扫「切片界含 `/` 或 float 字面量」），
  **漏了「该原语的既有消费方」**（`int()` 被 `i_fmt_float` 当**范围探测**用）。
  ⇒ **新规矩：收紧一个共享原语前，既扫「语法形态」，也扫「全部消费方」。**
- ⚠️ **第二次全量门又抓到一类**（`m189_bytes_bounds` 的**负控 D** 失效）：该负控的靶心是
  「`int_val` 对 float **静默截断**」那一半，而 M216 已把它改成**响亮** ⇒ 那半条判据被
  **新修复吸收**、永远不成立 ⇒ 负控**失去牙**（M183 记过的老坑：**新修复会吸收旧缺陷**）。
  处置 = 靶心改为该负控**独有**的那半（「打了桩之后**词条必须与用户面分叉**」），
  并在门头如实登记「为什么换形态」。（`M189-VERIFY-OK` · PASS=22 FAIL=0）
- ⚠️ **第三次全量门**：`emitc_freeze` 判红 —— **类别 B 为空**（仅新增本门语料 1 件）
  ⇒ 按纪律 `--freeze` 重定基 **406 → 407 件**，`--check` 复跑逐字节一致。

### 六 CI 失败诊断注解的第三次复发（`::error::` 的 **4096 字符硬上限**）

CI（run 36243511065）的「工具自测」步在**本地复跑完全 ALL-OK**（129 条命令、逐条 rc=0 ——
用 R68 的办法从 `ci.yml` 抽出该步本地复现），而 CI 判红且**注解里看不到真凶**。查证得到三条**新事实**：

1. **`::error::` 的 message 硬上限 = 4096 字符**（实测注解 #1 长度**恰好 4096**，从中间截断）。
   此前注释里写的「注解 message 上限 64KB」是 GitHub **check-run** 的限制，
   与 **workflow command** 的限制**不是一回事** ⇒ 按文件名排序靠后的门**永远**在截断点之后。
2. **末行裁决会把「══ 通过 165 · 失败 3 ══」判成绿**（含「通过」）⇒ 真凶不进 ★ 名单。
3. **连 `M216-VERIFY-FAIL` 也不在 BAD_LAST 里**（只认 `FAILED` / `FAIL=[1-9]`）。

修法（`s1`）：**「最后写入的日志」放最前**（失败门必然是最后写日志的那个 —— 其后的门根本没跑）
+ 补中文 / `FAIL-N` 判据 + **总长截到 3900** + **有 ★/★★ 时不再附 ③**（③ 是刷屏源）。
自证：**100 份真实全绿日志 ⇒ 0 误报**；合成「通过 N · 失败 3」⇒ 旧判据漏、**新判据抓到**；
合成「失败 0」⇒ 不误报。（`SELFTEST-OK`）

⚠️ 另外纠正一个**我自己的判据盲点**：一度用「整日志扫 `❌`」当辅助信号 ⇒ 在 `v_m157.log` 上
**假报**（它含负控的 `❌`，而末行是 `M157-VERIFY-OK（失败 0 · SKIP 1）`）⇒ 100 份里 1 份误判
⇒ 回退为**只用末行裁决**（中文判据已覆盖真盲区）。

### 七 验收

- 入库件**重烘 14/14**（`PXSRC-8e34a48ad48fadaa` / `PXRT-51ea68b3f2e788da`）· `--check-all` **14/14** · `--check`（55 例）· `--check-vm`（**40423 行**镜像）全绿
- **全量门 `m116_gates.sh`：127 ✅ / 0 ❌**（首跑 2 红：m189 负控失效 + emitc_freeze 新增语料，两处均已按纪律收口后复跑全绿）
- **生态扫描（开口径收紧前先做）**：全仓 1236 个 `.px/.py` 扫「切片界含 `/` 或 float 字面量」⇒ **38 处命中全部人工定性为假阳**（字符串里的 URL / Python 的 `//` / 本门语料）⇒ **无生态代码使用 float 切片界**（M194 的规矩：「收紧类型口径前，先看生态用法」）
- 文档：速查表**事实 241 / 242** · `docs/spec.md` 类型转换口径 · 本 CHANGELOG
- 上游 `registry-px`（128 用例 × 双轨）随全量门回归

### 八 M216 补 4：诊断注解的**第四次**复发 —— 根因是「按**原始字符**裁剪」

CI（run `36248218474`）的「工具自测」步判红，而注解**读不出真因**（第 299 家族的第四次复发）。
本轮把根因钉死，并把诊断逻辑**从 Workflow 里抽出来**做成可自测的工具。

**三条新事实**（此前每一版都栽在同一处，但都没说清）：

1. **`::error::` 的 message 硬上限 = 4096 字符** —— 它是 **GitHub workflow command** 的限制，
   与 **check-run 注解**的 64KB 限制**不是一回事**（此前注释里把两者混为一谈）。
   实测：注解 #1 长度**恰好 4096**，**从中间截断**，而 `M216-VERIFY-OK/FAIL` 这个
   **最终结论行**正好落在被切掉的那几行里 ⇒ **白跑一轮**。
2. **`urllib.parse.quote()` 对中文膨胀 9 倍**（`中` → `%E4%B8%AD`）。
   而旧版按「**原始字符数** 3900」裁剪 ⇒ 编码后可达 **3.5 万** ⇒ **必然溢出**。
3. **末行裁决必须「失败优先」**：`══ 通过 165 · 失败 3 ══` 含「通过」⇒ 旧判据判**绿**，
   真凶进不了 ★ 名单（这是 M216s1 记下的真盲区）。

**修法（`packaging/ci_diagnose.py`，新增 · 逻辑从 `ci.yml` 内联搬出）**：

- **`fit()`：按编码后长度二分裁剪**（后缀也计入预算 —— 自测当场抓到首版「先裁主体再贴后缀 ⇒ 溢出」）；
- **一条注解只放一件事**：★★ 摘要（门名 + 末行 + 判定 + `❌` 行）/ 末 20 行（结论行在此）/
  **头 20 行**（失败常在开头 —— 旧版只有尾部 ⇒ 天然后天失明）/ ★ 可疑门末行一览；
- 判据不变：**最后写入的日志 = 失败门**（其后门根本没跑）⇒ **确定性**定位，不依赖关键词；
- m187 的现场复现搬到 `--m187-repro`（命令顺序 / cwd / env **逐字保留**，仅把末尾硬切换成统一裁剪）；
- **`--selftest`（10 断言）**：合成 5 场景（含「通过 N · 失败 3」真盲区、超长中文二分裁剪、
  `::error::` + 编码后 ≤ 4096）+ **真实语料判据**。

⚠️ **真实语料当场照出我自己的判据漏了三类成功末行**：`verify done` / `10P/0F/0S` / `pass=4 fail=0`
⇒ 100 份全绿日志里**假报 6 个「可疑门」**（刷屏）。补进 `OK_LAST` 后 **0 误报**。
⇒ **纪律重申：判据的「找不到」先怀疑判据；有真实语料就必须拿它当判据。**

**端到端验证**（本机，真实 100 份 + 合成失败形态）：
摘要注解 **782** 编码字符、尾块 **3799**、头块 **3013**、可疑一览 **141** ——
**每条 ≤ 4096**，且**末行 `M216-VERIFY-FAIL` 与 `❌` 行都在第一条注解里**（旧版正是这两个读不到）。

**未决（下一轮由新诊断直接给出答案）**：m216 门在 CI 上是否 `FAIL`。
本机**完整模拟 CI 环境**（隐藏 `/opt/aarch64-linux-musl-cross` ⇒ ③ 层 SKIP + 清 dev 件）
复跑 ⇒ **`M216-VERIFY-OK`（rc=0）**；而 CI 上日志的尾部 25 行与本机**逐字一致**
（直到被 4096 切断处）⇒ 需新诊断的「摘要 + 末行」才能裁定。

## M215 · **整数运算符的「逐值真值对拍」—— 三轨一致 ≠ 正确**（缺陷 305 / 306 / 307）（第 94 轮）

> 主题：本仓 300+ 道门里，凡是「三轨对拍」的，都只问**三轨是否彼此一致**。
> 可当**病灶长在运行时原语内部**时，三轨跑的是**同一份语义** ⇒ 输出**逐字节一致**
> ⇒ 门全绿，而答案全错。本轮把这类缺陷量化出来，并补上**「独立真值」**这条判据线。
> 一句话：**「三轨一致」是必要条件，不是充分条件。**

### 一 三个缺陷（同一主题的两个方向）

| # | 形状 | 修前实测（三轨） | 修后 |
|---|---|---|---|
| **305** | `//` 的商由 `(n - r) / d` 求：`n` 逼近 `INT64_MIN` 且 `r > 0` 时 **`n - r` 溢出**（构建**无** `-fwrapv` ⇒ 签名溢出 = UB），回绕后果是**商符号翻转** | `-9223372036854775807 // 3` = **`+3074457345618258602`**（真值 `-3074457345618258603`）；930 对里 **65 对**错。**三轨一致地错** | 截断商 + ±1 调整（= Rust `div_euclid` 写法），全程不做可溢出的减法 |
| **306** | `INT64_MIN // -1` / `% -1` ⇒ 硬件 `idiv` 陷阱 | **SIGFPE + core dump**（rc=136，三轨同崩） | 显式短路：`q = INT64_MIN`（回绕）· `r = 0` —— 唯一不可表示的商；与「一元负号在 `INT64_MIN` 回绕」同族，且回绕算术下不变量仍成立 |
| **307** | 解释轨 `i_bin_compare` 对 **int/int 也先转 `float`**，而 double 只有 53 位尾数 ⇒ 超 `2^53` 的整数被舍入到**同一个** double | `9223372036854775806 < 9223372036854775807` = **false**（真值 true）· `2^53 < 2^53+1` = false · `2^62-1 < 2^62` = false · `INT64_MIN < INT64_MIN+1` = false。**三轨分叉**（编译轨对） | int/int ⇒ 整数比较；任一侧 float ⇒ double（与 runtime `px_lt/le/gt/ge` 逐条对齐） |

### 二 ⚠️ 为什么旧门**按定义**看不见 305/306（本轮最值钱的认知）

病灶在运行时原语内部，而且 `px_idiv`（`runtime.c`）与 `pxc_vm` 的 `PXOP_IDIV`/`PXOP_MOD`
快路径**各写了一遍同样的公式** ⇒ **三轨跑的是同一份语义**。
⇒ 「三轨逐字节一致」这条判据**恒真**，与正确性无关。

**修法两条**：
1. **收敛到唯一实现** —— 新增 `runtime.h` 的 `px_idiv64` / `px_imod64`，`px_idiv`、`px_mod`
   与 VM 两个快路径**共用**（消灭"两处公式各写一遍"这个结构性隐患）；
2. **引入独立真值** —— 判据改成「三轨彼此一致 **且** 与 `truth/` 的独立真值一致」。

### 三 新门 `examples/m215_int_truth/`（真值对拍 + 不变量 + 陷阱回归）

- **语料**：`gen_corpus.py` 是**单一事实源**，生成 `corpus.px` + `domain.txt`（**929 对**：
  INT64_MIN 邻域 / ±2^53 邻域 / ±2^62 / INT64_MAX / 全符号组合）。门首层做**漂移检测**
  （重生成必须与入库件逐字节一致）。
- **真值三份**：`truth/ref.py`（**定义式**：`n % abs(d)` 得非负余数再精确除，Python bignum）
  · `truth/ref_mag.py`（**量级域 + 符号分支**，与被测实现不同构）
  · `truth/main.go`（Go，跨语言第三份意见）。前三者须**逐字节互等** —— 判据自身不可信比没门更糟。
  ⚠️ CI 无 Go 步骤（口径同 m136/m138）⇒ 第三份自动跳过；**本地全门用 `M215_REQUIRE_GO=1` 强制三份齐**。
- **判据**：三轨输出与真值**逐字节一致** + **不变量自断言**（`0 <= r < |d|`）+ **段 B 陷阱回归**
  （5 条定点字面值）+ **段 D 比较真值**（929 对 × 6 运算符）+ 反向判据「**两个不同整数绝不许判等**」。
- **负控 4 道**（各自独立判红，源逐字节还原）：A 撤回 305 ⇒ 152 行差异 · B 撤回 306 ⇒ **rc=136**
  （SIGFPE 复现），而**段 A 全段 931 行仍绿** ⇒ 与 A 解耦 · C 撤回 307（**重建 pxi**）⇒ 183 行差异 ·
  D 判据自伤（比对恒真）⇒ 证明红来自比对而非"碰巧崩了"。

### 四 两个设计取舍（都写进了门里）

- **叉积里排除 `(INT64_MIN, -1)` 这一对**：它是唯一"真商不可表示"的、也是修前 SIGFPE 的崩点，
  由**段 B 专管**。若留在叉积里，撤回 306 那道负控会让进程在**段 A 中途**崩掉，而 C 的 stdout 是
  **块缓冲**（静态件用不了 `stdbuf`）⇒ 段 A 输出整块丢失、两道负控无法解耦。
  ⇒ 牺牲 1/930 的叉积覆盖，换两道负控的独立性（覆盖没丢，在段 B）。
- **`INT64_MIN` 不能写成字面量**（词法 `E1004`：`9223372036854775808` 按正数解析溢出，
  与 Go 同款限制）⇒ 语料用 `-9223372036854775807 - 1` 求出来。

### 五 文档补正（事实 239 / 240）

- `docs/spec.md` §2.6 运算符表：算术行补 **int64 边界口径**（商给精确欧几里得值 · 唯一不可表示
  情形回绕 · **边界上 `q*b` 会溢出 ⇒ 不变量不能在 int64 内验算**）；比较行补
  「**int 对 int 按整数精确比较**，不退化为 `double`」。
- `docs/PUXIAN_CHEATSHEET.md` **事实 239**（边界口径 + 为什么旧门看不见）· **事实 240**
  （大整数比较必须精确 + `INT64_MIN` 字面量补正 + 移位计数走宿主掩码）。

### 六 未纳入本门（如实登记，下一轮主项）

- **浮点→int 转换族**：`int(1e30)` / `int(-1e30)` / `int(2^63)` / 浮点 `//` 越界 —— x86 实测
  **一律给 `INT64_MIN`**（`cvttsd2si` 的"不定值"），而 ARM 的 `fcvtzs` 是**饱和**语义
  ⇒ **疑似平台相关**，需交叉实测确认。
- **移位计数**：`1 << 64` = 1 · `1 << 65` = 2 · `1 << (0-1)` = `INT64_MIN` —— 走宿主 C 的**掩码**
  行为，文档此前未明示。

### 七 纪律（本轮新踩的坑）

- ⚠️ **门在跑的时候不要改门自己的脚本**：`bash` 是**增量读取**脚本的 ⇒ 中途编辑会让正在跑的
  那次解析出错（实测 `syntax error near unexpected token '('`，而 `bash -n` 事后是绿的）。
  （M213 记过「门在跑时别 `git add -A`」，本轮是同一条纪律的**另一个面**。）
- ⚠️ **负控锚点要用 `.px` 的 `#` 而不是 C 的 `/* */`**：首版把 `ival.px` 的代码块替换成
  C 风格注释 ⇒ devbuild 报 `E2001: 意外的 token: /` ⇒ 负控 C 直接失败。
- ⚠️ **`local a="$1" b="$W/$a"` 又踩了一次**（`set -u` 下 `a: unbound variable`）——
  M200/M213 记过的老坑，本轮在 `three_tracks()` 里重犯 ⇒ 拆两行。
- ⚠️ **门的提示文本一律用单引号**：双引号里的反引号会触发命令替换（M212 记过，本轮又踩）。
- ⚠️ **别做「看起来等价」的编辑**：一次想把行尾换行去掉，结果把两行**合并**了
  （`sed -n` 一眼看出）—— 编辑后要**核对 diff**。

## M214 · **GC 根面审计器：把 4 条「人工判定」下沉为三条窄条件豁免**（缺陷 301–304）（第 93 轮）

> 主题：M213 收尾时全仓候选还剩 **4 条**，每条的「假阳」理由都**只写在代码注释里**。
> 本轮把三条理由变成**可复算的判据**（候选 4 → **0**），并把第四条（`//`/`%` 的负债数语义）
> 从「文档声称同 Python」纠正为「**欧几里得对**」。
> 一句话：**豁免必须是「加一条可验证的条件」，不能是「凡不危及就放过」。**

### 一 三条豁免规则（缺陷 302 / 303 / 304）

| 规则 | 豁免条件（必须成立） | 消掉的候选 |
|---|---|---|
| **R-A** 提前返回不可达 | 触发点**就是** `return <表达式>` 里的那个分配，**且该表达式不含受害者** | `bi_http_unix` 错误支的 `headers` |
| **R-B** 全局根可达 | 受害者由**出参**交回，而该出参在被调方体内的写出**全部**是「全局表元素 **或** 非对象立即数」 | `px_http_dispatch` 的 `vhandler` · `px_route_try_dispatch` 的 `handler` |
| **R-C** `&victim` + 被调方先读参后分配 | 调用点传的是 **`&victim`**，且被调方**全部形参读都在它的首次触发点之前** | `xml_build_node` 的 `sv` |

⚠️ **本轮最贵的一课**：首版 R-A 写成「触发点之后本函数内再无任何读」（后置死值）——
听起来最自然，**结果一口吃掉 6 条既有锚点**（`hit5`/`hit6b`/`hit7`/`hit8`/`hit9`/`hit10`），
而那些 fixture **恰恰是真形状**（本审计器的本分是「创建后必须登记」的**规则合规**）。
⇒ 三条规则全部收紧，并各配一条**反向判据**（`hit11`/`hit12`/`hit13`），自证 **19 → 25** 锚点。

### 二 判据回放（precision replay）—— 最强的精确性证据

撤销 `px_route_try_dispatch` 的 `px_root_push_keep(params)` ⇒ 必须报出 **`params@379`**，
而**同一个调用**上的 **`handler@379` 仍被 R-B 豁免**。

> 同一条语句、同一个调用、两个出参：一个（`handler_out` = `g_routes[i].handler`）豁免，
> 一个（`params_out` = `*params_out = params;` 本地构造）报出。

### 三 A/B（每条规则**独立**有牙）

默认 **0** · `--no-postdead` ⇒ 1 · `--no-globalout` ⇒ 2 · `--no-argread` ⇒ 1 ·
**三个全关 ⇒ 4**（= M213 的人工判定面）。

### 四 上游 `registry-px` 再引入 + **PX-DEF-038 / 039 独立复核**（缺陷 301）

- 上游 `d4ae476 → 8b2e9ef` ⇒ **16 件新引入**（14 新包名 + 2 同包新版本目录：
  `fsnotify 0.2.0` / `xlsx 0.2.0`）；`registry/` **123 → 137**；上游用例 **112 → 128**；
  双轨回归 **233 通过 / 0 失败 / 23 跳过 / 期望值缺失 0**。
  · 23 条 SKIP 各有独立理由：3 条**设计性**（`mock`/`actor`/`semaphore` 解释轨需
    `spawn`/`chan`/`mutex`）；1 条**性能性**（`bcrypt_test` 解释轨 —— **实测量化：
    单独跑 1801s 仍未完成**，超 30 min 上限 rc=124；上游文件头自注「解释轨极慢 →
    验收以编译轨为准」，与 `qrcode_test` 同款处置）。
  · `EXPECTED.tsv` 是「**先登记、后实跑**」：16 条新登记里 **1 条**与实跑不符
    （`bcrypt_test` 解释轨）⇒ 按实测定性为性能性 SKIP（**没有**把实跑结果抄进去充数）。
- **PX-DEF-038**（`/` 对整数返回浮点）：**三轨逐字节一致** + 速查表**事实 51** 早已明写 ⇒
  上游说的「文档未在显眼处明示」**与事实不符** ⇒ 判「**不是语言缺陷**」。
- **PX-DEF-039**（`%` 负操作数 / `<<` 回绕 / `>>` 算术右移）：**三轨一致**（含 12 组不变量探针全 OK）
  + **事实 124** 已明写整数 `%` 是**欧几里得**余数；但上游把它称作「**Python 语义**」**不准确**
  （`17 % -3`：我方 `2` ⇄ Python `-1`）。
- ⭐ **缺陷 301**：`//` 与 `%` 是「**欧几里得对**」（= Rust `div_euclid`/`rem_euclid`），
  余数恒 `∈ [0,|b|)`、`a == (a//b)*b + (a%b)` **恒成立**；而 `docs/spec.md` 原写
  「`//` 整除…**同 Python**」⇒ **负除数下 12 组探针有 7 组不同**（`17//-3`：我方 `-5` ⇄ Python `-6`）。
  **处置**：语义自洽且三轨一致 ⇒ **不改行为、改文档**（`spec.md` 已改准 + 速查表新增**事实 237**）。
  未做（下一轮候选）：本仓**没有**逐值对拍 `//`/`%` 的门。
...
| 项 | 结果 |
|---|---|
| 门 `examples/m214_audit_exempt/` | **24 通过 / 0 失败**（`M214-VERIFY-OK`）：自证 25/25 · 候选 0 且豁免 1/2/1 · A/B 1/2/1/4 · 判据回放 · **负控 3 道各自独立判红**（NC-A 三规则全关 ⇒ 4 · NC-B `glob` 恒真 ⇒ 回放不再报 `params` · NC-C 放宽 R-A ⇒ 自证判红）· 覆盖边界 |
| 既有 GC 门 | `m206` / `m209` / `m212` / `m213` 同步（自证锚点 19 → 25） |
| 上游用例 | **128 用例 × 双轨** · 233 通过 / 0 失败 / 23 跳过 |
| 文档 | `docs/GC_ROOTS.md` **§12** · `docs/PX_DEF_TRIAGE.md` **§十四** · `docs/spec.md` 运算符表更正 · 速查表**事实 237 / 238** |

> ⚠️ **门的两个自伤教训**（本轮）：① 负控锚点必须落在**生效处** ——
> `POST_DEAD_ON` / `ARG_READ_FIRST_ON` 在 `main()` 里被 `not args.no_xxx` **重新赋值**，
> 只改模块默认值**只关掉一条**（首版实测候选 2 而非 4，同 M209 的 `LEGACY_GROW` 老坑）；
> ② 判据必须与**快照** `cmp`，**不能拿 `git diff` 比 HEAD** —— 工作树本来就是未提交状态。

> **未判据化**（登记为下一轮主项）：「保守兜底·过近似」触发点仍参与候选 ·
> 跨函数 F1 只覆盖直接调用 · R-C 依赖「文本序」判据 · `//`/`%` 缺逐值对拍门。

## M213 · **GC 根面审计器的「判据诚实度 + 覆盖面」**（缺陷 293–300）（第 92 轮）

> 主题：M209 把触发点定义成「可达 `gc_register`」后，「**候选 0**」被当成干净；M212 把集合改为
> **源码派生**、照出候选 3。本轮继续追同一件事的**两个面** —— 结论是：
> **「候选 0」与「候选 N」都可能是判据自身的产物。**
> 一句话：**判据既要看得见漏报，也要说得出自己为什么报。**
...
- **294 的实证**：撤掉 `bi_join` 的 `PX_KEEP(xs)` 后**修前报不出** —— 而 M207 缺陷 263 正在这条路上（当年只有**动态**压力筛抓得到）。
- **296 的实证**：把两处口子都堵上后，**tight 档**三个入口族**全部掉出** TRIGGER，而 **loose 档仍在**
  （宽松兜底**偶然**收住了它们：`px_gc_collect` 体内有 `sizeof(*b)` 形状）⇒ 既是缺陷 296 真实存在的证据，
  也是「**不能收紧间接调用判据**」的实证。

### 二 诚实度（假阳）· 缺陷 295

间接调用判据 M212 的**意图**写的是「被调方是形参 或 `(*p)(…)`」，**实现**却是
`re.search(r'\(\s*\*\s*[A-Za-z_]', body)` —— 匹配的是「**括号后跟星号**」这一**语法形状**，
与「是否调用」无关，于是把 `while (*pp)`（条件解引用）/ `sizeof(*b)`（类型操作数）/ `(*p).f`（成员访问）全当调用。
**实测：假种子 99**（seed 119 → 精确 20）· 闭包 **518 → 349**（虚增 169 函数）· 并**产出候选 #3**。

**处置：分层 + 分诊，默认留「过近似」档**（漏报比假阳危险）：

| 档 | 规则 | 用途 |
|---|---|---|
| **precise** | ① 被调名 ∈ 形参 ② `(*name)(…)` ③ 成员调用 `x.name(…)`/`x->name(…)` | 真的「调用」 |
| **conservative** | 任何 `(*x` 形状（含非调用语境） | **有意的过近似**（宁多收） |
| 默认 | `loose` = precise ∪ conservative | 安全侧 |

新增 `--indirect loose|tight`；候选 JSON 带 **`trigger_kind`** ⇒「过近似代价」**一眼可见**
（本轮 4 条候选里恰 1 条是 `conservative`）。

### 三 新判据 **F1「被调方登记了实参」**（缺陷 297 · 收口 M212 §10.6 缺口①）

调用点的活值作**实参**传进被调方，而被调方**入口**即 `PX_KEEP(形参)`（形参**按位置**配对），
且该登记**先于被调方第一个触发点** ⇒ 进入被调方后在任何分配之前即被登记 ⇒ 调用点**不构成风险**。
**必须**带「先于第一个触发点」这条：登记迟到（M182 缺陷 192 同族）**不得豁免**（自证锚点 ⑰ 是**反向判据**）。
⚠️ 实现坑：`body` **含签名**，而函数**自身**往往是触发点 ⇒ 从 body 开头搜「之前的触发点」会**每次自命中**
⇒ F1 整条**静默失效**（实测 F1 表只剩 3 条）⇒ 一律从**开括号之后**搜。

### 四 产品侧两处收口（缺陷 293 / 298）

| # | 位置 | 修前 | 修后 |
|---|---|---|---|
| **293** | `bi_session_del` | 全程**无登记**，靠 M183「根栈延迟收缩」的**副作用**兜住 ⇒ **隐性耦合** | 与 `bi_session_set` **对称化**（`px_root_push()` + 逐个 `PX_KEEP` + 出口 `px_root_pop()`） |
| **298** | `px_route_try_dispatch` | `params` 登记点在**限流块之后**，中间隔着 429 短路分支里的 `route_send`/`px_access_log`（**都会分配**） | 登记**前移**到 `route_match` 之后 + 429 分支补 `px_root_pop()` 配对（**每出口各一次**，静态门 ⑦ 守） |

> 同处 `handler` **无需登记**：`route_match` 里 `*handler_out = g_routes[i].handler` ⇒ 经**全局表**可达 ⇒ 本就是 GC 根（同族：`vhandler` 经 `g_vhosts[].handler`）。

### 五 候选账（**8 → 4** · 全判假阳）

- **消** `bi_session_del`（293 已修）· **消** `h3_out_send` / `h3_srv_{pipe,stateless}_cb`（F1 判据化）
- **增** `px_route_try_dispatch ← px_rate_limit_try( · handler`（298 把 `params` 移出受害面后只剩 `handler`）

### 六 判据基础设施两个编号（**本轮白跑三轮 CI 换来的**）

| # | 现象 | 处置 |
|---|---|---|
| **299** | CI 红时**唯一可读通道是注解**（job 日志 403），而 ③ 段的过滤只认 ASCII（`❌/FAIL/Traceback`）⇒ ① 中文判据（`失败 N`）整族看不见；② 大量**负控 PASS** 行挤满注解 ⇒ **真凶被截掉**（实测 ③ 块止于 `v_m189`） | 注解最前插 **★ 可疑门**：判据改 **末行裁决**（末行含成功标记且不含失败字眼 ⇒ 绿），**最近修改者排最前**；本机 97 份日志实测**全绿 0 误报** |
| **300** | 门里写死开发机绝对路径（`/data/code/puxian/...`）⇒ 本人全绿、**CI 红**（`FileNotFoundError`） | ① 4 处改经 argv 传 `$ROOT`；② `m198` NC-D 同族写死一并修；③ 新守卫 `selfhost/check_gate_paths.sh`（扫 150 门 + 脚本 + workflows，**代码行**含 `/data/code/` 或 `/home/runner/work/` 即判红，**注释行允许**留证；自带 3 条自证） |

⚠️ **本轮最值钱的一课**：我把 `git archive` 导到 `/tmp` 里跑了**整个质量门步骤（95 个门）全绿**，
**却没抓到缺陷 300** —— 因为写死的 `/data/code/puxian/...` 在**本机依然存在**。
⇒ **「换目录复现」抓不到「写死路径」这一类**；只有 **CI** 或**静态守卫**能抓。

### 七 验收

| 项 | 结果 |
|---|---|
| 新门 `examples/m213_gcroot_precision/` | **12 通过 / 0 失败**（9 层；负控 3 道**各自独立判红**：撤 F1 ⇒ 候选 7 / 堵显式 GC 口子 ⇒ tight 档三族掉出 / 撤 293 登记 ⇒ 候选 5） |
| 下游门同步 | m206（锚点 16→**19** · 候选 **4**）· m212（锚点 10→**19** · A/B 3→**4**）· m209（接受候选 4 · **旧规则对照 11→10** = 缺陷 293 修掉其中一条） |
| 入库件 | 重烘 **12/12**（烘前/烘后 62 文件快照一致）· `--check-all` **14/14** |
| 全量门 `m116_gates.sh` | **122 ✅ / 1 ❌** → 唯一失败项（m209 旧规则对照 11 vs 10）已按「产品侧修好了就要改对照计数」收口 ⇒ **失败 0 项** |
| 发射冻结门 | **405 件逐字节一致**（本轮**没有**新增 `.px` 语料 ⇒ 无需重定基） |
| 提交链 | `c8d22d4` → `a75aafa` → `bd5ccbd` → `9869cba` → `d305c24` → `ea30526` |
| CI / Tag Guard / Release | 见下 |

### 八 发布链插曲（三轮 CI 红 · 全部如实登记）

1. **s1**：m208/m209 门里硬编码 `self-test: 16 通过`，而锚点已 16→19 ⇒ 我「同步了 m206/m212/m213」却**漏了两个**
   ⇒ **纪律：改判据后必须 `grep -rn '<被改的断言串>'` 全仓扫**，不能靠回忆。
2. **s2**：`git add -A` 在**全量门正在跑**的时刻执行 ⇒ 抓到 m179 负控 **NC-C 的中间态**
   （`px_req_cmp` 被改成 no-op）⇒ ① 功能回退；② `runtime/runtime.c` 在**源码链**里 ⇒ 入库件 14 件指纹全失配
   （干净导出实测：`c8d22d4` 14/14 ✅ ⇄ `a75aafa` **0/14** ❌）。
   ⇒ **纪律：门在跑时禁止 `git add -A` / 提交；提交前逐文件看 diff。**
3. **s3/s4/s5**：③ 修 m209 对照计数；④ 注解「先点名可疑门」（缺陷 299）；⑤ 修写死路径（缺陷 300）+ 加守卫。
...
> EOF、服务端不响应（无 `[px-access]`）」—— 当时 `git worktree` A/B 已证**非本轮引入**，真因记「待定」。
> 本轮一次钉死：**`px_conn_read` 明文分支原为裸 `return recv(...)`，不重试 EINTR**，而并发 GC 的 STW
> 会给各线程发 `SIG_GC_STOP` **打断阻塞中的 recv** ⇒ 调用方 `if (n <= 0) break;` 判
> 「客户端关闭 / 空闲超时」⇒ **未进 handler 即关连接**（客户端只见空响应 / FIN）。
> 一句话：**「信号打断」被当成了「对端关闭」。**
...
| # | 事实 | 证据 |
|---|---|---|
| 1 | 双开档下客户端 `connect ✅` / `send ✅` ⇒ **recv 立即 = 0（EOF）** | `strace -tt`：`sendto(8,…,54)=54` → `recvfrom(8,"",65536)=0` |
| 2 | 服务端**从未调用 handler** | 无 `H-ENTER`、无 `[px-access]`、诊断计数全 0 |
| 3 | 服务端**确实 accept 了**，但在 `recv` 返回 **EINTR** 后**立即 close** | `recvfrom(7,…) = -1 EINTR` → `SIGRT_4 {SI_TKILL, si_pid=主线程}` → `close(7) = 0` |
| 4 | 读路径 = **`px_conn_read`**（**不是** `px_recv_wait`） | `strace -k`：`recv ← px_conn_worker+0x482 ← px_pool_worker` |
| 5 | 源码：明文分支 `return recv(...)` **不重试 EINTR**；调用方 `if (n <= 0) break;` | `runtime/runtime.c` |

**开关矩阵**（本轮最值钱的一张表）：正常 ✅ · **仅** `STRESS` ✅ · **仅** `INLINE` ✅ ·
`STRESS+INLINE` **双开 ⇒ 0/0/0** ❌ ⇒ 它**不是「压力档」的问题，而是「两个开关同开」的组合问题**
（与缺陷 192 同族）；而**同一个压力筛正是靠双开做判据** ⇒ 「双开自身有无伪影」直接决定筛子结论的可信度。

**两刀切开「谁的问题」**：压力档服务端 ← **外部 `curl`** ⇒ `/big` 200/3200B、`/index` 200/3226B
⇒ **服务端无罪**；跨进程**任意档组合**的 px 裸客户端 ⇒ A/B/C 三组全 `TOTAL=275 / 141`
⇒ **客户端桥无罪**。⇒ 病灶在「**同进程**」（`spawn serve()` 的服务端协程 + 主协程客户端）。

### 二 修复（四处）

| 编号 | 位置 | 修法 |
|---|---|---|
| **284** | `px_conn_read` **明文** | `EINTR` 无限重试（15s 空闲语义不变 —— 超时仍返回 `-1/EAGAIN`） |
| **286** | `px_conn_read` **TLS** | `MBEDTLS_ERR_SSL_WANT_READ/WANT_WRITE` 非错误 ⇒ 重试（同族病灶的 TLS 面） |
| **287** | `px_conn_write` 明文 | `EINTR` 不再当写失败（否则**响应截断**） |
| **285** | `gc_stop_handler` | 保存 / 恢复 `errno`（POSIX 要求）—— ⚠️ A/B **实测不是本缺陷根因**，如实登记为**独立小缺陷**，不冒充根因 |

### 三 验证（修前 → 修后）

| 验证 | 修前 | 修后 |
|---|---|---|
| 探针 C **双开** `V1 raw-ex` | **0** ❌ | **3335** ✅ |
| 探针 E **8 连裸 TCP**（双开） | R1–R8 **全 0** ❌ | **R1–R8 全 3335** ✅ |
| **原始复现点** `m173 vhost_gzip` 压力档 | **FAIL_RC**（fails=7） | **PASS** ✅ |

**新门 `examples/m211_conn_read_eintr/`：38 通过 / 0 失败**（7 层 · 含**负控 3 道各自独立判红**：
A 明文退回裸 `recv` ⇒ 压力档必红（R1–R8 全 0）· B 撤回 TLS 重试 ⇒ 静态必红 ·
C **判据自伤**（A 补丁在位 + `M211_THRESH=-1` ⇒ 同一故障**不再**判红 ⇒ 证明红确实来自阈值判据））。

### 四 教训三条（都记进纪律）

1. **插桩无输出，第一步先确认「插对地方了没有」**：首版在 `px_recv_wait` 插桩 ⇒ 跑完**零输出** ——
   因为 `px_serve` 走的是 `px_conn_worker` → `px_conn_read`（`px_recv_wait` 属 `http_serve` 那条）。
   差点据此走错方向。
2. **「看起来最像」的答案可能是错的**：`recv = EINTR` 紧接 `close` 的时序太像 errno 污染 ⇒
   先修 `gc_stop_handler` 的 errno（编号 285）—— A/B 证明**无效** ⇒ 如实登记为独立小缺陷，**不冒充根因**。
3. **`strace -k`（栈回溯）是本轮最值钱的工具**：一次就把「谁调的 recv」钉死。
   此前的 `strace`、`-tt`、gdb-attach（ptrace 失败）、`PX_SERVE_DIAG`、`max_conn=1`、`GS_HARD`
   六路排查都没它直接。

### 五 本条同时关闭 / 否证

- **M207 的缺陷 265**（`examples/m207_gcstress/` 的 FAIL 项之一）⇒ 由 FAIL 转 **PASS**。
- **一条假设被否证**（省下下一轮走错路）：曾怀疑「挂起协程的帧槽不是精确根」，并写了最小复现器
  （worker 持帧槽对象 → 挂起 → 主协程猛分配 → 唤醒后读）⇒ **8 跑全绿**、逐字节一致、无 LIVECHK/UAFDET
  标记 ⇒ **该方向不成立**，与缺陷 267 家族无关。
- **两条我自己的操作事故（如实登记）**：① `pkill -f 'probe_coro_frame'` **打到了自己的 shell**
  （M183 记忆里写明要避免的，本轮重犯 ⇒ 改按 PID kill）；② 探针自身两个错（忘了 worker 先 `send`
  ⇒ 双方都 recv ⇒ **死锁**；**单通道双向 recv** ⇒ 竞态 ⇒ 实测 `go=ready` + rc=124）⇒ 修正为
  **每个方向各一个通道**。**「探针异常」先怀疑探针**（第 4 次了）。

## M210 · **保留字作「形参名」漏拦截收口（缺陷 283）+ 上游 registry-px 再引入（98 → 108 库）**（第 89 轮）

> 主题两条：① **上游生态再引入**（10 新库 + 2 就地更新 + 12 用例；双轨 **205 通过 / 0 失败 / 19 跳过**）；
> ② 复核第三方登记的 **PX-DEF-036 / 037** 时**照出一处真缺口** —— `expect_name` 是「形参名 / 成员名」
> 两条路径**共用**的取名字入口，它那条「放行整张 `is_name_kind` 列表」的后门让**形参面**也宽松
> ⇒ `def f(x, fn)` **静默通过**，而 `var fn` / `for fn in` 早已 `E2001`。
> 一句话：**同一个名字在四个声明位置，此前是「三个拦、一个放行」。**

### 一 缺陷 283 —— 保留字作形参名漏拦截（本轮核心）

| 位置 | 修前 | 修后 |
|---|---|---|
| `var fn = 1` | `E2001 期望变量名，实际得到 fn` ✅ | 同 |
| `for fn in xs:` | `E2001 期望循环变量，实际得到 fn` ✅ | 同 |
| `def f(fn)` / `def f(x, fn)` / `def f(x, y, fn)` | **静默通过**（且 `fn` 一被**使用**就报「期望 '('」—— 错误指不到**声明处**）❌ | `E2001 期望参数名，实际得到 fn`（**三轨同码同文**）✅ |
| `s.fn`（成员名） | 放行 ✅ | **保持放行** ✅ |

**根因**（`selfhost/parser.px` 的 `expect_name`）：

```px
def expect_name(what):
    if pk() == "标识符": …
    if is_name_kind(pk()):      # ← 后门：整张关键字表都放行
        …
```

**为什么后门必须存在**：**成员名面**的关键字是合法的 —— `ch.send(x)` / `ch.recv()` 的成员名
**就是关键字**（chan 的内建方法名），`cases_bc/bc11`、`bc12`、`registry/workerpool` 都依赖它。
⇒ 正解不是「一律拒」，而是**把两面的宽松度分开**：

```px
def expect_name(what, allow_kw):            # 形参面传 false；成员名面传 true
    …
    if is_name_kind(pk()) and (allow_kw or pk() == "self"):   # `self` 例外：方法接收者形参
```

调用点 4 处：形参面 1 处 `false` · 成员名面 3 处 `true`（含 match 模式里的成员名）。

### 二 影响面普查（改动的最大风险面）

**方法**：用**含改动的新编译器**编全仓 `.px`（`selfhost/ stdlib/ tools/ registry/`），
抓「期望参数名/成员名/变量名/循环变量」命中的文件 —— 那是被误伤的候选。

| 轮次 | HIT | 明细 |
|---|---|---|
| 第一版（一律拒真关键字） | **5** | `capability.px:369` 与 `bc8.px:20` 的 `def area(self)`（**`self` 被拒**）· `workerpool.px:30`/`bc11:10`/`bc12:8` 的成员名 `send`/`recv` |
| 定稿（两面分离 + `self` 例外） | **0** | 264 件全部通过 |

⇒ 第一版的 5 条 HIT **正是「成员名面必须宽松 + `self` 必须例外」的证据**（否则会打破自己的
`capability.px` 与 3 个 golden 用例）—— **普查是把「我认为」换成「实测」的那一步**。

### 三 上游再引入（`70e8123` → `d4ae476`，20 提交）

| 项 | 结果 |
|---|---|
| 新库 | **10**：`bson` `dns` `hant` `imap` `mongodb` `richmd` `sched` `statx` `targz` `tempfile` |
| 就地更新 | **2**：`cnnum`（人民币金额大写 `cn_amount_str`）· `xlsx`（读取 `xsx_read`/`xsx_read_sheet`） |
| `registry/` | **113 → 123** 项（上游 108 + 本仓自建 13） |
| 上游用例 | **100 → 112**（逐字节照搬 · `MANIFEST.sha256` **+12 / -0**） |
| 双轨回归 | **205 通过 · 0 失败 · 19 跳过 · 期望值缺失 0**（与 `EXPECTED.tsv` 完全一致） |
| 引入门 `m187` | **PASS=19 / FAIL=0**（108 包全量 `pxpkg add`→`install`→`import` + 5 组负控） |

**新 fixture（本仓自建，不引依赖）**：`upstream-tests/fixtures/imap_mock.px`（IMAP4rev1 · 1143）——
上游 `imap_test.px` 打 `127.0.0.1:1143` 却**不带服务端**（同 ftp/pop3/oauth2 先例）。
⚠️ 两个坑写在门头：**tag 必须回显请求里的**（客户端 tag 位数可变）·
**literal 长度按字节算**（`len("…普贤")` 是 rune 数 —— PX-DEF-015）。
⇒ 双轨 PASS（`--only imap_test` 首次即通过）。

**两条 SKIP（如实登记，各有理由）**：`dns`（需外网 UDP 解析器 `223.5.5.5:53`，GitHub runner 无 UDP 出网；
自建 mock + lo 别名列为下一轮）· `mongodb`（需真实 `mongod 7.0`；与 mysql/pg/mqtt/redis 同族）。

### 四 第三方登记的复核（不盲信）

| 登记 | 上游判定 | 我方独立复核 |
|---|---|---|
| **PX-DEF-036**（`bytes_slice` 半开区间） | 速查表**已记载** ⇒ 非缺陷 | **实测确认**（`bytes_slice(b,1,3)` = `bc` · 三轨一致）；但速查表的记载只说了「clamp / `start>end` ⇒ 空」，**「第 3 参是结束索引而非长度」未明示** ⇒ 判为**文档补正**（已补） |
| **PX-DEF-037**（`fn` 保留字） | `def f(x, fn)` 报 `E2001 期望变量名…` | ⚠️ **复现与实测不符** —— 本机（含改动前的入库件）实测 `def f(x, fn)` **rc=0 静默通过**；真正的问题是**形参面漏拦截** ⇒ 当作**真缺口**收口（缺陷 283） |

### 五 验收（本机）

```
门 examples/m210_param_kw/   M210-VERIFY-OK · **通过 26 · 失败 0**
  [1] 静态 5 项（含**反向断言**：不得出现「参数名→true」「成员名→false」）
  [2] 拒绝侧 3 探针 × 三轨：rc≠0 + E2001 + 词条**逐字相同**
  [3] 放行侧 4 探针 × 三轨（普通形参 / 软名字 type / 匿名函数 / impl 的 self）
  [4] 真实文件不被误伤 3 件 × 三轨 rc=0（capability · workerpool · bc8）
  [4b] bc11/bc12（字节码轨专用）⇒ 三轨均不得出「期望成员名」E2001
  [5] **全仓普查 264 件 HIT=0**
  [6] 负控 3 道**各自独立判红**（A 退回共用放行路径 · B 形参面传 true · C 去 self 例外）
      —— 每道**强制重建** devbuild 并断言 `/tmp/pxcdev -nt parser.px`（防「跑的还是旧件」假绿）
  [7] 源逐字节还原
入库件      重烘 **12/12**（烘前/烘后 62 文件快照一致）· `--check-all` **14/14**
自举 golden C 轨 `compiler.c` 18491 行（**自举成立**）· BC 轨 `compiler.bc.dump` **40423 行**（VM 重放逐字节一致）
归族核对    C 轨：归一化后 6 个非等块**全部**属本族（未归类 **0**）
            BC 轨：9 个块 = **1 个函数（arity 1→2 · nslots 27→30）+ 4 个调用点**
codegen     19 件 golden **未变**（本改动不动 C 轨发射）
发射冻结门  **404 件逐字节一致**（改 parser 不改变语料编译产物 · 类别 B 为空）
上游回归    112 用例 × 双轨：205 通过 · 0 失败 · 19 跳过 · 缺失 0
```

### 六 覆盖边界（如实）

- 普查只覆盖**可独立编译**的 `.px`（`selfhost/ stdlib/ tools/ registry/`）；`examples/` 里大量
  **故意报错**的负控探针未纳入（否则噪声淹没判据）。
- `def f(self)` 在**普通函数**里也被放行（parser 不区分「方法 vs 函数」）—— 现状如此，
  与「`self` 是方法接收者」的约定一致，未单独收紧。
- 成员名面的关键字放行**继续**（`ch.send`/`s.type`），并由 [4]/[4b] 的真实文件锁住。
## M209 · **GC 根面审计器的「触发点」更正 + 三条排除规则（缺陷 279–282）**（第 88 轮）

> 主题：M208（缺陷 270）给审计器加了一条「隐式分配」规则 —— 把 `px_dict_set` / `px_list_push`
> 也算成**触发点**，据此报出 **19 处**「缺陷 271 族（下一轮收口）」。本轮把它**逐条定证并收口**：
> **11 条**由「触发点只认 `gc_register`」消除（前提错位），**8 条**由三条**排除规则**消除
> ⇒ 全仓候选 **0**；并把 M206 那 8 条**人工判定**下沉为**判据**。
> 一句话：**审计器不再是「靠人读代码维护的清单」，而是能自证的判据。**

### 一 缺陷 279 —— 「隐式分配」前提被证伪（本轮核心）
...
| `px_list_push` 同 | `:5020` |
| `g_tmp_root` 是 `__thread`，回收出口**不清**它（M208 缺陷 269） | `:1457` · `:2696` |

⇒ 这两个调用**不会触发 GC**。它们内部确有 M110-S2 安全点（`gc_unblock_stop` →
`gc_pause_if_requested`），但那只在**另一个线程的 GC 已在跑**时暂停本线程，而那一刻被保护的面 =
本线程 `ti→tmp_root`（`:1973` 快照自 `g_tmp_root`；`:2635`/`:2742` 逐线程标记）= **本线程最近登记的对象**；
而审计器报告的每个候选，受害者**恰是「创建后的第一个触发点」**上的对象 ⇒ 触发点是 push/dict_set 时，
受害者**就是**最近登记对象 ⇒ **被护住** ⇒ 安全。

### 二 缺陷 280 / 281 / 282 —— 三条排除规则（人工判定 → 判据）

| 规则 | 判据（可证） | 消灭的候选 |
|---|---|---|
| **覆盖排除** | 触发点语句**正在写同名直接局部**、且右值**未读**旧值 ⇒ 旧值此刻已死 | `runtime_h3_qpack.c:350 name` · `runtime_sqlite.c:209 v` |
| **作用域排除** | 受害者的**声明块**在触发点之前已闭合 ⇒ 读不到 ⇒ 不是受害者 | `keep`（`px_gen_materialize`/`px_gen_next`）×2 · 宏体 `rv`（`bi_http_get_stream`）×3 |
| **deref 交棒** | `*outv = px_str_len(…)` 把值交给调用方（与 `return` 同口径）⇒ 不计本帧 C 局部活值 | `runtime_h3_qpack.c:262 outv` |

⚠️ **两个判据细节（都实测踩过，已写进 `docs/GC_ROOTS.md` §9.2）**：
- 作用域必须按**块身份**而非**括号深度** —— 宏体（`#define STREAM_FLUSH()` 的 `do{…}`）在**源码里**
  位于较浅深度，而其**调用点**在更深的分支里 ⇒「深度变小」永不成立 ⇒ 宏体局部 `rv` 被误当活值（自证 `miss7` 判红）；
- 作用域要绑**声明**而非**赋值** —— `v = px_str_len(…)` 写在 `switch` 的 case 块里而 `LXValue v;` 在外层
  ⇒ 绑赋值会**误剪**（把仍在作用域的值判死）⇒ **漏报**，违反审计器的本分。

### 三 判据不放水（本轮最重要的一层）

改判最容易把工具改瞎 ⇒ 补两层反向证据：

1. **自证 10 → 16 锚点**：新增 `hit6b`（push 之后**插一次真构造器**仍必中）· `hit7`（受害者**仍在作用域**必中）·
   `hit8`（右值**读了**旧值时，覆盖排除**不得**生效）；`miss5` = M208 那条 `hit6` 的**改判**
   （缺陷 279 的预期翻转：同形状现在必须**不中**）。
2. **判据回放**（新门层 ⑤）：把 **2 处已修**站点的修复**原样退回** ⇒ 审计器必须**命中**：
   撤 `px_session_read` 的 `PX_KEEP(v)` ⇒ 在 `px_call` 处报出 `v`（缺陷 254 形状）；
   撤 `bi_map` 的 `px_root_push_keep(r)` ⇒ 报出**容器 `r`** —— 顺带证明 **M208 的受害者归因是错的**：
   真实风险在**容器**（下一轮 `px_call` 的分配），而不是 push 的实参 `res`。

### 四 新门 `examples/m209_gcroot_rules/`（**23 通过 / 0 失败** · `M209-VERIFY-OK`）

8 层：① 自证 **16/16** ② 新规则全仓**候选 0**（两次一致）+ 规模锚点（函数 ≥1500 · 登记 ≥200）
③ 旧规则对照 `--grow` = **11** 且**全部**带「隐式分配·旧规则」标记 ④ 四条规则的**源码在位**断言
⑤ **判据回放** ⑥ 动态两档（`probe_rules.px` 覆盖 `bi_map`/`bi_filter`/`px_gen_*`/`px_cell`/`bi_os_popen`
五族；`PX_GC_STRESS=1 PX_GC_INLINE=1 PX_GC_LIVECHK=1 PX_GC_UAFDET=1` 与正常档**逐字节一致**）
⑦ 负控 3 道（撤 `bi_map` 的 keep ⇒ 压力档 **rc=134 + LIVECHK**；旧规则设默认 ⇒ 候选 **11**；
撤作用域排除 ⇒ 候选 **5**；各自独立 + 源逐字节还原）⑧ 覆盖边界登记。已挂 `m116_gates.sh` 与 `ci.yml`。

⚠️ **探针必须让回调真的分配**：首版 `inc(x) = x + 1`（`px_int` 是立即数、不 `gc_register`）⇒ 循环里
**根本没有 GC**，负控 A 照样绿 ⇒ **假绿**（实测踩过，已写进 probe 头注释）。

### 五 收敛 · 验收 · 记账更正

- `examples/m206_gcroot/BASELINE.tsv` **8 条 → 0 条**：M206 的人工判定转入 §② **档案区**，
  逐条注明「现由哪条规则排除」⇒ 本表自此是**安全网**（任何**新增候选**都必须进表）。
- `examples/m208_vm_c_local_roots/verify.sh`：自证断言 10 → 16；`--grow` 措辞改为**旧规则对照**；
  覆盖边界段注明「18 处已由 M209 全部收口」。
- `docs/GC_ROOTS.md`：§2 判据精确化（「分配」= 触发点）· §3 例外 2 注明已下沉为判据 ·
  §8.5 标记收口 · **新增 §9**（本题的完整定证 + 两条判据细节 + 不放水证据 + 覆盖边界）。
- `selfhost/gcroot_audit.py`：`--grow` 降为**旧规则对照**（默认档 = 新规则集）；新增
  `LEGACY_GROW` / `DECL_RX`（声明块）/ 三条排除规则。
- **记账更正**：`m208` 门里把 M208s1 的工作误标为「M209 更正」⇒ 改回「**M208s1 更正**」。
- **仍未做（下一轮候选）**：逐桥**人工**审计（本系列覆盖的是「能扫出来的形状 + 动态筛得到的」）·
  `PX_GC_STRESS` 铺到全部 `examples/*/verify.sh`（按语料分批）· 动态覆盖 `bi_session_*` / H3 两族
  （本门只给静态判据，理由已在层 ⑧ 登记）。



> 全量门（干净树）结论：**rc=1 · 3 个失败门**（`m170` / `m183` / `m206`）—— 它们**全部**是
> M208 的「锚点同步」后坐力 + 一处**判据假绿**。这一批与上一批（272/273）同源：
> **M208 改的是 GC 基础设施，而「谁依赖它写判据」没被系统清点。**

### 一 三个编号

| 编号 | 形状 | 影响 |
|---|---|---|
| **274** | M208 门里的 **S13 判据是假绿** —— 判据用**整行正则** `px_root_push\(\);[ \t]*\n[ \t]*PX_KEEP\(`，⇒ **行尾带注释的形态（`px_root_push();   // …`）整族漏判**；且扫描面**只覆盖 3 个文件**（runtime.c/vm.c/coro.c）⇒ 报「相邻两段式 = 0」时**全仓其实还有 30 处**（分布在 11 个文件） | S13 声称的「全仓改写 55 处」**并未覆盖全仓** —— 判据看不见的地方等于没改 |
| **275** | 负控的两种「静默通过」：① **锚点未命中 = 静默 no-op**（m170 负控 A 用 `s.replace(old,new,1)`，`PX_KEEP(out);` 已被 S13 改写 ⇒ 命中 0 次 ⇒ **什么都没改** ⇒ 全绿 ⇒ 报「未判红」）；② **锚点不唯一、只删一处**（m183 负控 C 的同一段代码在 `px_root_push_keep` 内联 + `px_root_keep` 本体各有一份 ⇒ `replace(…,1)` 只删其一 ⇒ 泄漏不复现 ⇒ **负控失牙**） | 负控**看起来在跑**，实际没验证任何东西 |
| **276** | **「构建失败」被当作判红**：m183 负控 D 的补丁把 `px_root_push_keep(env);` 换成 `LXValue env = px_dict();` ⇒ 与上一行**重声明** ⇒ **编译失败** ⇒ 判定函数返回非零 ⇒ 被记成「判红 ✓」。**判红原因不是本缺陷** | 假绿（`negcase` 只看 `$fn` 的返回码） |

### 二 处置

1. **S13 真收口**：新增 `selfhost/s13_check.py` —— **先剥注释**（`//` 与 `/* */`）、把 `\` 续行**拼成逻辑行**、覆盖**全部** `runtime/*.c`；两条规则（① 同一逻辑行内 `px_root_push()` 与 `PX_KEEP(` 共现；② 相邻两段式）；**自带 6 条自证**（含「行尾注释必须判红」与「注释里提到两者不得误报」两个负例）。
   - 据此改写 **30 处（11 个文件）** + **1 处宏续行**（`STREAM_FLUSH` 里 `px_root_push(); … PX_KEEP(arg);` 同一逻辑行）⇒ 全仓**违例 0**。
   - 改写脚本本身带**逐行可判定**护栏：处数 != 预期即拒；`difflib` 对齐后要求每个差异块都是「**2 行 → 1 行**」且三行形态正确（其它块一律拒绝 → 不会把非本族差异一起「重写」掉）。
2. **m170 负控 A**：锚点随收口改 `px_root_push_keep(out);` **并断言唯一性**（命中 != 1 即响亮失败）；形态换「**撤登记 + pad**」—— 缺陷 269（回收出口不清 `g_tmp_root`）会**吸收**单纯的撤登记（`out` 就是最近登记的对象）⇒ 必须插一次分配把 `g_tmp_root` 移开。修后判红并打出**原始症状** `FAIL-TYPE n=15 i=0 t=unknown`（与清歌报障同码）。
3. **m183 负控 C**：正则匹配**全部 2 处**并断言「恰好 2 处」（`px_root_push_keep` 内联 + `px_root_keep` 本体）。修后根栈峰值 **121**（> 64 阈值）⇒ 判红 ✓。
4. **m183 负控 D**：补丁改为**正确删除**该行（不从声明拼锚点），三处锚点各加**唯一性断言**；并给 `negcase` 加**守卫**：`BUILD_FAIL=1` 时**不算判红**（`build()` 置位）。
5. **m208 门 ② 层**：判据换成 `selfhost/s13_check.py`，并加**判据自证**（现场注入「行尾带注释」形态 ⇒ 必须判红；不红即门红）。

### 三 验收

- 四个受影响门逐个复跑：**m170 VERIFY-OK**（负控 A/B/C/D 全判红 · 源逐字节还原）· **m183 VERIFY-OK**（峰值 121 · 负控 D 走 UAFDET 真症状）· **m206 门 9/0** · **m208 门 PASS=15 FAIL=0**
- 入库件 `--rebake-all`：**12/12**（烘前/烘后 62 文件快照一致）· `--check-all` **14/14**（`PXSRC-ef21bcbaf1d2acc3` / `PXRT-4dba81c9ebf5685d`）· `--check` **55 例逐字节** · `--check-vm` **40412 行镜像逐字节** · `diffcheck --errors` 全一致
- 全量门 `m116_gates.sh`（干净树）见提交后复跑记录

### 三之补（M208s1 补二 · 缺陷 277 / 278）

- **277** M208 新增的两件语料**未重定基**发射冻结门 ⇒ CI 质量门判红（该门在 m116 第 705 行，
  M208 收尾没跑全量门）。已 `--freeze` 重定基 **403 件**，类别 B（src 未变、out 变）为空。
- **278** m154 负控 C 的锚点（`bi_join_item`）自该函数重构为 `px_tostr_n` 后**早已失配**
  ⇒ 补丁从未生效；而 `negctl_patch` 只打 ⚠️ 就跳过 ⇒ **整道负控长期没跑过而门仍报 OK**
  （= 缺陷 275 同族：负控的「静默跳过」）。修法：锚点改到当前形态 + **锚点失配改记失败**。
  复跑负控三红齐全（C：RC=0 · FAIL=9）。

### 四 纪律（本轮第 3 次撞同一族，这次把「机制」换掉）

- **判据的「找不到」先怀疑判据**：正则要容忍**行尾注释**；扫描面要写**全仓**并配**规模下限**；
  改完判据要**故意造一个必须判红的输入**去验它还有牙（本轮 m208 门已内置该自证）。
- **负控的补丁必须「可判定」**：锚点命中数**必须断言**（`!= 1` 即失败）；**宁可用词更长的真锚点，也不要凭 diff 拼锚点**。
- **负控判红要问「为什么红」**：**构建失败 / 崩溃 / 超时**都**不算**判红 —— 那只是「没跑起来」。
