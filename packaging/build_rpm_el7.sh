...
#       且 CentOS 7 已 EOL（源迁移 vault）——故不能在 rockylinux:9 容器交叉
#       冒充，必须在真实 el7 容器内构建 + 用 yum 3.4 真实验签/安装。
...
# 流程：修 EOL 源(vault) → yum 装工具 → 导入签名子密钥 → build_rpm.sh(DIST=7)
#       → yum file:// 仓库双验签 + 真实安装 pxc --version
# ============================================================
set -euo pipefail

echo "== [el7] 修正 CentOS 7 EOL 源 → vault.centos.org（备源 CERN vault）=="
cat > /etc/yum.repos.d/CentOS-Base.repo <<'EOF'
[base]
name=CentOS-$releasever - Base
baseurl=http://vault.centos.org/centos/$releasever/os/$basearch/
        http://linuxsoft.cern.ch/centos-vault/centos/$releasever/os/$basearch/
gpgcheck=1
...
baseurl=http://vault.centos.org/centos/$releasever/updates/$basearch/
        http://linuxsoft.cern.ch/centos-vault/centos/$releasever/updates/$basearch/
gpgcheck=1
gpgkey=file:///etc/pki/rpm-gpg/RPM-GPG-KEY-CentOS-7
...
baseurl=http://vault.centos.org/centos/$releasever/extras/$basearch/
        http://linuxsoft.cern.ch/centos-vault/centos/$releasever/extras/$basearch/
gpgcheck=1
gpgkey=file:///etc/pki/rpm-gpg/RPM-GPG-KEY-CentOS-7
EOF

echo "== [el7] yum 安装构建工具链 =="
yum -y makecache >/dev/null 2>&1 || true
yum -y install rpm-build rpm-sign createrepo gnupg2 git tar >/dev/null
echo "   ✅ 工具: $(rpmbuild --version | head -1), $(rpmsign --version | head -1), createrepo $(createrepo --version 2>&1 | head -1)"
...
echo "== [el7] 导入签名私钥（gpg2.0 无 loopback → 直接 --import，签名走 passphrase-file）=="
mkdir -p ~/.gnupg && chmod 700 ~/.gnupg
echo "$GPG_PRIVATE_KEY" | gpg2 --batch --yes --import
gpg2 --list-secret-keys --keyid-format LONG | grep -E '^(sec|ssb)' || { echo "❌ 私钥导入失败"; exit 1; }
...
/usr/bin/pxc --version
rpm -q puxian
echo "✅ el7 全链路通过：vault 源 + gpg2.0 签名 + createrepo(gzip) + yum 双验签 + 安装运行"
