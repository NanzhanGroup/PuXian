...
## 十一、M203（第 82 轮）：缺陷 243（「此类型不支持X / 不可迭代」族的三轨同码同文）+ **顺带发现缺陷 245**

### 11.1 缺陷 243-a/b/c：同一族**四个站点只收口了一个**

| 站点 | 解释轨（修前） | 编译两轨（修前） |
|---|---|---|
| 切片 | `此类型不支持切片: <t>` ✅ **M191 已修** | `此类型不支持切片: <t>` |
| **索引** | `此类型不支持索引`（**缺类型**） | `此类型不支持索引: <t>` |
| **迭代** | `此类型不可迭代`（缺类型） | **`len 不支持类型 <t>`** ← 借了用户**从未写过**的 `len` |
| **索引赋值** | `索引赋值目标不支持`（**整个词条都不同**） | `此类型不支持索引赋值: <t>` |

⇒ 同一份源码，三轨两/三个答案；按「码 + 文案」做匹配或断言的一方必然漏。

**统一口径（三轨一条真相）**：
`R1002 此类型不支持索引: <t>` · `R1002 此类型不可迭代: <t>` · `R1002 此类型不支持索引赋值: <t>`

**为什么「迭代」自成一码**：迭代与索引在 M164 已明确分开（`px_iter_at` ⇄ `px_index`）——
用索引的词条描述迭代，正是 M164 要根治的那类混淆。

**实现（一份实现、三轨共用）**：runtime 新增 `int px_iter_prepare(LXValue)`：
可迭代（`str`/`bytes`/`list`/`dict`/`tuple`/生成器）⇒ 返回长度；否则响亮报词条。
- **C 轨**：直接发射 `px_iter_prepare(...)`（**两条**发射点：for 循环 + 推导式）；
- **VM 轨**：经**内部全局** `__iter_len` 调用**同一个**函数。
  修前它借 `px_len` ⇒ `for x in 1` 报 `len 不支持类型 int`：**消息里的名字用户根本没写过**
  （诊断纪律：错误消息里出现的名字，必须是用户在源码里写过的东西）。

`__` 前缀 = **内部名**：`tools/gen_native_table.sh` 增加过滤 ⇒ 不进公开 native 索引
（公开名册仍是 387）；`gen_builtin_list.sh` 本就排除 `__`。

### 11.2 缺陷 245（**由 M203 的门现场照出来 · 高严重度 · C 轨静默错值**）

```
let m = [1, 2, 3]; m[0] += 10     解释轨 [11,2,3]   VM [11,2,3]   **C 轨 [10,2,3]**
let q = [5, 6];    q[1] -= 2      解释轨 [5,4]      VM [5,4]      **C 轨 [5,2]**
let d = {"k":5};   d["k"] += 1    解释轨 {k:6}      VM {k:6}      **C 轨 {k:1}**
struct P: x,y ; p.x += 10         解释轨 11         VM 11         **C 轨 10**
l[0] = l[0] + 100                 三轨都对          ← 只坏在「复合」这一条路
```

**根因**：C 轨的赋值发射（`selfhost/cg_stmt.px`）在 `Index` / `Field` 目标上**无条件**发射
`px_index_set(o, i, rhs)` / `px_field_set(o, "f", rhs)` —— `op`（`+=` / `-=` / …）**被静默丢弃**，
等价于「直接赋值」。（`Var` 目标那一支走 `cg_assign_op_local`，是对的。）

**危害**：静默错值（不崩、不报错）。计数器 / 累加器 / 词频统计这类代码在 C 轨**全错**且难察。

⇒ 归 **M204** 专项（要动发射路径 + 重定基 + 守 M165 的求值顺序）；本门刻意**不放**这类用例。

## 十二、M204（第 83 轮）：「复合赋值」在 `Index` / `Field` 目标上的三轨同一真相（缺陷 245 + 同轮照出的 246）

### 12.1 缺陷 245（**高严重度 · C 轨静默错值**）
...
`getL()[idx()] += rhs()` 的副作用日志实测 `[o, k, v]`；对比显式展开
`getL3()[0] = getL3()[0] + 1` ⇒ `[o, o]`（两次）—— 门把这个差异**写成判据**。

### 12.2 缺陷 246（同轮实测照出）

复合赋值到**不存在的成员**时，解释轨把"缺失"当 `null` 硬算：
...
统一为 **`R1008`** —— **与成员的「读」一致**（`d["k"]` / `p.z` 本来就报 R1008；
...
门 examples/m204_assign_op/   M204-ASSIGN-OP-VERIFY-OK · **通过 22 · 失败 0**
  [1] 静态 6 项 · [2] 正例 2 组 × 三轨逐字节一致（12 种复合运算符 × list/dict/struct ·
      嵌套 · 显式展开对照 · Var 零回归 · **求值次数与顺序**）· [3] 拒绝侧 3 例 × 三轨
      （缺键 / 缺字段 / 越界：rc≠0 + 同码 + **整条消息体逐字相同**）·
  [4] 负控 5 道：A Index 退回旧形态 · B Field 退回旧形态 · C **顺序违规** · D **重复求值** ·
      E 解释轨 246 退回 —— 前四道**必须重编 C 轨编译器**后用 `PX_PXC_BIN` 注入
      （改动只在 C 轨可见，否则跑的还是入库件 = 假绿）
入库件      重烘 12/12 · `--check-all` 14/14 · `--check`（55 例）· `--check-vm`（**40412 行**镜像）
自举双轨    `bootstrap_prove`（C 轨）· `bootstrap_prove_bc`（BC 轨 · 40412 行 dump）
对拍        diffcheck 六路全绿 · 发射冻结门 **397 件逐字节一致**（VM 轨未被本改动触碰）
...
| 产物 | 归族判据 | 实测 |
|---|---|---|
| `golden/compiler.c` | 编号归一化后**纯加法** | 新增 **37** 行 · 修改 **0** · 删除 **0** ✓ |
| `golden/compiler.bc.dump` | **直接法**：退回 HEAD 的三个源 → 造「旧 C 轨编译器」→ `bc compiler.px` 与基准逐字节对拍 | **40282/40282 行逐字符一致** ✓ |
| `golden/s0*.c / v0*.c`（codegen） | —— | **未变**（无语料用复合索引/字段赋值，19 例全一致） |
| `emitc_freeze.sha` | —— | **未变**（397 件逐字节一致：本改动只在 **C 轨**发射器，冻结门走 VM 轨） |

⚠️ **`compiler.c` 的归族必须「先编号归一化」**：本轮**新增了函数** `cg_seq_new()` 与两个分支
⇒ 编译器自身产物里的 `_vN` / `_tN` / `_ilN` / `px_err_N_*` **整体位移**，
逐行 diff 会被编号噪声淹没（首版实测「不可归类 3651 行」，归一化后 **0**）。M165/M167 的
`norm_num.py` 同款做法。

---

## 十三、M210（第 89 轮）：上游 98 → 108 库 + **PX-DEF-036 / 037 的独立复核**（并由此抓出缺陷 283）

> 来源：`banshanhanfu/registry-px` @ `d4ae476`（20 提交 · 新增 10 库 + 2 就地更新 + 12 用例）。
> 本轮的规矩照旧：**第三方说什么都要自己复现** —— 036 / 037 两条在本机逐条实测，结果如下。

### 13.1 PX-DEF-036（`bytes_slice` 半开区间）—— **实测确认；但上游的「已记载」只对了一半**

| 项 | 内容 |
|---|---|
| 上游登记 | `bytes_slice(b, start, end)` 第三参是**结束索引**而非长度；从 Python `bytes[start:start+len]` 心智写会得到空/截断 |
| 上游判定 | 「官方速查表**已记载**」（`bytes_slice`/`b[a:b]` 负边界越界一律 clamp、`start>end` ⇒ 空）⇒ 非缺陷 |
| **我方实测** | `bytes_slice(b,1,3)` = `bc`（**半开区间**）· `bytes_slice(b,1,1)` = 空 · `bytes_slice(b,4,2)` = 空 · `bytes_slice(b,0,99)` = 全长 —— **三轨一致** ✅ |
| **我方判定** | **行为正确、且是有意语义**（与 Go/列表切片 `a[i:j]` 一致）。但速查表原文只写了「clamp / `start>end` ⇒ 空」，**没有明示「第 3 参是结束索引，不是长度」** —— 上游写库时踩的就是这一句没写。⇒ 判为**我方文档补正**（已在 `PUXIAN_CHEATSHEET.md` 事实 220 的 bytes 条目落实），**不是语言缺陷**。 |

### 13.2 PX-DEF-037（`fn` 保留字）—— ⚠️ **上游的复现与实测不符**；真缺口在**形参面**

| 项 | 内容 |
|---|---|
| 上游登记 | 「`fn` 是匿名函数关键字，不能用作标识符；`def f(x, fn)` 报 **`E2001 期望变量名，实际得到 fn`**」⇒ 判定「速查表已记载，非缺陷」 |
| **我方实测（改动前的入库件）** | `def f(x, fn): return x` ⇒ **rc=0 静默通过**（编译轨与解释轨都是）；`def f(fn)`、`def f(x, y, fn)` 同 —— **上游描述的那个报错没有出现** |
| 真实现状（四个位置一句话） | `var fn = 1` ⇒ 拦 ✅ · `for fn in xs` ⇒ 拦 ✅ · **形参名 ⇒ 放行 ❌** · 成员名（`s.fn`）⇒ 放行 ✅（有意） |
| 附带症状 | `fn` 一旦被**使用**（`def f(x, fn): return fn`）就报 `E2001 期望 '('` —— 报的是**使用处**，而根因在**声明处** |
| **我方判定** | **真缺口**（不是「已记载」能盖住的）⇒ 当作 **缺陷 283** 收口：`expect_name(what, allow_kw)` 把**形参面**收紧、**成员名面**保持宽松（`ch.send`/`ch.recv` 的成员名就是关键字）、`self` 例外（方法接收者）。三轨同码同文；全仓 264 件普查 HIT=0。 |
| 给上游的话 | 两条登记**都不是「官方已记载」能结案的**：036 是「文档没写全」、037 是「复现不准确 + 真缺口在形参面」。建议他们按 **m210** 重新复查这两条并把 `def f(x, fn)` 的复现换成 `def f(fn)` / 三参形态（或直接用 `var fn` 那条已记载的）。 |

### 13.3 本轮生态面（其余 10 库 / 12 用例）

- `registry/` **113 → 123** 项（上游 108 + 本仓自建 13）；上游用例 **100 → 112**（逐字节照搬，`MANIFEST.sha256` **+12 / -0**）。
- 双轨回归 **205 通过 · 0 失败 · 19 跳过 · 期望值缺失 0**；引入门 `m187` **PASS=19 / FAIL=0**（108 包全量 `add`→`install`→`import` + 5 组负控）。
- 新 fixture：`upstream-tests/fixtures/imap_mock.px`（IMAP4rev1 · 1143，本仓自建，不引依赖）。
- 两条 SKIP（如实）：`dns`（需外网 UDP 解析器 `223.5.5.5:53`，GitHub runner 无 UDP 出网；自建 mock + lo 别名列下一轮）· `mongodb`（需真实 `mongod 7.0`，与 mysql/pg/mqtt/redis 同族）。
- 上游本轮自判「已修复」的 **PX-DEF-021 / 031–035**：与我在 **M201** 的收口结论一致（035 的「解释器件全能力」由 M201 的 `native_symbols()` + `--full` 闭环）。

---

## 十四、M214（第 93 轮）：上游 **108 → 124 库** + **PX-DEF-038 / 039 的独立复核**（并由此照出缺陷 301）

### 14.1 上游再引入（用户长期指令）

- 上游 `registry-px`：`d4ae476 → 8b2e9ef`（**18 提交**）⇒ 我方 **16 件新引入**：
  「14 个新包名 + 2 个同包新版本目录（`fsnotify 0.2.0` / `xlsx 0.2.0`）」。
  ⚠️ **数「引入件数」要按 `<name>/<version>` 算**，只按包名会漏掉那 2 件。
- `registry/` **123 → 137** 项（上游 124 + 本仓自建 13）；上游用例 **112 → 128**
  （逐字节照搬 · `MANIFEST.sha256` **+16 / -0** · 128/128 `sha256sum -c` OK）。
- 双轨回归：**233 通过 · 0 失败 · 23 跳过 · 期望值缺失 0**。
  - 新增 3 条设计性 SKIP：`mock` / `actor` / `semaphore` 的**解释轨**（需 `spawn`/`chan`/`mutex`）；
  - 新增 1 条**性能性** SKIP：`bcrypt_test` 解释轨 —— **实测量化**：单独跑 **1801s 仍未完成**
    （超 30 min 上限被 `timeout` 杀掉，rc=124）；且**上游文件头自注**「解释轨极慢 →
    验收以编译轨为准（PX-DEF-024 同族）」⇒ 与 `qrcode_test` 同款处置。
  - `EXPECTED.tsv` 的**期望值是先登记、后实跑**（「登记的数据」不是「跑一遍抄结果」）：
    本轮 16 条新登记里 **1 条**与实跑不符（`bcrypt_test` 的解释轨）⇒ 按实测定性为性能性 SKIP。

### 14.2 PX-DEF-038（`/` 对整数返回浮点）—— **三轨一致 + 文档已明示** ⇒ 判「文档补正（已记载）」

| 探针 | 三轨实测 |
|---|---|
| `2023/400` | `5.0575`（float） · `2023//400` = `5` |
| `type(7/2)` | `float` · `type(7//2)` = `int` |
| `-7/2` / `-7//2` | `-3.5` / `-4` |

- **三轨逐字节一致**（interp = VM = C）。
- 速查表**事实 51** 早已明写「`/` 是浮点除法，`//` 才是整除（缺陷 42）」⇒
  上游说的「文档未在显眼处明示」**与事实不符**（事实 51 就是显眼处）。
- ⇒ 判定：**不是语言缺陷**（三轨一致 + 已记载）。

### 14.3 PX-DEF-039（`%` 负操作数、`<<` 回绕、`>>` 算术右移）—— **三轨一致 + 已记载**，但上游的「Python 语义」说法**不准确**
...
| **`17%-3`** | **`2`** | Python **`-1`** ✗ |
| **`-17%-3`** | **`1`** | Python **`-2`** ✗ |
| `1<<63` | `-9223372036854775808` | **int64 回绕**（= C/Go；Python 会得正数） |
| `-8>>1` / `-1>>1` | `-4` / `-1` | 算术右移（= C/Go） |

- **三轨逐字节一致**（包括不变量探针：12 组 `a == (a//b)*b + (a%b)` **三轨全 OK**）。
- 速查表**事实 124** 附近已明写：整数 `%` 是**欧几里得**余数（`-7 % 3` = `2`，对标 Rust `rem_euclid`）。
- ⇒ 上游把它称作「**Python 语义**」**不准确**：Python 的 `%` 是 floor-division 语义
  （余数取**除数**符号），**负除数下两者不同**（上表两行 ✗）。

### 14.4 ⭐ 由此照出的**缺陷 301**：`//` 与 `%` 的负债数语义，文档声称「同 Python」而实为**欧几里得对**

- **实证**：12 组探针里 **7 组**与 Python 不同（全部是**负除数**）：
  `17//-3`：我方 `-5` ⇄ Python `-6`；`7//-2`：我方 `-3` ⇄ Python `-4`；
  `1//-3`：我方 `0` ⇄ Python `-1`；`17%-3`：我方 `2` ⇄ Python `-1` …
- **我方语义**：**欧几里得对**（Rust `div_euclid`/`rem_euclid`）—— 余数恒 `∈ [0,|b|)`，
  且 `a == (a//b)*b + (a%b)` **恒成立**（三轨 12/12）。
- **文档不符**：`docs/spec.md` 运算符表原写「`//` 整除、`**` 幂，**同 Python**」。
- **处置**：语义**自洽且三轨一致** ⇒ **不改行为**；**改文档**（`spec.md` 已改准）+
  速查表新增**事实 237**。
- **风险面**：从 Python 移植「带负数的 `//` / `%`」（取模索引 / 环形缓冲 / 日期偏移 /
  `hash % n` 的负数分支）会拿到**不同的商与余数**，且**不报错** ⇒ 事实 237 明写「必须显式验算」。
- **未做（下一轮候选）**：本仓**没有**逐值对拍 `//` / `%` 的门 ⇒ 语义目前只靠
  「三轨实现一致 + 文档」维持；建议补一道**逐值对拍门**（含 `±` 除数 × `±` 被除数 +
  不变量断言 + 与 Python/Rust 参考值对拍）。

### 14.5 上游登记的**「走法」建议**复核

上游对 038/039 给的规避建议（整数除法一律写 `//`；位运算用掩码组合）**与事实一致**，
可直接采纳 —— 唯一要纠正的是它把 `%` 的语义称作「Python 语义」。
