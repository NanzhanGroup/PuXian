...
| — | PX-DEF-029 | `bytes_get` 越界返回 `null`（不报错） | 文档缺口（已在事实 215 写明守卫写法） |
| — | PX-DEF-030 | `bytes_to_int` 默认**无符号**读取 | 文档缺口（已在事实 215 写明 `signed=true`） |
| — | PX-DEF-022/023/028 | 浅拷贝 / Err 嵌套展示 / 无 C 风格 `for(;;)` | **设计如此**（速查表写明） |
| 内部 | `passhash` 库 | M184 严格解析后 `int(parts[1])` 语义不再正确 | 待上游修（`registry/THIRD_PARTY.md` 留痕，未引入） |
