...
| PX-DEF-003 | `Result` 无 `unwrap_err()` | 三轨一致报 `R1007 类型 result 没有方法 'unwrap_err'`；`str(Err("boom"))` = `"Err(boom)"`（可剥壳取消息） | 设计缺口（有绕行） | 📄 **文档已补**（速查表写明剥壳写法） |
...
| PX-DEF-015 | `len()`/`s[i]` 按 **rune**、`s[a:b]` 切片按 **字节**（混用） | 三轨**一致**：`len("你好")`=2、`s[1]`=`"好"`、`s[0:1]` 取**首字节**（3 字节切片长度=3） | **真缺陷**（文档与实现不符 + 双模型混用） | 🔜 **M185 候选**（统一口径） |
...
| **新 1**（我方新登记，内部编号 **203**） | **`bytes` 切片三轨分叉** | 解释轨 `R1002 此类型不支持切片`；**VM/C 轨支持** `b[1:3]` | **真缺陷（三轨分叉）** | 🔜 **M185 候选** |
