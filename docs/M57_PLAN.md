...
>
> 范围：仅 PuXian 语言 / runtime / 工具链侧（外部私有生产应用归其维护方，不在本里程碑）。
...
| S6 ✅ | D6 文档收尾 | commit `89e5123`：spec §8.17（边缘设备层 fd 原语/数据通道/mmap 活映射 API 文档）+ ROADMAP（主线表 M57 行、能力基线「边缘设备层」、远期 M57 段移入完成）+ CHANGELOG（仓库治理 · Mahesvara 迁出条目 + M57 里程碑闭环行，S1–S5 commit 引用 S5 修正为 main 实际 62d9275）+ README/README.en（特性表「🔌 边缘设备」、里程碑表 M57、示例列表 m57_s1–s5）同步定稿；M57 里程碑 **S1–S6 全部闭环 ✅** |
