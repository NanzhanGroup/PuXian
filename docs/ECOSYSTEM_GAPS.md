...
> 更新：M167（2026-09-21）—— **严格化族已收口**（字典键 `R1002` · 迭代期间改容器 `R1003` · 解包形状 · `sorted` 值比较+稳定 · 求值顺序词法左→右，规范见 `docs/spec.md` §17，迁移见 `docs/DICT_STRICT_MIGRATION.md`）；
> 其中 **`d["k"] = v` 赋值已可用**（M163 起三轨一致，键必须字符串；缺键读仍为 `R1008`）。本文档 §1/§3 的历史结论保留供追溯，**现状以 spec + `PUXIAN_CHEATSHEET.md` 为准**。
> 配套：写码速查见 `docs/PUXIAN_CHEATSHEET.md`（易错事实表 + native 名册）· 文档总览见 `docs/README.md`。
...
- [ ] **空 dict**：`json_parse("{}")`（`{}` 字面量 = null，见 F1）；dict 写 `.set(k,v)` 或 `d["k"] = v`（键必须字符串，M163 起三轨一致）、查 `.has(k)`、读 `d["k"]`（缺键 ⇒ `R1008`）、键序 `d.keys()`、键值对 `d.items()`；**迭代期间不要修改被迭代容器**（`R1003`）。
...
- **M119 修订（2026-09-15，Issue 72 现场）**：M70 的①里"二元运算符后不换行"**在括号内被取消** —— `(` `[` `{` 内的表达式可在运算符处断行（行尾或行首皆可），多行 dict 不再要求尾逗号；括号外维持原规则。行首一元运算符（`-` `~` `not`）不参与续行（与新语句歧义）。

### G3 · `let` 不可变（赋值报 E3002）

- **现象**：`let` 默认不可变，对其赋值 = 编译错误 E3002（M41.2 实现）；要可变用 `let mut` 或 `var`。
- **两编译器轨一致（M114 尾核验，qg-issue 46 结案）**：**默认 VM 轨（`px build`）与 C 轨逃生舱
  （`px build --c`）都报 E3002**（E3003 空安全 / E3004 FFI 形参个数同）—— 三条检查走
  「轨无关共享语义层」，诊断正文与 golden 逐字节一致。**例外**：`px run`（pxi 解释器）**宽松**、
  不报静态语义错（MINI_SUBSET §十三.0 已记录，属解释器 Mini 子集的既有边界，非缺陷）。
