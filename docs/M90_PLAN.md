...
| 段 | 内容 | 状态 |
|---|---|---|
| **S1** | F1：bc_emit 默认参数支持（切换前置硬缺口）+ 附带修复 + 系统文件 VM 化验证 | ✅ 完成（见下方 S1 完成记录） |
| **S2** | F3 快查：HTTP 20 并发双轨 RPS 骤降 + ~9s 尖刺（runtime 服务层，与轨无关） | ✅ 快查完成（docs/M90_S2_F3.md）—— 判定真 bug：15s keep-alive 空闲超时误杀活跃连接（M88-B-S2 事件化遗留，与轨无关）；**修复落地列独立批次 F3-fix**（超时 close 前二次确认兜底 + 事件循环漏报深查） |
| **S3** | F1 落地验证收尾：stdlib 全量 VM 编译冒烟（capability 全量运行已由 S1 兑现） | ✅ stdlib 13/13 VM 编译通过（无缺口） |
| **S4/收口** | 默认轨切换立项评估：golden 大迁移范围 + AB 回退开关设计 | ✅ 评估完成（docs/M90_S4_switch_eval.md）：技术前置全齐，建议 F3-fix 先行后按 S0-S4 立项 M91 |
