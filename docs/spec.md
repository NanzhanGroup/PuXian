...
| 类别 | 运算符 |
|------|--------|
| 算术 | `+ - * / // % **`（`/` 浮点除 · `//` 整除 · `**` 幂）。⚠️ **`//` 与整数 `%` 是「欧几里得对」**（= Rust `div_euclid`/`rem_euclid`）：余数恒 `∈ [0, |b|)`、且 `a == (a//b)*b + (a%b)` **恒成立**；**负除数下与 Python 不同**（`17//-3` = `-5` ⇄ Python `-6`；`17%-3` = `2` ⇄ Python `-1`）。浮点 `%` 是 `fmod`（余数取**被除数**符号），与整数 `%` **不是同一个算子** |
| 比较 | `== != < <= > >=` |
| 逻辑 | `and or not`（不用 `&& ||`） |
| 位运算 | `& | ^ << >> ~` |
| 赋值 | `= += -= *= /= //= %= **= &= |= ^= <<= >>=` |
