...
> 本章把这类角落逐条收成**一条真相**（M159–M167）。总原则：**响亮优于静默** —— 宁可报错，不给静默错值。
...
- **顶层循环变量的作用域与捕获（缺陷 181）**：顶层 `for p in [1,2,3]:` 之后在顶层引用 `p` —— 解释/VM 轨
  保留最后值（Python 语义），**C 轨编译失败**；顶层闭包捕获顶层循环变量时三轨行为不同（函数内三者一致）。
  根因：顶层（main 体）缺少「函数顶 hoist + 装箱预扫描」。修法已定，待排期。
  规避：需要跨迭代保留的循环变量，请在**函数内**使用。
