...
| 12 | 类型标注强制检查（空安全/不可变） | 不可变已实现（M41.2：E3002，let 默认不可变、let mut/var 可变、对 let 赋值编译报错）；空安全已实现（M41.3：E3003，显式非空类型标注赋 null 编译报错，渐进类型省略标注不查）；泛型 E3007 待 M41.4 | 自举后由 PuXian 编译器自己实现（M41.1-M41.3 完成，M41.4 待办） |
