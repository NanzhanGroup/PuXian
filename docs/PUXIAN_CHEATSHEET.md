...
185. **VM 轨闭包打通了（第 46 轮 · M160 · 缺陷 159 收口）**：闭包（匿名 `fn`）与**函数体内
     ...
     ⚠️ 仍不支持的形态见事实 188（生成器 transform/filter）。
...
188. **缺口 163（第 46 轮登记 · 未修）：生成器 `transform/filter` 引用外层局部**：
     `[x * k for x in xs]` 里的 `k` 在 VM 轨仍按**全局名**解析（该 lambda 是 `bc_emit_genexp`
     合成的，不在 AST 闭包分析面内）；C 轨 M129 是按**值**建 cell（读得到、写出不回流）。
     ⇒ 生成器体内**别依赖对外层局部的引用语义**；需要就显式传参或先绑成局部函数。
