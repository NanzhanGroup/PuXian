...
192. **迭代**位置语义**与用户索引分离（第 50 轮 · M164 收口缺陷 170/174/175）**：
     ...
     · **迭代期间修改被迭代容器**（**仍未收口，登记缺陷 176**）：`for k in d` 里改 `d`、
       `for x in l` 里 `l.append(...)` —— 解释轨 dict 走**键快照**、list 走**活长度**（Python 口径）；
       VM 轨长度在**进入循环前取一次**（list 也成了快照）、C 轨 dict 用**活长度** ⇒ 三轨仍分叉。
       结论：**不要在迭代中修改被迭代的容器**；要改就先 `let ks = d.keys()` 再按 `ks` 迭代。
