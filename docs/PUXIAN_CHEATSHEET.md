...
195. **解包（destructuring）统一（第 53 轮 · M167 收口缺陷 180/182）**：
     ...
     · **已知边界（缺陷 181，未修 · 下一轮）**：**顶层**（模块 main）循环变量在**循环之后**引用时，
       C 轨**编译失败**（`_vN undeclared`）、VM 轨闭包捕获顶层循环变量报索引错，而解释轨正常 ——
       根因是顶层没有「函数顶 hoist」这一层（函数内三轨一致）。**函数内**写 `for … ` 后用循环变量是 OK 的；
       顶层要保留最后值请先自行 `var last = null` 在循环里赋值（或放进函数里）。
