...
> 版本基线：M69（2026-09-05）· 三轨（解释 `px run` / VM `px build` 默认 / C `px build --c`）行为一致（M68 起 native 零 extern def 可达；M159–M167 起语义收口，见下）。
> **读前必看（2026-09-21）**：文档总览 [`README.md`](README.md)（docs 索引）· **规范条文** [`spec.md`](spec.md) **§17 语义一致性收口**（求值顺序 / 迭代长度快照 / 解包 / 字典键 / `sorted` / 闭包与生成器捕获）+ **§11.2 错误码真值表**（R1001–R1011）· **迁移指引** [`DICT_STRICT_MIGRATION.md`](DICT_STRICT_MIGRATION.md)。
...
> 闭包缺口**已收口**（第 46 轮 M160）：VM 轨 upvalue + 函数体内 `def`（缺陷 159）、C 轨嵌套 `def`
> 捕获（缺陷 160）—— 三轨一致（按引用捕获）；实现纪律与坑见事实 185–189。
...
175. **VM 轨（用户面默认轨）闭包已打通（第 40 轮登记 · 第 46 轮 M160 收口 · 缺陷 159/160）**：
     此前 `def` 写在**函数体内** ⇒ 编译期 `bc_emit_stmt 未实现: [FuncDef, …]`；匿名 `fn ():`
     引用**外层局部** ⇒ 运行期 `无法相加: null + int`；C 轨函数体内 `def` ⇒ `未定义变量: hits`。
     **M160 起三轨一致（按引用捕获）**：C 轨走 `cg_gen_closure`（匿名 `fn` 与具名 `def` 同一条路径），
     VM 轨走 cell 四指令（`CELLGET`/`CELLSET`/`CELLNEW`/`MKCLO`）。**写法、边界与坑见事实 185–189。**
     （M158 门第 ⑤ 层已由"缺口 SKIP"升格为硬判据；新门 `examples/m160_closure/` 守正判据 + 3 道负控。）
...
185. **VM 轨闭包打通了（第 46 轮 · M160 · 缺陷 159 收口）**：闭包（匿名 `fn`）与**函数体内
     `def`** 现在都按**引用**捕获外层帧的绑定，三轨语义一致。机制：cell 四指令
     （`CELLGET`/`CELLSET`/`CELLNEW`/`MKCLO`）+ 帧入口按 `PxVMFunc.upvals` 序把 env 里的
     cell 绑回槽（槽 = 本帧参数数 + i）。写移植代码时可以放心用了：
     · `x = x + 1` 里 `x` 是外层局部 ⇒ 闭包**改的是外层那份**（不是新建局部）；
     · 捕获对**参数**同样有效（含**默认参数**：入口填充走 `CELLSET`，不冲掉 cell）；
     · **跨两层/三层**也成立（自由变量逐层上提：中间帧因自己的闭包出现了该自由变量而一并
       捕获）—— 门 `examples/m160_closure/closure_edge.px` 实测。
     ⚠️ **生成器**（`(x + k for x in xs)`）的捕获规则**不同** —— 那是**按值快照**，见事实 188。
186. **闭包捕获要"先装箱、再发射"（第 46 轮 · M160 的实现纪律）**：外层帧必须在**发射任何
     读/写之前**就把"被帧内闭包捕获的绑定"标记为 cell 并在入口 `CELLNEW` 原地装箱；否则
     先生成的读取点按普通值生成、而槽后来变成 cell ⇒ 语义全错（VM 轨实测
     `此类型不支持索引赋值: null`）。两条配套纪律：① 已是 cell 的名字（本帧 upvalue）
     **不得再装箱**（再包一层会读成 cell 本身）；② 默认参数填充、`VarDecl`、复合赋值
     （`+=`）都必须走 `CELLSET`/`CELLGET`。
187. **`bc_emit_push_lambda` 的下标要在"压入时"取（第 46 轮 · M160 · 缺陷 162）**：函数体发射
     期间还会压入**内层**闭包/函数体内 `def` ⇒ 用 `len(funcs)-1` 当返回下标会把外层闭包指到
     内层函数（实测三层嵌套捕获 ⇒「无法调用非函数: int」）。同源坑：lambda 自己的局部**没有
     hoist 预扫描** ⇒「闭包内的局部又被内层闭包捕获」漏装箱（实测「此类型不支持索引: function」）。
     两条都只在**嵌套闭包**里暴露 —— 单层闭包看不出来。
188. **生成器捕获 = 按值快照（第 47 轮 · M161 收口缺口 163/164）**：`(x + k for x in xs)` 里的
     `k`（外层局部 / 形参）在**创建生成器那一刻**取值快照 ⇒ **之后改 `k` 不影响已建的生成器**
     （实测 `g = (x + k for x in [1])` 后 `k = 7` ⇒ `gen_next(g)` 仍得 2，不是 8）。
     三轨同一条真相：解释轨 GenExp **创建时求值** · C 轨 `px_cell(px_cell_get(cv))` ·
     VM 轨 `bc_emit_caps_snapshot`（`CELLGET` 取值 + `CELLNEW` 造新 cell）。
     ⚠️ 与**闭包**相反（闭包按**引用**捕获 ⇒ 改外层局部闭包看得见）；同帧里两者并存时
     各按各的规则走（门 `examples/m161_genexp_capture/` 断言 5 `co_live` 专测这条）。
     捕获源是**引用类型**（list/dict）时，「快照」= 共享同一对象（与解释轨一致，不是深拷贝）。
189. **发射冻结门的信号要护住（第 46 轮 · M160 的做法）**：给 `PxVMFunc` 加捕获字段时，把
     **无捕获（`nup=0`）时的 emit-c 文本保持原样**（`.upvals=NULL`）⇒ 267 件样例的
     `--emit-c` 里**只有真有闭包的那 2 件**变化（其余逐字节不变）。否则一次无关紧要的字段改名
     会让全量样例同时"变红"，把门的主职（**源码未变而产物变** = 发射回归）淹没在噪声里。
190. **`sorted` 的语义：按值比较 + 稳定排序（第 48 轮 · M162 · 缺陷 166/167 收口）**：
     `sorted(x)` 返回**新 list**（不改原值），排序依据是 `compare_values` 的**值序**，
     **不是** `str()` 渲染串序 —— 这正是本里程碑前解释轨与编译轨**分叉**处：
     · 修前解释轨 `sorted([10, 9, 2])` = `[10, 2, 9]`（渲染串 `"10" < "2"`）、编译轨
        `[2, 9, 10]`（值序，对）；复合元素同上（`[[1,"a"],[1,"a2"]]`：解释轨 `[[1,a2],[1,a]]` /
        编译轨 `[[1,a],[1,a2]]` = 第三方 `registry-px` 登记的 **PX-DEF-011**）。
     · 值序规则（= `compare_values`）：int/float 按数值（**int-int 不绕 double**，护住
       `9007199254740993` 一族）；string/bytes 按**字节序**（UTF-8 下即码点序）；bool `false < true`；
       null 相等；enum 比 `variant`；list/tuple **逐元素、再比长度**；dict 比键数、再按键序比值；
       result 比 `ok` 标志再比载荷；**其余类型（含 struct）按类型名字符串**比较
       （`"bool" < "dict" < "float" < "int" < "list" < "string" < "tuple"`）⇒
       `sorted([true, "a", 1])` = `[true, 1, a]`、`sorted([[2], 1])` = `[1, [2]]`。
     · **稳定性**：比较器判相等的元素**保持原序**（`1.0` 与 `1` 数值相等、渲染不同 ⇒ 可观测）：
       `sorted([1.0, 1, 0.5])` = `[0.5, 1.0, 1]`、`sorted([1, 1.0, 0.5])` = `[0.5, 1, 1.0]`。
     · 同族：`min` / `max` 走 `compare`（**只支持数值/字符串**，其余报 `R1002`），语义未变。
     · 门：`examples/m162_sorted_order/`（三轨逐字节一致 + 26/12/4 断言 + 3 道负控）。
191. **字典**构造**位置的非字符串键 = R1002（第 49 轮 · M163 收口缺陷 168/169/171）**：`{k: v}` 字面量与
     `{k: v for …}` 推导式里，键**必须是字符串** —— 非字符串 ⇒ `R1002 字典键必须是字符串，实际是 <t>`
     （`<t>` = `int`/`float`/`bool`/`list`/`dict`/`null`/…，三轨同码同文）。
     · 修前只有**解释轨**报错：**VM 轨与 C 轨静默丢弃该项** —— `{1: 11, 2: 22}` 得 `len=0`、
       `{"a":1,"b":2,3:4}` 得 `len=2`（只丢非字符串那一项）⇒ 与 R1008「静默 null」同族的**静默数据丢失**；
       推导式上 VM 轨还只是借 `set` 方法报 `方法 set 参数 1 需要 string`（同码不同文）。
     · 迁移代价：源码里若有 `{1: "a"}` / `{x: x for x in [1]}` 这类写法，修前**静默得空（或残缺）dict**、
       修后**当场报错** = 设计意图（把沉默的错误提前暴露）。键请显式转字符串：`{str(x): x for x in xs}`。
     · 同族口径（**未变**，M120 文档 `docs/DICT_STRICT_MIGRATION.md` §1）：索引读写 `d[k]`（k 非字符串）
       ⇒ `R1002 字典索引键必须是字符串`；方法族 `d.set(k,v)`/`d.get(k,·)`/`d.has(k)` ⇒
       `R1002 方法 <m> 参数 1 需要 string`。
     · ⚠️ **别用 `d[0]` 取「第 0 个键」** —— 该写法**已由 M164 正式改为报错**（见事实 192）：
       三轨一律 `R1002 字典索引键必须是字符串`；要键序请用 `d.keys()`（或 `for k in d`）。
     · 门：`examples/m163_dict_key_strict/`（三轨逐字节一致 16+6 断言 + **严格性层 8 用例 × 三轨** + 3 道负控）。
192. **迭代**位置语义**与用户索引分离（第 50 轮 · M164 收口缺陷 170/174/175）**：
     · **`d[k]`（用户索引）**：`k` 必须是字符串 —— 非字符串（`int` / `float` / `bool` / `null` / …）⇒
       **三轨一律** `R1002 字典索引键必须是字符串`。修前**解释轨**报 R1002，而 VM/C 轨返回
       **第 i 个键**（M37 为 `for k in d` 迭代内部把「位置语义」塞进了共用的 `px_index`）⇒ 三轨分叉。
       ⇒ 取键序请用 `d.keys()`；`for k in d` 照旧给出**插入序的键**。
     · **迭代协议**（`for x in it` / 推导式展开 `[e for x in it]` / `{k: v for k in it}` / 惰性生成器
       `(e for x in it)`）由**迭代专用入口** `px_iter_at`（C 轨）/ `PXOP_ITERAT`（VM 轨）提供：
       `dict` → 第 i 个键；`list` / `tuple` / `str` / `range` → 与整数索引等价。**用户面看不到它**。
     · **GenExp 的 `seq` 接受全部可迭代类型**（缺陷 174 修）：修前 `px_lazy_seq_get` 只认
       list/gen ⇒ `(c for c in "abc")` / `(e for e in (1,2,3))` / `(k for k in d)` 在 **VM/C 轨
       静默产出空生成器**（`for` 一圈都不跑），解释轨正常。`range` 看着正常只因 C 端早已物化成 list。
     · **GenExp 的形参遮蔽同名外层局部**（缺陷 175 修）：`(x for x in seq)` 的 `x` 在体内**遮蔽**
       同名的外层局部（与 C 轨 `cg_gen_lambda` 同一条真相）。修前 **VM 轨**把外层同名局部当成自由
       变量装箱捕获 ⇒ 体内读到外层值（`var c = "Z"` 时 `(c for c in ["1","2"])` 得 `ZZ`，解释/C 轨 `12`）。
     · **迭代期间修改被迭代容器** ⇒ **已由 M166 收口**（三轨一律 `R1003`，见事实 194）。
     · 门：`examples/m164_iter_index_split/`（三轨逐字节一致 22+10+4 断言 + **严格性层 5 用例 × 三轨**
       + 4 道负控）。
193. **求值顺序 = 词法左→右（第 51 轮 · M165 收口缺陷 178/179）**：
     · **一条规则**（对齐 **Go 规范**；C 标准**未定义**实参顺序 ⇒ 修前三轨分叉）：
       **表达式 / 赋值 / 函数实参一律按「词法左→右」求值**。
     · 赋值的求值点：`o[k] = v` → **目标对象 → 键 → 右值**；`o.f = v` → **目标对象 → 右值**。
     · **短路运算保持短路**：`a and b` / `a or b` / `a ?? b` / `c ?: a : b` 的右操作数
       **不会**被提前求值（把 `f()` 写在右边时，左边为假就**不执行** `f()`）。
     · 修前实测（**用户可见**的分叉，不是理论问题）：
       | 写法 | 解释轨 / VM 轨 | 修前 C 轨 |
       |---|---|---|
       | `f() + g()` | `F,G` | **`G,F`**（gcc 实参右→左） |
       | `"${f()}${g()}"` | `F,G` | **`G,F`** |
       | `print("A=" + str(l.pop()) + " len=" + str(len(l)))` | `A=3 len=2` | **`A=3 len=3`**（结果值都不同） |
       | `o[k] = v`（两侧都带副作用） | 解释轨 `V,O,K` / VM 轨 `O,K,V` | 同 VM |
       | `o.f = v` | 解释轨 `V,O` / VM 轨 `O,V` | 同 VM |
     · 修法：C 轨在一个表达式里若 **≥2 个操作数含副作用**，按序提升为临时变量
       （语句表达式 `({ LXValue _tN = …; <式>; })`）；纯操作数内联（无副作用 ⇒ 顺序无关）。
       解释轨 `i_assign` 改收右值 AST 节点、按上述点位求值。VM 轨本来就是字节码序（左→右）。
     · ⇒ **写法上不需要任何规避**：`f() + g()` 现在三轨都是 `f` 先、`g` 后；
       只有一条**建议**：想让读者看清顺序时，仍可先 `let a = f()` 再 `let b = g()`。
     · 门：`examples/m165_eval_order/`（三轨逐字节一致 24 断言 + 4 道负控；
       断言用**副作用日志**直接观测求值顺序，含「短路不被破坏」的正面保证）。
194. **迭代期间修改被迭代容器 ⇒ R1003（第 52 轮 · M166 收口缺陷 176）**：
     · **一条规则**：`for x in it` / 推导式在**进入循环时快照长度**；迭代过程中被迭代容器的长度
       **必须恒定** —— 变化（**增或减**）一律
         `R1003 迭代期间被迭代容器长度变化: <n0> → <n1>`
     · 修前实测（同一份源码 **四种**行为，除 VM 的越界外全部**静默**）：
       | 写法 | 解释轨 | VM 轨 | C 轨 |
       |---|---|---|---|
       | `for x in l: l.append(9)` | **活长度**多迭代 | 快照（3 次） | **活长度**多迭代 |
       | `for x in l: l.pop()` | 正常退出（跳过元素） | **越界 rc=1** | 正常退出 |
       | `for k in d: d.set(k,1)` | 键快照 | 键快照 | **活长度**（迭代到新键） |
       | `for k in d: d.remove(k)` | 键快照（迭代到已删键） | **越界 rc=1** | 中途停 |
     · 为什么选「响亮报错」：与 Python 对 dict 的 `RuntimeError: dictionary changed size during
       iteration` 同向（用户熟悉），只是把 list 也纳入 —— Python 的 list 会**静默跳过**元素；
       且三轨必须同一条真相（「活长度」下「边遍历边删」的结果依赖容器实现，跨轨必然漂移）。
     · **合法侧**（本门 18 断言覆盖）：只读迭代；**改元素值**（长度不变 ⇒ 合法）；`l[:]` /
       `d.keys()` / `d.values()` 先做**快照**；遍历 A 写 B；`while` 循环（**不受本约束**）；
       `range` / `str` / `tuple`；推导式（不改源）；`break` / `continue`。
     · **推荐写法**（「遍历时过滤/删」）：`let ks = d.keys()` / `let snap = l[:]` 先快照再遍历；
       或 `for v in d.values()`（返回副本）；或写到**另一个**容器。
     · 门：`examples/m166_iter_mutate/`（三轨逐字节一致 18 断言 + 严格性 6 用例 × 三轨 + 3 道负控）。
195. **解包（destructuring）统一（第 53 轮 · M167 收口缺陷 180/182）**：
     · **新语法：语句形式也能解包** —— `for a, b[, c…] in xs:`（此前只有**推导式**
       `[k for k, v in xs]` 能解包，语句形式 parser 直接 `E2001 期望 'in'`）。语义与推导式
       **同一条真相**：逐元素解包（元素是 list/tuple ⇒ 第 i 个变量取 `元素[i]`）。
     · **一条严格规则**（变量个数 N ≥ 2 时）：被解包值必须是 **list/tuple 且长度恰为 N**，否则
         `R1002 解包需要 list/tuple，实际是 <t>`         （元素不是 list/tuple）
         `R1002 解包需要 N 个元素，实际是 M`               （长度不等，无论少了还是多了）
       修前实测（同一份源码 **三轨四种行为**）：
       | 写法 | 解释轨 | VM 轨 | C 轨 |
       |---|---|---|---|
       | `[k for k, v in d]`（元素是键·字符串） | `R1002 推导式解包需要…` | **`R1003 字符串索引越界: 1`** | **同 VM** |
       | `[a for a, b in [[1]]]`（长度不足） | **静默给 `null`**（错值） | `列表索引越界: 1 (len=1)` | `R1003: 列表索引越界: 1 (len=1)` |
       | `[a for a, b in [[1,2,3]]]`（长度超出） | 静默忽略多出的 | 静默忽略 | 静默忽略 |
       ⇒ 为什么选「严格」而不是「缺位给 null / 多的忽略」：后者是**静默的错值**（与 M116/M120
       静默 null 族、M163 键严格化、M166 长度变化报错同一哲学：**响亮优于静默**），且只有「严格」
       能把三轨收敛成一条真相；与 Python 的 `ValueError: not enough/too many values to unpack` 同向。
     · **`dict.items()`（本轮新增）** ⇒ `for k, v in d.items()` 即 **Go `for k, v := range m`** 的
       对应写法（返回 `[[k, v], …]` **插入序快照**，与 `d.keys()`/`d.values()` 同口径）。
       ⚠️ **`for k, v in d` 会响亮报错**（dict 直接迭代只产**键**，键是字符串 ⇒ 非 list/tuple）——
       这不是坑，是**有指引的响亮**；要键+值就写 `.items()`。
     · 顺带收口**缺陷 182（VM 轨 · 循环变量被闭包捕获）**：装箱（cell）循环变量必须走 `CELLSET`，
       不能直写槽 —— 直写会把 cell 冲掉，随后闭包构造读到的就是**裸值**（`px_cell_get(裸值)` 走索引 ⇒
       `R1002 此类型不支持索引: int`）；**值为字符串时 `px_index("z",0)` 恰好返回 "z" ⇒ 静默碰对**，更难发现。
     · **已知边界（缺陷 181）已由第 54 轮 M169 收口** ⇒ 见下方事实 196（模块体绑定 = 模块级全局）。
     · 门：`examples/m167_unpack/`（三轨逐字节一致 16 断言 + 严格性 8 用例 × 三轨 + 3 道负控）。
196. **模块体 / 帧的「绑定归属」（第 54 轮 · M169 收口缺陷 181/183/184）**：
     · **模块体（顶层，含嵌套块）里的所有绑定 = 模块级全局** —— `var/let/const`、`for` 循环变量、
       嵌套 `def`、`chan` 一律如此：**模块体没有块作用域**。
       ```python
       for p in [1, 2, 3]:
           0
       print(p)              # 3   ← 循环后仍可引用（模块级绑定）
       if true:
           let x = 42
       print(x)              # 42  ← 嵌套块内的声明也是模块级
       for i in [7, 8]:
           0
       def f():
           return i          # 8   ← 模块级绑定对**函数**可见
       ```
     · **顶层闭包捕获这些绑定 ⇒ 按引用**（`fs[0]()` 与 `fs[1]()` 都看最后值）。要「每迭代一份」
       请显式做值快照（`let snap = q` 再捕获 snap）。
     · **闭包体（lambda 体）也是帧**：体内绑定提升到帧顶；体内被**内层闭包**捕获的局部先装箱成
       cell（与函数体同口径）—— `fn (): var s = 0; for k in …: s += k; return s + k` ⇒ 9；
       `fn (): var c = 0; let g = fn (): c = c + 1; …` ⇒ `[1, 2, 2]`（共享 cell）。
     · **帧内绑定归属**：**声明式**（`var/let/const`、嵌套 `def`、`for` 变量）⇒ **帧局部**（遮蔽同名
       模块全局，模块全局不被改）；**赋值式**（`x = …` 无声明）⇒ 名字是模块绑定 ⇒ 写**模块全局**，
       否则也是帧局部（`def f(): zz = 5` 之后模块体 `print(zz)` ⇒ 「未定义变量: zz」）。
     · 修前实测（同一份源码 **三轨四种行为**）：
       ...
       最后一行是**缺陷 184**：VM/C 轨的「帧局部判定」用了**惰性登记**的 `bc_is_global` —— 任何一次
       「读未知名」都会把该名变成「全局」，结论**依赖发射顺序**（谁先发射谁说了算）。
     · 修法：新增 `cg_topbody` / `cg_topnames`（C/VM 各一份，**确定性**）+ 闭包帧补 hoist/装箱预扫描
       + 模块体循环变量与嵌套 `def` 改 `px_set_global`/`SETG`。⇒ **三轨逐字节一致**。
     · 门：`examples/m169_toplevel_scope/`（15 断言 × 三轨 + 严格性 3 用例 + 4 道负控）。
...
198. **`px lint` 的作用域模型 = 语言的作用域模型（第 56 轮 · M171 收口台账缺陷 116）**：
     lint 曾按「一个函数体一个干净上下文 + 逐语句顺序」判名字可见性 —— M169 之后**结构性过时**，
     对**合法程序**报假 `E L002`。全仓实测 **151 个文件 3651 条 → 21 个文件 801 条**
     （`stdlib/collections.px` **46 → 0**）。六类全误报，逐条对照语言语义：

     | 写法 | 修前 lint | 真实语义（三轨一致） |
     |---|---|---|
     | 闭包读写外层局部（`var n` + `get`/`bump`） | 4×`E L002` + `W L001` | 帧捕获**按引用**（§17 ②） |
     | 同帧嵌套 `def g()` 调后面才定义的 `def h()` | 2×`E L002` | 编译器**帧顶 hoist** |
     | 赋值式绑定 `acc = []` / `k = 0`（无 var/let） | 每处 `E L002` | 赋值式命中可见绑定 ⇒ 写它；否则**就地成为本帧绑定**（§17 ④） |
     | 推导式 `[x*2 for x in nums if x>1]` 的 `x*2` | `E L002 'x'` | 推导式变量在**整个推导式**内可见 |
     | 模块体嵌套块里的 `let` + 函数引用**后面**的绑定 | 多处 `E L002` | 模块体（含嵌套块）**没有块作用域**（§17 ①） |
     | `import semver` → `<dir>/.px_modules/semver/semver.px` | 3×`E L002` | 编译器按 `module_search_paths` 解析（§8.6） |

     · 四条修法：① ctx 增 **`outer`（名字 → 拥有者 ctx）**，内层读/写外层名时 `used` **回写拥有者**
       （否则外层 `L001` 反向误报）；② **帧顶声明 hoist**（`lc_collect_frame_decls`，沿块递归、
       不进入嵌套帧；模块体同收）；③ **赋值式绑定就地声明**（`lc_bind_assign`，左值**不报 L002**）；
       ④ **推导式变量先声明**（提到结果表达式/iterable/cond 之前）。
     · 两处口径修正：**导入候选补 `.px_modules`**（⚠️ 实现坑：`list_dir` 返回**目录项名**不是全路径，
       必须补 `root + "/"` 前缀，否则候选相对 cwd 解析恒不存在）；**名册增第 ④ 源**——
       `px_serve`/`px_exec` 用 `px_dict_set(env, …)` 注入的 Web 全局 `REQUEST/GET/POST/SERVER`
       **既不经 `px_set_global` 也不经 `px_ffi_register`** ⇒ 派生名册漏收（`examples/webapp/*.px` 假 L002）。
     · 门：`examples/m171_lint_scope/`（`M171-VERIFY-OK` · 26 断言）——
       合法侧 5 例要求「lint **0/0** *且* 三轨 stdout **逐字节一致**」（只改绿 = 漏判；只一致 = 仍假红），
       真阳性侧 2 例（读未绑定名仍 `L002` / 声明未读仍 `L001`），仓内 8 个真实文件 0 错误，
       负控 3 道（帧顶 hoist / 推导式声明 / 外层帧可见链）各自独立判红。
     · **遗留**：`selfhost/` 子模块的 774 条属**已知单文件检查局限**（宿主提供名，M64 起留档，
...
202. **`d.get(k[, default])` 的三轨真相（第 57 轮 · M174 收口缺陷 194 · 晨曦 QA 清单 P2-7）**：
     规则（文档与解释轨一贯）：**默认值只覆盖「键不存在」**；**键存在但值为 `null` ⇒ 返回 `null`**
     （= Go 两值语义 / Python `dict.get`）。**缺陷 194**：runtime 侧此前用「取出的值是不是 null」
     判存在性（`px_is_null(v) && nargs >= 2`）⇒ 编译轨（VM/C）在「键存在 + 值 null + 给了默认值」时
     返回**默认值**、解释轨返回 `null` ⇒ 三轨分叉且与文档相悖。修法：改判 `px_dict_has`。
     · **读字典三条正路**（选哪条取决于「缺键算不算正常」）：
       | 写法 | 缺键 | 键存在但值 null |
       |---|---|---|
       | `d["k"]` / `d.k` | **R1008**（响亮，带源位置） | `null` |
       | `d.has("k")` 再取 | 你决定 | `true` ⇒ 取到 `null`（守卫写法） |
       | `d.get("k", 默认)` | 默认值 | **`null`**（不是默认值） |
     · 为什么主口径不是「缺键给 null」：M116 D2 的历史实锤 —— 移植 ws-center 时 `str(r["role"])`
       在编译轨下得到字符串 **`"null"`** 并**写进数据库**，解释轨当场报错 ⇒ **静默错值 > 报错**。
...
205. **零停机换二进制（第 57 轮 · M176 收口晨曦 P1-5 · SO_REUSEPORT）**：
     修前监听套接字只设 `SO_REUSEADDR`（TIME_WAIT 期可复用）⇒ **两个进程无法同时监听同一端口**
     ⇒ 每次换二进制必有空窗（晨曦三节点实测 **352ms / 75ms / 249ms**）。
     · 开关：`px_serve(port, doc, timeout, {"reuse_port": true})` · `http_serve(..., opts)` 同键；
       `sse_serve` / `tcp_listen`（**没有 opts 参数**）走环境变量 **`PX_REUSE_PORT=1|true|yes`**。
       四处 bind 共用 `px_want_reuse_port()` 一处实现。
     · **失败响亮**：内核不支持时 `px_error` 直接终止（不能假装设上了）；
       `bind` 失败文案补 `strerror` —— 换二进制最常见的失败是 `EADDRINUSE` =
       「**对端没开 SO_REUSEPORT**」，只说「绑定失败」会让人查错方向。
     · 升级链：**新进程启动（同端口）→ 健康门 → SIGTERM 旧进程**（M27 起「停 accept + 等在途」）
       ⇒ 监听套接字**没有一刻为空**。⚠️ 内核要求**双方都 opt-in** ⇒ 旧版也必须带开关启动；
       这条**是判据不是注释**：门的反向层就是「B 在监听时起 C（reuse=0）⇒ bind 必须失败」。
     · 门：`examples/m176_reuseport/`（`M176-VERIFY-OK` · 9 断言 · 真机时序）——
       700 次高频探测 **refused=0 bad=0** 才是「零停机」的定义；负控 1 道。
     · **门的自身纪律（两条，都是本轮实测踩的）**：
       ① **`trap` 挡不住 SIGKILL** —— 门被外部 kill 后 `runtime.c` 留在负控补丁态，
          下一次运行「主场景」全红，**看起来像功能坏了，其实是门没扫干净**。⇒ 门加**前置不变量
          自查**（进门先验锚点文本，缺一即 `rc=2`），宁可红在"自己没扫干净"，也不给假的绿/红。
       ② **门里每个「会等待」的动作都要有上界** —— `fetch` 没套 `timeout` 时 `cat <&3`
          永久阻塞、门挂了 6 分钟才被发现。

206. **两个引擎的「内置面」必须一致（第 57 轮 · M177 · 缺陷 165 的审计发现）**：
     把「GenExp 物化路径」按**矩阵**（内置 × 实参形态 × 三轨）展开后，跳出来的不是 GenExp 的
     捕获问题，而是**两个引擎的内置集/实参接受面根本不一致**：
     · **解释轨有、编译轨没有**：`dict()` / `unique()` / `flatten()`
       ⇒ 照解释轨写、一上编译轨 `R1001 未定义变量: 'dict'`（**空字典构造器**都没有！）。
       **为什么长期没人发现**：lint 名册取的是两来源**并集**（runtime ∪ interp Mini），
       名册门 ③④ 只判「⊆ 名册」这一个方向 ⇒ **一个盲区同时满足了所有门**，直到真去编译才炸。
       ⇒ M177 补 runtime 三个内置 + 名册门新增判据 **⑦「解释轨注册名 ⊆ runtime 可达名」**。
     · **`min(x)` / `max(x)` 单参数**：编译轨**静默返回实参本身**（`min(gen)` 得到**生成器对象**！），
       解释轨则报「需要两个参数」⇒ 一个静默错值 + 一个报错。现在两轨都支持「单参数可迭代 ⇒
       元素最值」，空可迭代 ⇒ `R1002 … 需要至少一个元素`（响亮）。
     · **`sum`**：编译轨只收 list、解释轨收 list/tuple ⇒ 现在统一收 **list/tuple/生成器**。
     · 统一实现：`px_as_list(v)` = `px_len` + `px_iter_at`（**与 `for x in xs` 同一条迭代语义**），
       三处共用；不再各写一套「收什么」。
     · ⚠️ **解释轨实现内置时的坑**：解释轨的生成器是 **dict `{"__gen__": [...]}`**（不是运行期
       `PX_GEN`）⇒ 直接交给 runtime 的 `list()` 会被当 dict 迭代出**键名**
...
207. **「可迭代实参」只有一条定义（第 57 轮 · M178）**：`list` / `tuple` / 生成器 / **字符串**（按 rune）。
     ...
     · **登记未修（缺陷 195）**：算术/比较运算的「码 + 措辞」三轨不同 —— 解释轨
       ...
       （无码）。取证 `examples/m178_iterable_args/err_sum_str.px`，**已由 M179 收口**（见事实 208）。

208. **运算族：一条真相（第 57 轮 · M179 收口缺陷 195/196）**：**先看值，再看文案** ——
     M178 登记的只是"措辞不同"，铺开矩阵后第一行是 **UB**：
     · ⚠️ **`num_val(v)` 对非数值读 `union.as.f`** = 读**对象指针的位模式**（UB）
       ⇒ `1.0 * "x"` 实测 **VM 轨 `6.905411902715e-310` / C 轨 `6.95199933934835e-310`**
       （同一台机、同一份源码，**两个不同的垃圾值**）；`1 / "x"` ⇒ `inf`、`1.0 + "x"` ⇒ `1.0`、
       `2 ** "x"` ⇒ `1.0`。解释轨一直是响亮的「需要数值」。
     · `1 < "x"` 编译轨 **静默 true**（`compare_values` 的类型名兜底 = 按 `strcmp(px_type_name)` 定序）；
       `1.5 & 1` ⇒ `1`、`[10,20][1.5]` ⇒ `20`（`int_val` 静默截断）。
     · 整数除零两侧**无码**（spec §11.2 早写明应 `R1006`）；索引越界三轨三种文（VM 轨连码都没有）。
     · 规则（§17.13）：算术要求**数值**（`str * int` 例外 = 重复，`n<=0` ⇒ `""`）·
       `< <= > >=` 要求**双数值或同类型** · 位运算与索引位置要求 **int** ·
       越界 `R1003: 索引越界: <i> (len=<n>)` · 整数除零 `R1006: 除零错误`；
       文案统一 `R1002: 无法相加: <ta> + <tb>` / `无法比较: <ta> vs <tb>`。
     · **`sorted`/`min`/`max` 是例外**：它们是**内部比较器 = 全序**（跨型按类型名，
       `sorted([1,"a",2])` ⇒ `[1, 2, a]`）—— 排序必须有全序才可能确定；`min`/`max` 单参数与变参
       **都已改成同一口径**（否则 `min([1,"a"])` 会变成「解释轨 1 / 编译轨报错」）。
     · **`==` / `!=` 不受影响**（跨型相等有意义：`1 == "x"` ⇒ `false`）。
     · 门：`examples/m179_arith_diag/`（`M179-VERIFY-OK` · 28 行合法侧矩阵 + 20 个错误用例
       三轨同码同消息体 + 负控 3 道：UB 恢复 / 解释轨文案退回 / 跨型比较放行）。

209. **帧内绑定的初始化：读到「声明尚未执行」⇒ `R1001`（第 59 轮 · M181 收口缺陷 193）**：
     帧局部槽的初值原来是 `px_null()`（VM 轨是 calloc 零值）—— 与「已声明、值就是 null」**不可区分**
     ⇒ `def f(): print(x); let x = 42` 在 **VM/C 轨静默打印 `null`**，而解释轨（参考）响亮报
     `R1001 未定义变量: 'x'`。**同族一整片**（都实测过）：

     | 写法（`f` 体内） | 修前 VM/C | 修后三轨 |
     |---|---|---|
     | `print(x)` 后 `let x = 42` | `null` | `R1001` ✅ |
     | 闭包引用 `x`，**声明前**调用 | `null` | `R1001` ✅ |
     | `for i in []` 后读 `i` / `while` 体 0 次迭代后的声明 | `null` | `R1001` ✅ |
     | `if c: let z = 1`（c=false）后读 `z` | `null` | `R1001` ✅ |
     | `let x = x + 1` | `R1002 无法相加: null + int`（与根因无关） | `R1001` ✅ |
     | `var x = 99` + 体内 `print(x)` 后 `let x = 1` | `null`（解释轨 **99**＝穿链） | `R1001` ✅ |
     | 外层帧 `var y` + 内层 `print(y)` 后 `let y = 9` | `null`（解释轨 **1**） | `R1001` ✅ |

     · **一条规则**：帧内读一个**本帧绑定**、而其**声明尚未执行** ⇒ 通道 **stderr** · 码 **`R1001`** ·
       消息体 **`未定义变量: '<名>'`** · `rc ≠ 0`（三轨一致）。
     · **初始化是单调的**（判据的一半）：声明执行过之后的任何读都合法 ——
       `let g = fn(): x` **先创建**、`let x = 1` **后声明**、`g()` **再调用** ⇒ 三轨都给 `1`（**合法程序**）。
       所以**不能**做编译期静态拒绝（那会误拒它）；只能运行期判 —— 与 Python 的 `UnboundLocalError` 同向。
       （Go 是词法作用域，同一份源码在 Go 里读**外层**的 `x`；PuXian 选 Python 那支 ⇒ 上表最后两行
       **必须**报错而不是穿链 —— 修前**解释轨自己**也在这儿穿链，M181 同批收口。）
     · **机制**：帧入口把**每个 hoist 局部槽**置为**未初始化哨兵**（C 轨 `px_uninit()`；VM 轨
       `PXOP_UNINIT`，**先于**装箱 `CELLNEW`）；只在**编译器无法证明声明已执行**的读点包检查
       （C 轨 `px_chk_uninit(v,"名")`；VM 轨 `PXOP_CHKINIT`）⇒ 其余发射文本**一字不变**（常规代码零代价）。
       判定用「发射顺序维护的 `inited` 集」：`if/else` 取**分支交集**、`while/for` 之后**回退入口集**、
       闭包体/嵌套 def 是新帧（起点 = 形参 ∪ 创建点已初始化的捕获名）。
     · **写代码时的自查**：`var x`（无初值）**语义是 null**，不是「未初始化」—— 真要「先用后赋」请
...
231. **`sorted(xs[, key_fn])` —— 可选 key 函数（第 81 轮 · M202 收口第三方 PX-DEF-034）**：
     ...
     · 门：`examples/m202_crypto_ext/`。
232. **「此类型不支持X / 不可迭代」族：三轨**同码同文**（第 82 轮 · M203 收口缺陷 243-a/b/c）**：
     · 这一族此前**四个站点只收口了一个**（M191 修的是「切片」）：

       | 站点 | 解释轨（修前） | 编译两轨（修前） |
       |---|---|---|
       | 切片 | `此类型不支持切片: <t>` ✅ M191 | `此类型不支持切片: <t>` |
       | **索引** | `此类型不支持索引`（**缺类型**） | `此类型不支持索引: <t>` |
       | **迭代** | `此类型不可迭代`（缺类型） | **`len 不支持类型 <t>`** ← 借了用户**从未写过**的 `len` |
       | **索引赋值** | `索引赋值目标不支持`（**整个词条都不同**） | `此类型不支持索引赋值: <t>` |

     · 统一口径（**三轨一条真相**）：
       · 不可索引 ⇒ `R1002 此类型不支持索引: <t>`
       · 不可迭代 ⇒ `R1002 此类型不可迭代: <t>`
       · 不可索引赋值 ⇒ `R1002 此类型不支持索引赋值: <t>`
     · **为什么把「迭代」单独起一个词条**（而不是并进「不支持索引」）：迭代与索引在 M164
       已经明确分开（`px_iter_at` ⇄ `px_index`）——拿索引的词条去描述迭代，正是 M164 要根治的
       那类混淆。
     · **编译两轨不再借 `px_len`**：新增 `int px_iter_prepare(LXValue)`（runtime）——
       可迭代（str/bytes/list/dict/tuple/生成器）⇒ 返回长度；否则响亮报上面那条。
       C 轨直接发射 `px_iter_prepare(...)`（for 循环 + 推导式两条发射点）；
       VM 轨经**内部全局** `__iter_len`（`__` 前缀 ⇒ **不进公开 native 名册/索引**，
       见 `tools/gen_native_table.sh` 的过滤）调用**同一个**函数 ⇒ 三轨共用一份实现。
     · 零回归：可迭代面上的迭代/索引/索引赋值语义**一字未动**（门 `examples/m203_iter_index_msg/`
       的正例逐字节一致：7 种可迭代类型 · 两条发射点 · 函数内/闭包内/嵌套/空迭代 · 索引 5 型）。
     · ⚠️ **同轮顺带发现（→ 缺陷 245 · M204）**：C 轨的**复合赋值**在 `Index` / `Field` 目标上
       **静默丢弃运算符** —— `let m=[1,2,3]; m[0] += 10` 实测 C 轨 `[10, 2, 3]`（解释/VM 轨 `[11, 2, 3]`）、
       `p.x += 10` C 轨 `10`（另两轨 `11`）；`l[0] = l[0] + 100` 三轨都对 ⇒ 只坏在「复合」这一条路。
       本门刻意**不放**这类用例（归 M204 专项）。
233. **复合赋值（`+=` `-=` …）在 `Index` / `Field` 目标上的三轨同一真相（第 83 轮 · M204 收口缺陷 245/246）**：
     · **缺陷 245（高严重度 · C 轨静默错值）**：`selfhost/cg_stmt.px` 在 `Index` / `Field` 目标上
       **无条件**发射 `px_index_set(o, i, rhs)` / `px_field_set(o, "f", rhs)` —— `op` 被**静默丢弃**
       （等价于"直接赋值"）。`Var` 目标那一支走 `cg_assign_op_local`，一直是**对的**。
       实测（同一份源码）：
     ...
     · 修法：`op != "Assign"` 时把目标对象/键**先落 `_sN` 临时**（**各求值恰一次**），
       ...
       `_sN` 走**专用计数器** `cg_seq_new()`（**不占** `cg_tmp()` 的 `_tN` 序号 ⇒ 其余发射文本
       一字不变 —— M165 同款纪律，冻结门差异面才可控）。
     · **求值次数与顺序**（守 M165 的**词法左→右**：目标对象 → 键 → 右值）：
       `getL()[idx()] += rhs()` 的副作用日志实测 `[o, k, v]`（目标对象/键**各恰一次**）；
       对比显式展开 `getL3()[0] = getL3()[0] + 1` ⇒ `[o, o]`（两次）。
     · **缺陷 246（同轮实测照出）**：复合赋值到**不存在的成员**时，解释轨把"缺失"当 `null` 硬算 ⇒
       `d["k"] += 1`（k 不存在）解释轨 `R1002 无法相加: null + int`，而 VM/C 轨 `R1008 字典没有键 'k'`；
       `p.z += 1` 同理（`R1008 结构体没有字段 'z'`）。⇒ 统一为**与成员的「读」一致**（R1008；
...
237. **`//` 与整数 `%` 是「**欧几里得对**」—— 负除数下**不等于** Python（第 93 轮 · M214 · 缺陷 301）**：
     · 实测（三轨逐字节一致）：`17//-3` = **`-5`**（Python `-6`）· `17%-3` = **`2`**（Python `-1`）·
       `-17//-3` = **`6`**（Python `5`）· `-17%-3` = **`1`**（Python `-2`）；12 组探针里 **7 组**与 Python 不同（全是负除数）。
     · 我方语义 = **Rust `div_euclid` / `rem_euclid`**：**余数恒 `∈ [0, |b|)`**，
       且 `a == (a//b)*b + (a%b)` **恒成立**（三轨 12/12 都满足）。Python 是 floor-division
       （余数取**除数**符号）⇒ 两者**各自自洽、但互相不同**。
     · `docs/spec.md` 早先说「`//` 整除…**同 Python**」—— **那句话不准确**，已更正。
     · 移植纪律：**从 Python 移植带负数的 `//` / `%`**（取模索引、环形缓冲、日期偏移、
       `hash % n` 的负数分支）⇒ **必须显式验算**，别假定「跟 Python 一样」。
     · 与浮点 `%` 不是一回事：浮点 `%` = `fmod`（余数取**被除数**符号，`-5.0 % 3.0` = `-2.0`）。
     · ⚠️ 本仓历史上**没有**这条的门 ⇒ 语义只靠「实现一致 + 文档」维持（M214 已把
       `spec.md` 改准；逐值对拍的门列下一轮候选）。

238. **上游 `registry-px` 已到 124 库（M214 引入 16 件）**：新增
     `actor` `bcrypt` `holidays` `lz4` `mock` `netprobe` `pinyin` `protobuf` `seg`
     `semaphore` `sqlparse` `tdtest` `tzmini` `x509` + `fsnotify 0.2.0` / `xlsx 0.2.0`
     （后两者是**同包新版本目录**，不是新包名 —— 数「引入件数」时要按 `<name>/<version>` 算）。
     · 双轨回归 **233 通过 / 0 失败 / 23 跳过**；`bcrypt_test` 解释轨
       **`>>180s`**（bcrypt 刻意慢 · 2^cost 轮；上游文件头自注「解释轨极慢 → 验收以编译轨为准」）
       ⇒ 按**性能性 SKIP** 登记（与 `qrcode_test` 同款处置）。
     · `mock` / `actor` / `semaphore` 的解释轨是**设计性 SKIP**（需 `spawn`/`chan`/`mutex`，Mini 子集排除）。
239. **整数 `//` / `%` 的 **int64 边界口径** —— 商不许"先算余数再减"（第 94 轮 · M215 · 缺陷 305 / 306）**：
     · 语义不变（仍是**欧几里得对** = Rust `div_euclid`/`rem_euclid`，见事实 237）；本轮收的是**实现口径**：
       商**不能**由 `(n - r) / d` 求 —— `n` 逼近 `INT64_MIN` 且 `r > 0` 时 `n - r` **溢出**，
       而构建**没有** `-fwrapv`（签名溢出 = UB，`-O2 -flto` 下编译器可合法地做任何事）。
       实测回绕后果是**商符号翻转**：`-9223372036854775807 // 3` 曾静默给
       **`+3074457345618258602`**，真值 **`-3074457345618258603`**（30 值域 × 930 对里 **65 对**错）。
     · 现口径 = **截断商 + ±1 调整**（Rust `div_euclid` 的写法），全程不做可溢出的减法；
       余数调整由 `r += (d<0 ? -d : d)` 改为 `r -= d`（`-d` 在 `d == INT64_MIN` 上同样溢出）。
     · **唯一不可表示**的情形：`INT64_MIN // -1`（真商 `+2^63`）⇒ 回绕 **`INT64_MIN`**、
       `INT64_MIN % -1` ⇒ **`0`**。理由：与「一元负号在 `INT64_MIN` 回绕（`-INT64_MIN == INT64_MIN`）」
       同族、与「`+`/`*`/`<<` 静默回绕」同族，且**回绕算术下不变量 `a == q*b + r` 仍成立**
       （`INT64_MIN * -1` 回绕回 `INT64_MIN`，故 `r = 0`）。
     · ⚠️ **边界上 `q*b` 本身会溢出** ⇒ `a == q*b + r` **不能在 int64 内验算**，
       真值必须用更宽域（本仓门用 **Python bignum + Go uint64 量级域** 两套独立实现互校）。
     · ⚠️ 修前 `INT64_MIN // -1` 与 `% -1` 会让进程 **SIGFPE + core dump**（硬件 `idiv` 陷阱，三轨同崩）。
     · ⚠️ **为什么旧门看不见**：病灶在运行时原语内部，且 `px_idiv`（runtime.c）与 `pxc_vm` 的
       `PXOP_IDIV`/`PXOP_MOD` 快路径**各写了一遍同样的公式** ⇒ **三轨跑的是同一份语义、输出逐字节一致**
       ⇒ **任何「三轨对拍门」按定义看不见它**。收口办法 = 把公式收敛到**唯一实现**
       （`runtime.h` 的 `px_idiv64` / `px_imod64`，两处共用）+ **引入独立真值**。
     · 逐值对拍门：`examples/m215_int_truth/`（930 对 + 不变量 + 陷阱回归 + 负控 3 道）。

240. **大整数的比较必须**按整数**精确比较（第 94 轮 · M215 · 缺陷 307）+ 两条文档补正**：
     · **缺陷 307**：解释轨的 `i_bin_compare` 对 **int/int 也先转 `float`**，而 double 只有 **53 位尾数**
       ⇒ 超过 `2^53` 的整数被舍入到**同一个** double：
       `9223372036854775806 < 9223372036854775807` 给 **false**（真值 true）· `2^53 < 2^53+1` 给 false ·
       `2^62-1 < 2^62` 给 false · `INT64_MIN < INT64_MIN+1` 给 false。
       而编译轨（runtime `px_lt/le/gt/ge`）**第一分支**就是 `a.as.i < b.as.i` ⇒ **三轨分叉**。
       现口径：**int 对 int ⇒ 整数比较；任一侧是 float ⇒ double 比较**（与 runtime 逐条对齐）。
       ⚠️ 同族的 `i_cmp_values_raw`（`sorted`/`min`/`max` 走的那条）**早就有** int-int 分支 ——
       故这是「**同一语义面只收口了一处**」的旧疾，不是新引入。
     · **文档补正 A**：`INT64_MIN` **不能写成字面量** —— 词法器按正数解析 `9223372036854775808`
       ⇒ `E1004 无效整数`（与 Go 同款限制）；须写 `-9223372036854775807 - 1`。
     · **文档补正 B**：**移位计数**走的是宿主 C 的**掩码**行为（`1 << 64` = **1**、`1 << 65` = 2、
       `1 << (0-1)` = `INT64_MIN`）—— 文档此前未明示，已登记（下一轮候选）。
     · **未纳入本门的邻居（如实登记）**：**浮点→int 转换族** —— `int(1e30)` / `int(-1e30)` /
       `int(2^63)` / 浮点 `//` 越界在 x86 实测**一律给 `INT64_MIN`**（`cvttsd2si` 的"不定值"），
       而 ARM 的 `fcvtzs` 是**饱和**语义 ⇒ **疑似平台相关**，需交叉实测确认后另开一轮。
       ➜ **已在第 95 轮（M216 · 缺陷 308）收口 —— 见事实 241 / 242。**

241. **浮点→int 只有**一个入口 `px_f2i`：越界 / 非有限一律**响亮**（第 95 轮 · M216 · 缺陷 308）**：
     · 修前全仓**各自**写 `(int64_t)f`（`int()` 内建 · `//` 的浮点支 · 内部的 `int_val`）——
       而 C11 **6.3.1.4p1** 规定「整数部分无法由目标类型表示 ⇒ **未定义行为**」：
       x86_64 的 `cvttsd2si` 给 **`INT64_MIN`**（不定值哨兵 `0x8000…`）、
       aarch64 的 `fcvtzs` **饱和**给 `INT64_MAX` ⇒ **同一份源码在两个架构上打印不同的数**（且都静默）。
       实测修前 `int(1e30)` · `int(-1e30)` · `int(1e19)` · `int(float(9223372036854775807))` ·
       `1e30 // 1.0` —— **三轨一律** `-9223372036854775808`。
     · **现行口径**（与同族 `px_str_to_i64`「str→int 溢出即非法 ⇒ 响亮」**同向**）：
       **非有限（NaN / ±Inf）或落在 `[INT64_MIN, 2^63)` 之外 ⇒ `R1003`**；区间内 ⇒ 向零截断。
       ⚠️ **上界是 `2^63` 而不是 `2^63-1`** —— 因为 `9223372036854775807.0` 在 double 里
       **本身就是** `2^63`（该整数不可表示）⇒ `int(float(INT64_MAX))` **响亮报错**。
       这是诚实的：那个 double **不等于** `INT64_MAX`，静默给一个数才是错的。
     · **良定义的两个端点**（可放心用）：`int(-9223372036854775808.0)` → `INT64_MIN` ·
       `int(9223372036854774784.0)` → 最大的、< `2^63` 的 double（`2^63 − 1024`）。
     · 消息形态：`R1003: int() 超出 int64 范围: 1e+30` · `R1003: int() 得到 NaN，无法转为 int` ·
       `R1003: 整除 超出 int64 范围: 1e+30`（三轨同码同文）。

242. **切片界与**索引位**同口径：必须是整数（第 95 轮 · M216 · 缺陷 308 的第二个面）**：
     · 修前**三轨分叉**：解释轨把 float 界**直接参与整数比较**，编译两轨走 `int_val` 的
       `(int64_t)1e30` = `INT64_MIN` ⇒ `l[0:1e30]` **解释给 `[1,2,3]` 而 VM/C 给 `[]`**；
       `l[1e30:2]` **解释给 `[]` 而 VM/C 给 `[1,2]`**；`bytes_slice(b, 1e30, 2)` 三轨都**静默给错值**。
     · **现行口径**：切片界（含 `step`）必须是 `int`，否则
       `R1002: 切片索引必须是整数，实际是 <t>`（与**索引位** M179 的 `索引必须是整数` 同族）。
       ⇒ 若代码里写了 `xs[0:len(xs)/2]`（`/` 给 **float**），现在会**响亮报错**；
       应改写 `xs[0:len(xs)//2]`（`//` 给 **int**）。Python 3 同样拒绝 float 切片界。
     · `int_val`（**内部**入口）也**不再静默截断 float** —— 修前 M179 / M194 收紧的只是
       native **参数位**（改走 `px_arg_int` / `px_req_int_idx`），内部位一直留着同一条口子。


243. **移位计数的越界语义（第 96 轮 · M217 · 缺陷 309）**：`<<` / `>>` / `>>>` 的**计数** `n`
     · `0 <= n <= 63` ⇒ 正常移位。
     · `n >= 64` ⇒ **掩码 `n & 63`**（即 `1 << 64` == `1 << 0` == `1`；`1 << 65` == `2`）。
       这与 `>>>` 早已有之的口径、以及 Rust `wrapping_shl/shr` **一致** ⇒ **零行为变更**。
     · `n < 0` ⇒ **`R1003: <运算符> 计数不能为负数，实际是 -1`**（三轨同码同文）。
       ⚠️ **这是行为收紧**：修前 `1 << -1` 静默给 `INT64_MIN`（`-1 & 63 == 63` 的掩码副作用）。
       Python 是 `ValueError: negative shift count`、Go 是 run-time panic ⇒ 本仓取「响亮」，
       与 M216（`int(1e30)` 越界响亮）**同向**。
     ⚠️ **为什么这件事值得单独立一条**：`a << n` 在 `n < 0` / `n >= 64` / **左移结果不可表示**
       时是 **C11 6.5.7p3/p4 的未定义行为**。同一份 C 实测：
       `f(1,64)`（运行期）**-O0 给 1、-O2 给 0**；常量 `1LL << 64` gcc 直接**告警**并给 0。
       ⇒ 「三轨一致」只是**恰好**都走 `shl` 指令（硬件会掩码），与语义无关；
       换个优化档/编译器版本/内联决策就会**静默变答案**。现行实现把语义**显式定义**下来：
       计数先 `& 63`、左移**一律经 `uint64_t`**（无符号回绕**良定义**）⇒ 优化再也改不动它。

244. **移位有唯一的实现入口（不要各自写裸 `<<`）**：`runtime.h` 的
     `px_shift_count` / `px_shl64` / `px_shr64` / `px_shru64` 是**唯一实现**，
     `px_shl`/`px_shr`/`px_ushr`、VM 轨 `PXOP_SHL/SHR/SHRU` 三个快路径、
     解释轨 `i_bin_numeric` 的三个分支**全部收敛到它**。
     · **修前**同族 4 个移位**只收口了 1 个**：`px_ushr` / `PXOP_SHRU` 早已显式 `& 63u`
       （注释明写「与解释器 `wrapping_shr` 一致」），`<<` / `>>` 一直是**裸移位**
       ⇒ 这是 M191 / M203 / M206 / M215 那条「**同一语义面只收口一处**」老病的第 N 次复发。
     · ⇒ 加/改移位语义时**只改上面四个函数**，不要在别处再写 `<<`/`>>`。
     · ⚠️ 顺带：`INT64_MIN` **不能**写成字面量 `0 - 9223372036854775808`（词法 `E1004`），
       必须写 `0 - 9223372036854775807 - 1`（M215 事实 240 已记，本轮语料又踩一次）。
