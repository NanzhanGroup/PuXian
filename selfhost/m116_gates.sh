...
# M196（第 74 轮）：诊断**文案**的三轨单一真相（缺陷 227 + 230）—— 解释轨的守卫**先于**原生
#   触发，于是它自己写的那条文案会盖住原生那条，而编译轨拿到的是原生那条 ⇒ 同一坏输入三轨两文。
#   静态普查 180 个「两边都有文案」的函数：批 1 对齐 49 条（缺「参数」尾词 / 数字写法 / 词面 /
#   函数面）+ 批 2 对齐 5 条（类型文案 `不支持此类型` → `不支持类型 <t>`）；顺带修缺陷 230
#   （`gen_next` 的 `let g = args[0]` 在 arity 检查之前 ⇒ 0 参撞 R1003 索引越界）。
...
step "M199 · [S10]「同一操作 × 每一位置 × 错类型」三轨单一真相（缺陷 237 三元兜底 + 238 解释轨路由）"
#   • 静态 [S10-a]：**全部已注册 native** 的逐参守卫声明（400+ 位置）+ 「三元兜底」站点 ⇄ 登记表对拍
#     （未登记 0 · 表漂移 0）。扫描器须收 `static` 与非 `static` 两种定义 —— 首版只收 static，
#     `ws_heartbeat` 整函数**静默漏掉**（假绿），本轮实测踩到。
#   • 动态 [S10-b]：从声明**自动生成**探针（全参类型正确、只第 P 位为毒值 `{}`），合成**一个**驱动器
#     ⇒ 两次编译覆盖全量；三轨（解释/VM/C）对拍 rc + R 码 + 消息体 + stdout，**分叉必须为 0**
#     （一例一次 build 需 7+ 小时 —— 驱动器把「逐参类型面」变成可**全量**度量）。
...
step "M203 · 「此类型不支持X / 不可迭代」族的三轨同码同文（第 82 轮 · 缺陷 243-a/b/c）"
#   这一族**四个站点只收口了一个**（M191 修的是「切片」）：
#     索引     解释轨 `此类型不支持索引`（缺类型）   ⇄ 编译轨 `此类型不支持索引: <t>`
#     迭代     解释轨 `此类型不可迭代`（缺类型）     ⇄ 编译轨 **`len 不支持类型 <t>`**（借了用户没写过的 len）
#     索引赋值 解释轨 `索引赋值目标不支持`（词条全不同）⇄ 编译轨 `此类型不支持索引赋值: <t>`
#   ⇒ 统一为「带实际类型」的三条词条；新增 runtime `px_iter_prepare`（三轨共用一份实现）：
#   C 轨两条发射点直接调用它，VM 轨经**内部全局** `__iter_len`（`__` 前缀 ⇒ 不进公开名册/索引）。
#   判据：[1] 静态（四站点 + 两条发射路径都接了新入口 + `__` 内部名不进索引）·
#   [2] 正例 2 组 × 三轨逐字节一致（零回归）· [3] 拒绝侧 8 例 × 三轨
#   （rc≠0 + 同码 + **整条消息体逐字相同**）· [4] 负控 5 道（A/B/C 三个解释轨站点 +
...
# M208s1（缺陷 273）：**交叉档可编译性**本地门 —— `runtime/vm.c` 少了 `<unistd.h>`，
#   宿主 gcc 侥幸通过而三档交叉 cc（clang/musl）一律 `undeclared function 'write'` ⇒ CI m67 三 job 全红。
#   判据：三档各自独立（有工具链+库 ⇒ 必须编译成功且产物架构正确；缺 ⇒ 记 SKIP 并打印原因；
#   全档 SKIP ⇒ 门整体 SKIP 并指明权威覆盖在 ci.yml 的 m67）。含 `--self-test` 三条自证。
run cross_ports bash selfhost/check_cross_ports.sh
