...
if [ "${1:-}" = "--fresh" ] || ! cache_valid; then
    # M113-S1：此处的时长改为实测值 —— 入库件重烘后（M112 性能修复在册）编 compiler.px 约 8s；
    #   而入库件落后源码时（S0 实测：09-07 的旧 pxc）同一动作要 **411s（≈50×）**。
    #   这也正是 Issue 58 的代价面：不重烘，用户拿到的就是慢 50 倍的编译器。
    echo "── 步骤 1：bootstrap/pxc 编译 compiler.px → B.c（重烘后实测约 8s；入库件落后源码时可慢约 50×）"
