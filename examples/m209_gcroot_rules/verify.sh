...
step "③ 静态：旧规则对照（--trigger-set legacy --grow）候选 = 10 且全部带「旧规则」标记"
...
# ⚠️ M213（第 92 轮 · 缺陷 293）：**11 → 10** —— 旧规则下 `bi_session_del ← px_dict_set(`
#   是一条候选，而 M213 把该函数与 `bi_session_set` **对称化**（补 `px_root_push()` +
#   逐个 `PX_KEEP` + 出口 `px_root_pop()`）⇒ 它不再是候选。**这类「对照计数」必须
#   跟着产品侧修复一起改**，否则门会把「修好了」误报成「对照被清空」。
if [ "${GROWN:-0}" = 10 ] && [ "${UNTAG:-1}" = 0 ]; then
    ok "旧规则候选 10 条、全部标记为「隐式分配·旧规则」（对照有效）"
else
    bad "旧规则对照不符：候选 ${GROWN:-?}（期望 10）· 未标记 ${UNTAG:-?}（期望 0）"
...
' '' NA; then
    ...
    # B：把旧规则设为默认 ⇒ ② 必红（候选 10 · M213 缺陷 293 后由 11 降为 10）
