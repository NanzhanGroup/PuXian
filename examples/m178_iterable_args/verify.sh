...
#   [4] 登记项（**不计失败**，必须打印编号）：`sum("12")` 三轨措辞仍不同（缺陷 195：
#       算术/比较运算的「错误码 + 措辞」三轨未统一 —— 解释轨 `+ 不支持: int + string`
#       带 R1002，编译轨 `无法相加: int + string` 无码）
...
echo "   ↑ 三轨都响亮报错（rc≠0），但**措辞不同**（解释轨 + 不支持: … / 编译轨 无法相加: …）⇒ 归 M179"
