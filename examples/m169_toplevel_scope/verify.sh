...
#      （⚠️ 错误**文本格式**：M172（缺陷 186）起 **通道与措辞已统一** ——
#        三轨都写 stderr、都带 `R1001` 与 `未定义变量: 'x'`；**仅位置前缀保留各自最优信息**
#        （解释轨 `行:列` vs 编译轨 `[fn 行N]`，runtime 无列号）⇒ 本门仍只比 stdout、
#        并用 rc + 关键字判「响亮报错」，**不按整行对拍**。见 examples/m172_diag_channel/）
...
#   ⚠️ 遗留差异（M172 后仅剩一条，且**有意保留**）：
#     ① 通道 —— **已统一**（M172：三轨都写 stderr）；
#     ② 措辞 —— **已统一**（M172：三轨都带 `R1001` + `未定义变量: 'x'`）；
#     ③ **位置前缀**保留各自最优信息：解释轨 `行:列` vs 编译轨 `[fn 行N]`（runtime 无列号）。
