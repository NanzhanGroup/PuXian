...
cat <<'EOF'
   ...
   · **现场回归（由全量门抓回 · 已固化为合法侧 4 例）**：`selfhost/ival.px` 的 `i_fmt_float`
     曾用 `int(f)` 做**整值判断**，而它的守卫按 **double 范围**短路（`f > DBL_MAX`）
     ⇒ **`DBL_MAX` 本身不算越界** ⇒ 落到 `int(DBL_MAX)`：修前静默给 INT64_MIN
     「碰巧」不出错（= **拿缺陷本身当护栏**），M216 把越界改成响亮后 ⇒
     `m148_ieee_div` 的解释轨当场 rc=1。修法 = 前置条件由 double 范围改为 **int64 范围**
     （`absf < 1e15 and f == int(f)`，`and` 短路 ⇒ `int(f)` 必在范围内）。
     ⚠️ **方法论教训**：M194 的「收紧类型口径前先看生态用法」本轮只做了一半 ——
     扫了**语法形态**（切片界含 `/`），**漏了「该原语的既有消费方」**。
     ⇒ 规矩升级：**收紧一个共享原语前，既扫语法形态，也扫全部消费方。**
EOF
