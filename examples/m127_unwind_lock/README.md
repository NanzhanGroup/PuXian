...
| B 关键（**持 `g_gc_mu`** 失败） | 进程 10s 内退出；文案含「无法安全回滚」＋持锁清单列出 `&g_gc_mu` ＋假死后果 ＋ `_exit(1)` ＋处理建议；**未走 longjmp**（用例内加 `PX_GROW_RETRY_MAX=0`，见 §6） |
...
## 6 后续：M128 已落地（本审计闸改为「兜底」）

M128（`examples/m128_unlock_grow`）已把 `px_list_push` / `px_dict_set` / `gc_register` 三处的
扩容与键副本分配**搬出临界区**（两阶段：锁外备货 → 锁内只做指针发布）⇒ 这些站点上的分配失败
现在落到**请求级 5xx**，不再需要本审计闸介入。

因此本门的 **B / C 用例显式加 `PX_GROW_RETRY_MAX=0`**：该开关把扩容压回**锁内**（= M127 及
以前的行为，源码里保留了 `px_list_push_locked` / `px_dict_set_locked`），以便继续验收审计闸
本身（「万一还有锁内分配失败，绝不留锁假死」）。这不是为了绕过失败，而是因为默认路径上那条
失败路径**已被 M128 消除**（这正是 M128 的成果）。

⚠️ 默认路径下**仍有**锁内分配站点（M128 门 C9 实测）：`px_rate_limit_try`（`g_rate_mu`，请求
路径）· `fserve_ensure`（初始化）· `gc_init_env`（一次性）⇒ **本审计闸仍然是必需品**，
并已列为 M129 候选（见 `examples/m128_unlock_grow/README.md` §5）。
